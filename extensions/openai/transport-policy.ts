...
import { normalizeProviderId } from "velaclaw/plugin-sdk/provider-model-shared";
...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-velaclaw-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-velaclaw-turn-id": turnId,
      "x-velaclaw-turn-attempt": attempt,
    },
    metadata: {
      velaclaw_session_id: sessionHeaders["x-velaclaw-session-id"] ?? "",
      velaclaw_turn_id: turnId,
      velaclaw_turn_attempt: attempt,
      velaclaw_transport: ctx.transport,
    },
  };
}
