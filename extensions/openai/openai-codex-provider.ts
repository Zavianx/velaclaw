...
import { buildOauthProviderAuthResult } from "velaclaw/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "velaclaw/plugin-sdk/provider-auth-login";
import {
  DEFAULT_CONTEXT_TOKENS,
  normalizeModelCompat,
  normalizeProviderId,
  type ProviderPlugin,
} from "velaclaw/plugin-sdk/provider-model-shared";
import { fetchCodexUsage } from "velaclaw/plugin-sdk/provider-usage";
import { normalizeLowercaseStringOrEmpty, readStringValue } from "velaclaw/plugin-sdk/text-runtime";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `velaclaw models auth login --provider openai-codex` or `velaclaw configure`.";
}
