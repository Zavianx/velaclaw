import { hasConfiguredSecretInput } from "velaclaw/plugin-sdk/secret-input";
import {
  ...
  type VelaclawConfig,
} from "velaclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "velaclaw/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: VelaclawConfig, accountId: string): VelaclawConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: VelaclawConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: VelaclawConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: VelaclawConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: VelaclawConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: VelaclawConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: VelaclawConfig; accountId: string }) =>
    ...
    disable: (cfg: VelaclawConfig) => setSetupChannelEnabled(cfg, channel, false),
