import { describeAccountSnapshot } from "velaclaw/plugin-sdk/account-helpers";
import { formatAllowFromLowercase } from "velaclaw/plugin-sdk/allow-from";
import {
...
import { patchChannelConfigForAccount } from "velaclaw/plugin-sdk/setup-runtime";
...
import { getChatChannelMeta, type ChannelPlugin, type VelaclawConfig } from "./channel-api.js";
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Velaclaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Velaclaw`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "Velaclaw"): string[] {
  return [
