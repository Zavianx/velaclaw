...
// Prefix for Velaclaw-generated action IDs to scope our handler
const VELACLAW_ACTION_PREFIX = "velaclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${VELACLAW_ACTION_PREFIX}`);

  // Handle Velaclaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
