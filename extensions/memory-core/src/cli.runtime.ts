...
import { resolveMemoryRemDreamingConfig } from "velaclaw/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "velaclaw/plugin-sdk/routing";
...
function resolveMemoryPluginConfig(cfg: VelaclawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: VelaclawConfig): string {
...
function resolveAgent(cfg: VelaclawConfig, agent?: string) {
...
function resolveAgentIds(cfg: VelaclawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: velaclaw memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-rem-backfill-"),
      );
      try {
