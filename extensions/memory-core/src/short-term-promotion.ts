...
import { formatMemoryDreamingDay } from "velaclaw/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "velaclaw/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "velaclaw-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*velaclaw-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*velaclaw-memory-promotion:([^\n]+?)\s*-->/gi);
