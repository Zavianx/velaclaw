...
import {
  ...
  formatHelpExamples,
  theme,
} from "velaclaw/plugin-sdk/memory-core-host-runtime-cli";
import type {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["velaclaw memory status", "Show index and provider status."],
          [
            "velaclaw memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["velaclaw memory status --deep", "Probe embedding provider readiness."],
          ["velaclaw memory index --force", "Force a full reindex."],
          ['velaclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'velaclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `velaclaw memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "velaclaw memory rem-backfill --path ./memory",
          ...
          ["velaclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.velaclaw.ai/cli/memory")}\n`,
    );
