...
import { withFileLock } from "velaclaw/plugin-sdk/file-lock";
import {
  ...
  writeFileWithinRoot,
  type VelaclawConfig,
} from "velaclaw/plugin-sdk/memory-core-host-engine-foundation";
import {
  buildSessionEntry,
...
} from "velaclaw/plugin-sdk/memory-core-host-engine-storage";
import { resolveAgentContextLimits } from "velaclaw/plugin-sdk/memory-core-host-engine-foundation";
import {
...
const MCPORTER_STATE_KEY = Symbol.for("velaclaw.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("velaclaw.qmdEmbedQueueTail");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so Velaclaw search semantics match direct qmd search.
  return trimmed;
}
...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  private readonly cfg: VelaclawConfig;
