...
import { resolveMemoryDreamingConfig } from "velaclaw/plugin-sdk/memory-core-host-status";
import { normalizeLowercaseStringOrEmpty } from "velaclaw/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: VelaclawConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: VelaclawConfig, enabled: boolean): VelaclawConfig {
...
function formatStatus(cfg: VelaclawConfig): string {
...
export function registerDreamingCommand(api: VelaclawPluginApi): void {
  api.registerCommand({
    name: "dreaming",
