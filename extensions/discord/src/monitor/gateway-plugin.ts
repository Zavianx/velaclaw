...
import type { DiscordAccountConfig } from "velaclaw/plugin-sdk/config-runtime";
...
} from "velaclaw/plugin-sdk/proxy-capture";
import { danger } from "velaclaw/plugin-sdk/runtime-env";
...
export function resolveDiscordGatewayIntents(
  intentsConfig?: import("velaclaw/plugin-sdk/config-runtime").DiscordIntentsConfig,
): number {
  let intents =
...
function createGatewayPlugin(params: {
  ...
  class SafeGatewayPlugin extends carbonGateway.GatewayPlugin {
    ...
    public override connect(resume = false): void {
      ...
      // firstHeartbeatTimeout race (Zavianx/velaclaw-dev#65009, #64011, #63387).
...
export function createDiscordGatewayPlugin(params: {
  ...
  const debugProxySettings = resolveDebugProxySettings() ?? { enabled: false };
  const options = {
