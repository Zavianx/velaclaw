...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("velaclaw/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
export function resetDiscordComponentRuntimeMocks() {
  ...
  resolveStorePathMock.mockClear().mockReturnValue("/tmp/velaclaw-sessions-test.json");
  resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
    status: "approved",
    binding: {
      bindingId: "binding-1",
      pluginId: "velaclaw-codex-app-server",
      pluginName: "Velaclaw App Server",
