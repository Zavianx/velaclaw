...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: VelaclawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): VelaclawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("velaclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("velaclaw/plugin-sdk/acp-runtime")>(
    "velaclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("velaclaw/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("velaclaw/plugin-sdk/command-auth")>(
...
vi.mock("velaclaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("velaclaw/plugin-sdk/reply-runtime")>(
...
vi.mock("velaclaw/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("velaclaw/plugin-sdk/config-runtime")>(
...
vi.mock("velaclaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("velaclaw/plugin-sdk/runtime-env")>(
    "velaclaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("velaclaw/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("velaclaw/plugin-sdk/infra-runtime")>(
