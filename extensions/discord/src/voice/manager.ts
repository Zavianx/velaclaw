...
import { resolveAgentDir } from "velaclaw/plugin-sdk/agent-runtime";
import { agentCommandFromIngress } from "velaclaw/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "velaclaw/plugin-sdk/agent-runtime";
import type { VelaclawConfig } from "velaclaw/plugin-sdk/config-runtime";
import type { DiscordAccountConfig, TtsConfig } from "velaclaw/plugin-sdk/config-runtime";
import { resolveAgentRoute } from "velaclaw/plugin-sdk/routing";
import { logVerbose, shouldLogVerbose } from "velaclaw/plugin-sdk/runtime-env";
import { createSubsystemLogger } from "velaclaw/plugin-sdk/runtime-env";
import type { RuntimeEnv } from "velaclaw/plugin-sdk/runtime-env";
import { parseTtsDirectives } from "velaclaw/plugin-sdk/speech";
import { formatErrorMessage } from "velaclaw/plugin-sdk/ssrf-runtime";
import { resolvePreferredVelaclawTmpDir } from "velaclaw/plugin-sdk/temp-path";
...
function resolveVoiceTtsConfig(params: { cfg: VelaclawConfig; override?: TtsConfig }): {
  cfg: VelaclawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredVelaclawTmpDir(), "discord-voice-"));
...
export class DiscordVoiceManager {
  ...
  private async processSegment(params: {
    ...
    const runtime = getDiscordRuntime() as unknown as {
      tts?: {
        textToSpeech?: (args: unknown) => Promise<{
          success?: boolean;
          audioPath?: string;
          error?: string;
        }>;
    ...
    })) ?? { success: false, error: "TTS runtime unavailable" };
