...
`velaclaw.invoke --each`) without adding new Velaclaw code per workflow.
...
This extension depends on Velaclaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Velaclaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.velaclaw/extensions` as a standalone plugin directory.
