...
import type { VelaclawPluginApi } from "../api.js";
...
export function createLlmTaskTool(api: VelaclawPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via velaclaw.invoke.",
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-llm-task-"),
        );
