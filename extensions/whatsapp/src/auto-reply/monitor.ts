import { resolveInboundDebounceMs } from "velaclaw/plugin-sdk/channel-inbound";
import { formatCliCommand } from "velaclaw/plugin-sdk/cli-runtime";
import { hasControlCommand } from "velaclaw/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "velaclaw/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "velaclaw/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "velaclaw/plugin-sdk/reply-history";
import { resolveAgentRoute } from "velaclaw/plugin-sdk/routing";
import { logVerbose } from "velaclaw/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "velaclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "velaclaw/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "velaclaw/plugin-sdk/runtime-env";
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("velaclaw channels login --channel web")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("velaclaw channels login --channel web")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
