...
import { VERSION } from "velaclaw/plugin-sdk/cli-runtime";
import { resolveAmbientNodeProxyAgent } from "velaclaw/plugin-sdk/extension-shared";
import { danger, success } from "velaclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "velaclaw/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "velaclaw/plugin-sdk/text-runtime";
import {
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("velaclaw channels login")}`,
              ),
            );
          }
        }
