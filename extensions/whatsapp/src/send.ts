...
import { loadConfig, type VelaclawConfig } from "velaclaw/plugin-sdk/config-runtime";
import { resolveMarkdownTableMode } from "velaclaw/plugin-sdk/config-runtime";
import { generateSecureUuid } from "velaclaw/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "velaclaw/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "velaclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "velaclaw/plugin-sdk/text-runtime";
import { redactIdentifier } from "velaclaw/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "velaclaw/plugin-sdk/text-runtime";
import {
...
function requireOutboundActiveWebListener(params: { cfg: VelaclawConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`velaclaw channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: VelaclawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const cfg = options.cfg ?? loadConfig();
