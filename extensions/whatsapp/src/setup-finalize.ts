...
import {
  ...
  type DmPolicy,
  type VelaclawConfig,
} from "velaclaw/plugin-sdk/setup";
import type { ChannelSetupWizard } from "velaclaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "velaclaw/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<VelaclawConfig["channels"]>["whatsapp"]>;
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Velaclaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Velaclaw" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("velaclaw channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
