...
Read-only diff viewer plugin for **Velaclaw** agents.
...
Set plugin-wide defaults in `~/.Zavianx/velaclaw-dev.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/velaclaw",
        },
      },
    },
  },
...
Before:
Velaclaw supports plugins.

After:
Velaclaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/velaclaw-diffs`).
