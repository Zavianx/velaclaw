...
- Use \`.velaclaw-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the Velaclaw memory-wiki plugin.
...
- \`.velaclaw-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- velaclaw:human:start -->
<!-- velaclaw:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".velaclaw-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".velaclaw-wiki", "log.jsonl"), "", createdFiles);
