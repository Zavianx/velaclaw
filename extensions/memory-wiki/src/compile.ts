...
import {
  ...
  withTrailingNewline,
} from "velaclaw/plugin-sdk/memory-host-markdown";
...
const AGENT_DIGEST_PATH = ".velaclaw-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".velaclaw-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- velaclaw:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- velaclaw:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- velaclaw:wiki:index:start -->",
      endMarker: "<!-- velaclaw:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- velaclaw:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- velaclaw:wiki:${group.dir}:index:end -->`,
