...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.memory-wiki.config.bridge.readMemoryCore is legacy; use plugins.entries.memory-wiki.config.bridge.readMemoryArtifacts. Run "velaclaw doctor --fix".',
    match: hasLegacyBridgeArtifactToggle,
  },
];

export function migrateMemoryWikiLegacyConfig(config: VelaclawConfig): {
...
export function normalizeCompatibilityConfig({ cfg }: { cfg: VelaclawConfig }): {
  config: VelaclawConfig;
  changes: string[];
} {
