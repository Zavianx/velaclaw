...
import { definePluginEntry, type VelaclawPluginApi } from "velaclaw/plugin-sdk/plugin-entry";
...
function resolvePersistentTranscriptBaseDir(api: VelaclawPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: VelaclawPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: VelaclawConfig): boolean {
...
function resolveActiveMemoryPluginConfigFromConfig(cfg: VelaclawConfig): unknown {
...
async function runRecallSubagent(params: {
  ...
  const tempDir = params.config.persistTranscripts
    ? undefined
    : await fs.mkdtemp(path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-active-memory-"));
...
export default definePluginEntry({
  ...
  register(api: VelaclawPluginApi) {
