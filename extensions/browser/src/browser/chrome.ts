...
export function resolveVelaclawUserDataDir(profileName = DEFAULT_VELACLAW_BROWSER_PROFILE_NAME) {
...
export function buildVelaclawChromeLaunchArgs(params: {
...
export async function launchVelaclawChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveVelaclawUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_VELACLAW_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildVelaclawChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateVelaclawProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 velaclaw browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`velaclaw browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`velaclaw browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 velaclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopVelaclawChrome(
