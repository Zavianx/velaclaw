...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".velaclaw-profile-decorated");
}
 ...
 */
export function decorateVelaclawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_VELACLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_VELACLAW_BROWSER_COLOR).toUpperCase();
