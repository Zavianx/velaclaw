import type { VelaclawPluginSecurityAuditContext } from "velaclaw/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "velaclaw/plugin-sdk/secret-input";
import { formatCliCommand } from "velaclaw/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "velaclaw/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: VelaclawPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("velaclaw security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.VELACLAW_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.VELACLAW_GATEWAY_PASSWORD) ||
