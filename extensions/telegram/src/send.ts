...
import { isDiagnosticFlagEnabled } from "velaclaw/plugin-sdk/diagnostic-runtime";
import { formatUncaughtError } from "velaclaw/plugin-sdk/error-runtime";
import { recordChannelActivity } from "velaclaw/plugin-sdk/infra-runtime";
import { createTelegramRetryRunner, type RetryConfig } from "velaclaw/plugin-sdk/retry-runtime";
...
import { normalizeOptionalString, redactSensitiveText } from "velaclaw/plugin-sdk/text-runtime";
...
export async function sendMessageTelegram(
  ...
  async function shouldSendTelegramImageAsPhoto(buffer: Buffer): Promise<boolean> {
    try {
      ...
      if (!isValidPhoto) {
        sendLogger.warn(
          `Photo dimensions (${String(width)}x${String(height)}) are not valid for Telegram photos. Sending as document instead.`,
        );
        return false;
      }
      return true;
...
export async function sendPollTelegram(
  ...
  const pollParams = {
    allows_multiple_answers: (normalizedPoll.maxSelections ?? 1) > 1,
    is_anonymous: opts.isAnonymous ?? true,
    ...(durationSeconds != null ? { open_period: durationSeconds } : {}),
    ...(Object.keys(threadParams).length > 0 ? threadParams : {}),
    ...(opts.silent === true ? { disable_notification: true } : {}),
  } as TelegramSendPollParams;
