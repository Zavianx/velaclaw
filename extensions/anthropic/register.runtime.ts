import { formatCliCommand, parseDurationMs } from "velaclaw/plugin-sdk/cli-runtime";
import type {
  VelaclawPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type VelaclawConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "velaclaw/plugin-sdk/provider-auth";
import { cloneFirstTemplateModel } from "velaclaw/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "velaclaw/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  "Anthropic setup-token auth is supported in Velaclaw.",
  "Velaclaw prefers Claude CLI reuse when it is available on the host.",
  "Anthropic staff told us this Velaclaw path is allowed again.",
  `If you want a direct API billing path instead, use ${formatCliCommand("velaclaw models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("velaclaw models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("velaclaw doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: VelaclawPluginApi): void {
