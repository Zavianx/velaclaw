...
> 当前 beta 说明：npm 上的 `velaclaw` 包目前是 public placeholder。当前可测试版本请安装 `velaclaw-dev`。
...
所有选择都写入 `~/.Zavianx/velaclaw-dev.json`。可以随时重跑 —— 向导支持保留现有值、只改某部分、或者整体 reset。之后想调整凭证、channels、gateway 或 agent 默认值，随时跑 `velaclaw configure`。
