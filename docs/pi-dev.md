...
This guide summarizes a sane workflow for working on the pi integration in Velaclaw.
...
```bash
VELACLAW_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm velaclaw agent --message "Hello" --thinking low`
...
State lives under the Velaclaw state directory. Default is `~/.velaclaw`. If `VELACLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `velaclaw.json` for config
