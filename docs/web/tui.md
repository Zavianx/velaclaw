...
```bash
velaclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `velaclaw tui --deliver`
...
- Local shell commands receive `VELACLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `VELACLAW_THEME=light` before launching `velaclaw tui`.
- To force the original dark palette instead, set `VELACLAW_THEME=dark`.
...
- Check the Gateway logs: `velaclaw logs --follow`.
- Confirm the agent can run: `velaclaw status` and `velaclaw models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `velaclaw agents list` and your routing config.
