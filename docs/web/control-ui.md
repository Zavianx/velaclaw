...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/velaclaw`)
...
If the page fails to load, start the Gateway first: `velaclaw gateway`.
...
created. Re-run `velaclaw devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `velaclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.Zavianx/velaclaw-dev.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop velaclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Velaclaw
...
```bash
velaclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Velaclaw **blocks** Control UI connections without device identity.
...
```bash
VELACLAW_CONTROL_UI_BASE_PATH=/velaclaw/ pnpm ui:build
```
