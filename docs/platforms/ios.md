...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `velaclaw.internal.`), **or**
...
Run `velaclaw devices list` again before approval.

4. Verify connection:

```bash
velaclaw nodes status
velaclaw gateway call node.list --params "{}"
```
...
- `VELACLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Velaclaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Velaclaw builds.
...
```bash
export VELACLAW_APNS_TEAM_ID="TEAMID"
export VELACLAW_APNS_KEY_ID="KEYID"
export VELACLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```

These are gateway-host runtime env vars. The public repository currently ships
the Gateway/CLI runtime, not an iOS native release pipeline.

Recommended gateway-host storage:

```bash
mkdir -p ~/.velaclaw/credentials/apns
chmod 700 ~/.velaclaw/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.velaclaw/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.velaclaw/credentials/apns/AuthKey_KEYID.p8
export VELACLAW_APNS_PRIVATE_KEY_PATH="$HOME/.velaclaw/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_velaclaw-gw._tcp` on `local.` and, when configured, the same
...
`velaclaw.internal.`) and Tailscale split DNS.
...
```bash
velaclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__velaclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__velaclaw__/canvas/` and `/__velaclaw__/a2ui/`.
...
```bash
velaclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__velaclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
velaclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `velaclaw devices list` and approve manually.
