---
summary: "Velaclaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Velaclaw on a Raspberry Pi
  - Running Velaclaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Velaclaw on Raspberry Pi

## Goal

Run a persistent, always-on Velaclaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
## 9) Access the Velaclaw Dashboard
...
```bash
ssh user@gateway-host 'velaclaw dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/velaclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/velaclaw-compile-cache
mkdir -p /var/tmp/velaclaw-compile-cache
export VELACLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `VELACLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Velaclaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit velaclaw-gateway.service
```

```ini
[Service]
Environment=VELACLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/velaclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Velaclaw state/cache on SSD-backed storage to avoid SD-card
...
Most Velaclaw features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled velaclaw-gateway.service

# Enable if not
systemctl --user enable velaclaw-gateway.service

# Start on boot
systemctl --user start velaclaw-gateway.service
```
...
```bash
# Check logs
journalctl --user -u velaclaw-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/velaclaw  # if using hackable install
npm run build
systemctl --user restart velaclaw-gateway.service
```
