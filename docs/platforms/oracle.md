---
summary: "Velaclaw on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up Velaclaw on Oracle Cloud
  - Looking for low-cost VPS hosting for Velaclaw
  - Want 24/7 Velaclaw on a small server
title: "Oracle Cloud (Platform)"
---

# Velaclaw on Oracle Cloud (OCI)

## Goal

Run a persistent Velaclaw Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for Velaclaw (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname velaclaw
...
sudo tailscale up --ssh --hostname=velaclaw
```

This enables Tailscale SSH, so you can connect via `ssh velaclaw` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@velaclaw` (or use the Tailscale IP).
...
velaclaw config set gateway.bind loopback

# Require auth for the Gateway + Control UI
velaclaw config set gateway.auth.mode token
velaclaw doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
velaclaw config set gateway.tailscale.mode serve
velaclaw config set gateway.trustedProxies '["127.0.0.1"]'

systemctl --user restart velaclaw-gateway.service
```
...
```
https://velaclaw.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `velaclaw security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.velaclaw`
- **Security audit:** `velaclaw security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@velaclaw
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=velaclaw --reset
```
...
journalctl --user -u velaclaw-gateway.service -n 50
```
...
- `~/.velaclaw/` — `velaclaw.json`, per-agent `auth-profiles.json`, channel/provider state, and session data
- `~/.velaclaw/workspace/` — workspace (SOUL.md, memory, artifacts)
