...
- Same path: run the app from a fixed location (for Velaclaw, `dist/Velaclaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.velaclaw.mac
sudo tccutil reset ScreenCapture ai.velaclaw.mac
...
Workaround: move files into the Velaclaw workspace (`~/.velaclaw/workspace`) if you want to avoid per-folder grants.
