...
service via the external `velaclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.velaclaw.gateway`
  (or `ai.velaclaw.<profile>` when using `--profile`/`VELACLAW_PROFILE`; legacy `com.velaclaw.*` is supported).
