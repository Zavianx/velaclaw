...
- You can run the CLI (`velaclaw`) on the gateway machine (or via SSH).
...
```bash
velaclaw gateway discover --json
```
...
1. Set up a DNS-SD zone (example `velaclaw.internal.`) on the gateway host and publish `_velaclaw-gw._tcp` records.
...
1. Create `~/.velaclaw/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
velaclaw nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__velaclaw__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__velaclaw__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__velaclaw__/a2ui/`.
...
Android supports launching Velaclaw from the system assistant trigger (Google
Assistant). When configured, holding the home button or saying "Hey Google, ask
Velaclaw..." opens the app and hands the prompt into the chat composer.
...
and whether the user has set Velaclaw as the default assistant app.
</Note>
