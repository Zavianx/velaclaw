---
summary: "Velaclaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Velaclaw on DigitalOcean
  - Looking for cheap VPS hosting for Velaclaw
title: "DigitalOcean (Platform)"
---

# Velaclaw on DigitalOcean

## Goal

Run a persistent Velaclaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
velaclaw config set gateway.bind tailnet
velaclaw gateway restart
```
...
```bash
velaclaw channels login whatsapp
# Scan QR code
```
...
- `~/.velaclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u velaclaw-gateway.service --no-pager -n 50
```
