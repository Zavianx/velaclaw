---
summary: "Velaclaw macOS companion app (menu bar + gateway broker)"
read_when:
...
# Velaclaw macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for Velaclaw. It owns permissions,
...
- Installs the global CLI (`velaclaw`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `velaclaw gateway install`.
...
The app manages a per‑user LaunchAgent labeled `ai.velaclaw.gateway`
(or `ai.velaclaw.<profile>` when using `--profile`/`VELACLAW_PROFILE`; legacy `com.velaclaw.*` still unloads).

```bash
launchctl kickstart -k gui/$UID/ai.velaclaw.gateway
launchctl bootout gui/$UID/ai.velaclaw.gateway
```

Replace the label with `ai.velaclaw.<profile>` when running a named profile.
...
The app registers the `velaclaw://` URL scheme for local actions.
...
```bash
open 'velaclaw://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **Velaclaw.app**.
...
Avoid putting your Velaclaw state dir in iCloud or other cloud-synced folders.
...
If `velaclaw doctor` detects state under:
...
Tip: compare against `velaclaw gateway discover --json` to see whether the
