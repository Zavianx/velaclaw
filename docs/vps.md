---
summary: "Run Velaclaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Velaclaw
title: "Linux Server"
...
Run the Velaclaw Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/velaclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `VELACLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  - `VELACLAW_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/velaclaw-compile-cache`
...
For the standard `velaclaw onboard --install-daemon` path, edit the user unit:
...
`velaclaw-gateway.service` via `sudo systemctl edit velaclaw-gateway.service`.
