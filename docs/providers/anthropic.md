---
summary: "Use Anthropic Claude via API keys or Claude CLI in Velaclaw"
read_when:
  - You want to use Anthropic models in Velaclaw
title: "Anthropic"
---

# Anthropic (Claude)

Anthropic builds the **Claude** model family. Velaclaw supports two auth routes:
...
Velaclaw treats Claude CLI reuse and `claude -p` usage as sanctioned unless
...
Claude 4.6 models default to `adaptive` thinking in Velaclaw when no explicit thinking level is set.
...
Velaclaw supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    Velaclaw's shared `/fast` toggle supports direct Anthropic traffic (API-key and OAuth to `api.anthropic.com`).
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Velaclaw
    ...
    When an image or PDF is attached to a conversation, Velaclaw automatically
  ...
  <Accordion title="1M context window (beta)">
    ...
    Velaclaw maps this to `anthropic-beta: context-1m-2025-08-07` on requests.
