---
summary: "Use OpenAI via API keys or Codex subscription in Velaclaw"
read_when:
  - You want to use OpenAI models in Velaclaw
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        velaclaw onboard --auth-choice openai-api-key
        ```
    ...
    **Best for:** using your ChatGPT/Codex subscription instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        velaclaw onboard --auth-choice openai-codex
        ```
    ...
    Velaclaw treats model metadata and the runtime context cap as separate values.
...
| `"off"`                | Use base Velaclaw prompt only      |
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    Velaclaw uses WebSocket-first with SSE fallback (`"auto"`) for both `openai/*` and `openai-codex/*`.

    In `"auto"` mode, Velaclaw:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    Velaclaw treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
