---
summary: "GLM model family overview + how to use it in Velaclaw"
read_when:
  - You want GLM models in Velaclaw
...
GLM is a **model family** (not a company) available through the Z.AI platform. In Velaclaw, GLM
...
<Steps>
  <Step title="Choose an auth route and run onboarding">
    ...
    velaclaw onboard --auth-choice zai-coding-global
    ```
...
<Tip>
`zai-api-key` lets Velaclaw detect the matching Z.AI endpoint from the key and
...
Velaclaw currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, Velaclaw inspects the key format
