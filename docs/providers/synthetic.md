---
summary: "Use Synthetic's Anthropic-compatible API in Velaclaw"
read_when:
...
Velaclaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Warning>
Velaclaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`velaclaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Velaclaw appends `/v1` automatically.
