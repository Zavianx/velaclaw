---
summary: "Use Xiaomi MiMo models with Velaclaw"
read_when:
  - You want Xiaomi MiMo models in Velaclaw
...
Xiaomi MiMo is the API platform for **MiMo** models. Velaclaw uses the Xiaomi
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    velaclaw onboard --auth-choice xiaomi-api-key
    ```

    Or pass the key directly:

    ```bash
    velaclaw onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.velaclaw/.env` or via `env.shellEnv`).
