...
read_when:
  - You want to use Groq with Velaclaw
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Velaclaw connects
...
Groq's model catalog changes frequently. Run `velaclaw models list | grep groq`
...
<Tip>
Use `velaclaw models list --provider groq` for the most up-to-date list of
...
media-understanding provider, Velaclaw uses Groq's `whisper-large-v3-turbo`
