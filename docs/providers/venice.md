---
summary: "Use Venice AI privacy-focused models in Velaclaw"
read_when:
  - You want privacy-focused inference in Velaclaw
  ...
  <Step title="Configure Velaclaw">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        velaclaw onboard --auth-choice venice-api-key
        ```
...
After setup, Velaclaw shows all available Venice models. Pick based on your needs:
...
```bash
velaclaw models set venice/kimi-k2-5
velaclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
velaclaw models list | grep venice
```

You can also run `velaclaw configure`, select **Model/auth**, and choose **Venice AI**.
...
Velaclaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
velaclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
velaclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
velaclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
velaclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
velaclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
