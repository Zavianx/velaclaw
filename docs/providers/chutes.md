...
read_when:
  - You want to use Chutes with Velaclaw
...
OpenAI-compatible API. Velaclaw supports both browser OAuth and direct API-key
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run the OAuth onboarding flow">
        ```bash
        velaclaw onboard --auth-choice chutes
        ```
...
When Chutes auth is available, Velaclaw queries the Chutes catalog with that
credential and uses the discovered models. If discovery fails, Velaclaw falls
