---
summary: "Sign in to GitHub Copilot from Velaclaw using the device flow"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `velaclaw models auth login-github-copilot` flow
...
models for your GitHub account and plan. Velaclaw can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in Velaclaw

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Velaclaw runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        velaclaw models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        velaclaw models set github-copilot/gpt-4o
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Velaclaw talks to
...
```bash
# Skip confirmation
velaclaw models auth login-github-copilot --yes

# Login and set the default model in one step
velaclaw models auth login --provider github-copilot --method device --set-default
```
...
<AccordionGroup>
  ...
  <Accordion title="Environment variable resolution order">
    Velaclaw resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Velaclaw uses the highest-priority one.
    The device-login flow (`velaclaw models auth login-github-copilot`) stores
...
have logged in, Velaclaw can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, Velaclaw discovers available
...
1. Velaclaw resolves your GitHub token (from env vars or auth profile).
...
available, Velaclaw skips Copilot and tries the next provider.
