---
summary: "Run Velaclaw through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run Velaclaw against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact Velaclaw compat flags for inferrs
title: "inferrs"
---
...
OpenAI-compatible `/v1` API. Velaclaw works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated Velaclaw provider plugin.
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    Velaclaw will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full Velaclaw agent-runtime
    turns.
    ...
    That disables Velaclaw's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal Velaclaw agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than Velaclaw's transport layer.
