...
read_when:
  - You want to use Google Gemini models with Velaclaw
...
<Tabs>
    ...
    **Best for:** standard Gemini API access through Google AI Studio.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        velaclaw onboard --auth-choice gemini-api-key
        ```
  ...
  <Tab title="Gemini CLI (OAuth)">
    ...
    **Environment variables:**
    ...
    - `VELACLAW_GEMINI_OAUTH_CLIENT_ID`
    - `VELACLAW_GEMINI_OAUTH_CLIENT_SECRET`
...
<Tip>
Gemma 4 models (for example `gemma-4-26b-a4b-it`) support thinking mode. Velaclaw
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    For direct Gemini API runs (`api: "google-generative-ai"`), Velaclaw
    ...
    - Gemini cache-hit usage is normalized into Velaclaw `cacheRead` from
  ...
  <Accordion title="Gemini CLI JSON usage notes">
    When using the `google-gemini-cli` OAuth provider, Velaclaw normalizes
