---
summary: "Use Amazon Bedrock Mantle (OpenAI-compatible) models with Velaclaw"
read_when:
  - You want to use Bedrock Mantle hosted OSS models with Velaclaw
...
Velaclaw includes a bundled **Amazon Bedrock Mantle** provider that connects to
...
When `AWS_BEARER_TOKEN_BEDROCK` is set, Velaclaw uses it directly. Otherwise,
Velaclaw attempts to generate a Mantle bearer token from the AWS default
