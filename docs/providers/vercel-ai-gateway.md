...
read_when:
  - You want to use Vercel AI Gateway with Velaclaw
...
<Tip>
Velaclaw auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    velaclaw onboard --auth-choice ai-gateway-api-key
    ```
...
Velaclaw accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Velaclaw resolves the canonical form automatically.
</Tip>
