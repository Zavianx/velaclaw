---
summary: "Run Velaclaw with vLLM (OpenAI-compatible local server)"
read_when:
  - You want to run Velaclaw against a local vLLM server
...
vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. Velaclaw connects to vLLM using the `openai-completions` API.

Velaclaw can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server does not enforce auth) and you do not define an explicit `models.providers.vllm` entry.
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, Velaclaw queries:
...
<AccordionGroup>
  <Accordion title="Proxy-style behavior">
    ...
    | Hidden Velaclaw attribution headers | Not injected on custom base URLs |
