---
summary: "Use Vydra image, video, and speech in Velaclaw"
read_when:
  - You want Vydra media generation in Velaclaw
...
Velaclaw uses the same `VYDRA_API_KEY` for all three capabilities.
...
<Steps>
  <Step title="Run interactive onboarding">
    ```bash
    velaclaw onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  <Accordion title="Image generation">
    ...
    Current bundled support is text-to-image only. Vydra's hosted edit routes expect remote image URLs, and Velaclaw does not add a Vydra-specific upload bridge in the bundled plugin yet.
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    VELACLAW_LIVE_TEST=1 \
    VELACLAW_LIVE_VYDRA_VIDEO=1 \
    ...
    ```bash
    export VELACLAW_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
