---
summary: "Use MiniMax models in Velaclaw"
read_when:
  - You want MiniMax models in Velaclaw
...
Velaclaw's MiniMax provider defaults to **MiniMax M2.7**.
    ...
    <Tabs>
      <Tab title="International">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            velaclaw onboard --auth-choice minimax-global-oauth
            ```
      ...
      <Tab title="China">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            velaclaw onboard --auth-choice minimax-cn-oauth
            ```
    ...
    <Tabs>
      <Tab title="International">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            velaclaw onboard --auth-choice minimax-global-api
            ```
...
## Configure via `velaclaw configure`
...
entries, Velaclaw materializes `MiniMax-M2.7` and
...
<AccordionGroup>
  ...
  <Accordion title="Thinking defaults">
    On `api: "anthropic-messages"`, Velaclaw injects `thinking: { type: "disabled" }` unless thinking is already explicitly set in params/config.
...
- Use `velaclaw models list` to confirm the current provider id, then switch with `velaclaw models set minimax/MiniMax-M2.7` or `velaclaw models set minimax-portal/MiniMax-M2.7`
...
<AccordionGroup>
  <Accordion title='"Unknown model: minimax/MiniMax-M2.7"'>
    ...
    - Running `velaclaw configure` and selecting a **MiniMax** auth option, or
