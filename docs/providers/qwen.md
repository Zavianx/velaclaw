---
summary: "Use Qwen Cloud via Velaclaw's bundled qwen provider"
read_when:
  - You want to use Qwen with Velaclaw
...
Velaclaw now treats Qwen as a first-class bundled provider with canonical id
    ...
    **Best for:** subscription-based access through the Qwen Coding Plan.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        velaclaw onboard --auth-choice qwen-api-key
        ```
...
Velaclaw currently ships this bundled Qwen catalog. The configured catalog is
...
<AccordionGroup>
  ...
  <Accordion title="Video generation details">
    For video generation, Velaclaw maps the configured Qwen region to the matching
