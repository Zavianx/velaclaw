---
summary: "Use Z.AI (GLM models) with Velaclaw"
read_when:
  - You want Z.AI / GLM models in Velaclaw
...
for authentication. Create your API key in the Z.AI console. Velaclaw uses the `zai` provider
with a Z.AI API key.
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        velaclaw onboard --auth-choice zai-coding-cn

        # General API
        velaclaw onboard --auth-choice zai-global
...
Velaclaw currently seeds the bundled `zai` provider with:
