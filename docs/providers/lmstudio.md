---
summary: "Run Velaclaw with LM Studio"
read_when:
  - You want to run Velaclaw with open source models via LM Studio
...
3. Velaclaw requires an LM Studio token value. Set `LM_API_TOKEN`:
...
```bash
velaclaw models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). Velaclaw
