---
title: "Kilocode"
summary: "Use Kilo Gateway's unified API to access many models in Velaclaw"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Velaclaw
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    velaclaw onboard --auth-choice kilocode-api-key
    ```
...
<Note>
Velaclaw treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Velaclaw.
</Note>

## Available models

Velaclaw dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Velaclaw queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Velaclaw keeps
