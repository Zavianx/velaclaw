---
summary: "CLI reference for `velaclaw security` (audit and fix common security footguns)"
read_when:
...
velaclaw security audit --deep --password <password>
velaclaw security audit --deep --token <token>
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that Velaclaw is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `velaclaw.browserConfigEpoch`) and recommends `velaclaw sandbox recreate --browser --all`.
...
```bash
velaclaw security audit --json | jq '.summary'
velaclaw security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
velaclaw security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files
  ...
  `*.jsonl`)
- also tightens config include files referenced from `velaclaw.json`
