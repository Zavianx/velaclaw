---
summary: "CLI reference for `velaclaw memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
velaclaw memory status
velaclaw memory status --deep
velaclaw memory status --fix
velaclaw memory index --force
velaclaw memory search "meeting notes"
velaclaw memory search --query "deployment" --max-results 20
velaclaw memory promote --limit 10 --min-score 0.75
velaclaw memory promote --apply
velaclaw memory promote --json --min-recall-count 0 --min-unique-queries 0
velaclaw memory promote-explain "router vlan"
velaclaw memory promote-explain "router vlan" --json
velaclaw memory rem-harness
velaclaw memory rem-harness --json
velaclaw memory status --json
velaclaw memory status --deep --index
velaclaw memory status --deep --index --verbose
velaclaw memory status --agent main
velaclaw memory index --agent main --verbose
```
...
```bash
velaclaw memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `velaclaw cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
velaclaw memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
velaclaw memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
