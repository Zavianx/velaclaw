---
summary: "Expose Velaclaw channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Velaclaw-backed channels
  - Running `velaclaw mcp serve`
  - Managing Velaclaw-saved MCP server definitions
title: "mcp"
---

# mcp

`velaclaw mcp` has two jobs:

- run Velaclaw as an MCP server with `velaclaw mcp serve`
- manage Velaclaw-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Velaclaw acting as an MCP server
- `list` / `show` / `set` / `unset` is Velaclaw acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`velaclaw acp`](/cli/acp) when Velaclaw should host a coding harness
session itself and route that runtime through ACP.

## Velaclaw as an MCP server

This is the `velaclaw mcp serve` path.

## When to use `serve`

Use `velaclaw mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Velaclaw-backed channel conversations
- you already have a local or remote Velaclaw Gateway with routed sessions
- you want one MCP server that works across Velaclaw's channel backends instead
  of running separate per-channel bridges

Use [`velaclaw acp`](/cli/acp) instead when Velaclaw should host the coding
runtime itself and keep the agent session inside Velaclaw.

## How it works

`velaclaw mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Velaclaw Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `velaclaw mcp serve`
...
conversations. A conversation appears when Velaclaw already has session state
...
# Remote Gateway
velaclaw mcp serve --url wss://gateway-host:18789 --token-file ~/.velaclaw/gateway.token

# Remote Gateway with password auth
velaclaw mcp serve --url wss://gateway-host:18789 --password-file ~/.velaclaw/gateway.password

# Enable verbose bridge logs
velaclaw mcp serve --verbose

# Disable Claude-specific push notifications
velaclaw mcp serve --claude-channel-mode off
```
...
Velaclaw equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Velaclaw channel configuration
...
Velaclaw ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `velaclaw mcp serve`
...
## Velaclaw as an MCP client registry

This is the `velaclaw mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Velaclaw over MCP. They manage Velaclaw-owned MCP
server definitions under `mcp.servers` in Velaclaw config.

Those saved definitions are for runtimes that Velaclaw launches or configures
later, such as embedded Pi and other runtime adapters. Velaclaw stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Velaclaw config
...
Velaclaw also stores a lightweight MCP server registry in config for surfaces
that want Velaclaw-managed MCP definitions.

Commands:

- `velaclaw mcp list`
- `velaclaw mcp show [name]`
- `velaclaw mcp set <name> <json>`
- `velaclaw mcp unset <name>`
...
```bash
velaclaw mcp list
velaclaw mcp show context7 --json
velaclaw mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
velaclaw mcp set docs '{"url":"https://mcp.example.com"}'
velaclaw mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Velaclaw uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
