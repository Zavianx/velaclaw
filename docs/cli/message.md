---
summary: "CLI reference for `velaclaw message` (send + channel actions)"
read_when:
...
```
velaclaw message <subcommand> [flags]
```
...
- On cache miss, Velaclaw will attempt a live directory lookup when the provider supports it.
...
- `velaclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Velaclaw does not force a `default` account SecretRef scope
...
```
velaclaw message send --channel discord \
...
```bash
velaclaw message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
velaclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
velaclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
velaclaw message poll --channel msteams \
...
```
velaclaw message react --channel slack \
...
```
velaclaw message react --channel signal \
...
```
velaclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
velaclaw message send --channel telegram --target @mychat \
