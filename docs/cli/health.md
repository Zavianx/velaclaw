---
summary: "CLI reference for `velaclaw health` (gateway health snapshot via RPC)"
read_when:
...
velaclaw health --timeout 2500
velaclaw health --verbose
velaclaw health --debug
```

Notes:

- Default `velaclaw health` asks the running gateway for its health snapshot. When the
