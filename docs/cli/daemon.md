---
summary: "CLI reference for `velaclaw daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `velaclaw daemon ...` in scripts
...
`velaclaw daemon ...` maps to the same service control surface as `velaclaw gateway ...` service commands.

## Usage

```bash
velaclaw daemon status
velaclaw daemon install
velaclaw daemon start
velaclaw daemon stop
velaclaw daemon restart
velaclaw daemon uninstall
```
...
Use [`velaclaw gateway`](/cli/gateway) for current docs and examples.
