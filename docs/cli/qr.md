---
summary: "CLI reference for `velaclaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
velaclaw qr
velaclaw qr --setup-code-only
velaclaw qr --json
velaclaw qr --remote
velaclaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, Velaclaw requires either `gateway.remote.url` or
