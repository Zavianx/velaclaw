---
summary: "CLI reference for `velaclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `velaclaw config` without a subcommand opens the same wizard. Use
`velaclaw config get|set|unset` for non-interactive edits.

For web search, `velaclaw configure --section web` lets you choose a provider
...
velaclaw configure --section model --section channels
velaclaw configure --section gateway --section daemon
```
