---
summary: "CLI reference for `velaclaw approvals` and `velaclaw exec-policy`"
read_when:
...
## `velaclaw exec-policy`

`velaclaw exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.velaclaw/exec-approvals.json`

Examples:

```bash
velaclaw exec-policy show
velaclaw exec-policy show --json

velaclaw exec-policy preset yolo
velaclaw exec-policy preset cautious --json

velaclaw exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `velaclaw exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `velaclaw approvals set --gateway`
or `velaclaw approvals set --node <id|name|ip>`.

## Common commands

```bash
velaclaw approvals get
velaclaw approvals get --node <id|name|ip>
velaclaw approvals get --gateway
```

`velaclaw approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
velaclaw approvals set --file ./exec-approvals.json
velaclaw approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
velaclaw approvals set --node <id|name|ip> --file ./exec-approvals.json
velaclaw approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Velaclaw policy aligned, also set:
...
```bash
velaclaw approvals allowlist add "~/Projects/**/bin/rg"
velaclaw approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
velaclaw approvals allowlist add --agent "*" "/usr/bin/uname"

velaclaw approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `velaclaw nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.velaclaw/exec-approvals.json`.
