---
summary: "CLI reference for `velaclaw cron` (schedule and run background jobs)"
read_when:
...
Tip: run `velaclaw cron --help` for the full command surface.
...
Note: `velaclaw cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `velaclaw cron runs --id <job-id>` to follow the eventual outcome.

Note: `velaclaw cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.velaclaw/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`velaclaw doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
velaclaw cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
velaclaw cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
velaclaw cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
velaclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
velaclaw cron run <job-id>
velaclaw cron run <job-id> --due
velaclaw cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
velaclaw cron edit <job-id> --agent ops
velaclaw cron edit <job-id> --clear-agent
velaclaw cron edit <job-id> --session current
velaclaw cron edit <job-id> --session "session:daily-brief"
```
...
```bash
velaclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
velaclaw cron edit <job-id> --best-effort-deliver
velaclaw cron edit <job-id> --no-best-effort-deliver
velaclaw cron edit <job-id> --no-deliver
```
