---
summary: "CLI reference for `velaclaw agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
velaclaw agent --to +15555550123 --message "status update" --deliver
...
velaclaw agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Velaclaw persists markers from the active source config snapshot, not from resolved runtime secret values.
