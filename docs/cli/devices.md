---
summary: "CLI reference for `velaclaw devices` (device pairing + token rotation/revocation)"
read_when:
...
```
velaclaw devices remove <deviceId>
velaclaw devices remove <deviceId> --json
```

### `velaclaw devices clear --yes [--pending]`

Clear paired devices in bulk.

```
velaclaw devices clear --yes
velaclaw devices clear --yes --pending
velaclaw devices clear --yes --pending --json
```

### `velaclaw devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Velaclaw only prints the selected pending
...
key), Velaclaw supersedes the previous pending entry and issues a new
`requestId`. Run `velaclaw devices list` right before approval to use the
current ID.

```
velaclaw devices approve
velaclaw devices approve <requestId>
velaclaw devices approve --latest
```
...
### `velaclaw devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
velaclaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `velaclaw devices revoke --device <id> --role <role>`
...
```
velaclaw devices revoke --device <deviceId> --role node
```
...
```bash
velaclaw devices rotate --device <deviceId> --role operator
```
