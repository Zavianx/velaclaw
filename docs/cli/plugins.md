---
summary: "CLI reference for `velaclaw plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
velaclaw plugins list --enabled
velaclaw plugins list --verbose
velaclaw plugins list --json
velaclaw plugins install <path-or-spec>
velaclaw plugins inspect <id>
velaclaw plugins inspect <id> --json
velaclaw plugins inspect --all
velaclaw plugins info <id>
velaclaw plugins enable <id>
velaclaw plugins disable <id>
velaclaw plugins uninstall <id>
velaclaw plugins doctor
velaclaw plugins update <id>
velaclaw plugins update --all
velaclaw plugins marketplace list <marketplace>
velaclaw plugins marketplace list <marketplace> --json
```

Bundled plugins ship with Velaclaw. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native Velaclaw plugins must ship `velaclaw.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: velaclaw` or `Format: bundle`. Verbose list/info
...
```bash
velaclaw plugins install <package>                      # ClawHub first, then npm
velaclaw plugins install clawhub:<package>              # ClawHub only
velaclaw plugins install <package> --force              # overwrite existing install
velaclaw plugins install <package> --pin                # pin version
velaclaw plugins install <package> --dangerously-force-unsafe-install
velaclaw plugins install <path>                         # local path
velaclaw plugins install <plugin>@<marketplace>         # marketplace
velaclaw plugins install <plugin> --marketplace <name>  # marketplace (explicit)
velaclaw plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
```
...
run `velaclaw doctor --fix` first. The only documented exception is a narrow
...
override, while `velaclaw skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`velaclaw.hooks` in `package.json`. Use `velaclaw hooks` for filtered hook
...
those to a prerelease, Velaclaw stops and asks you to opt in explicitly with a
prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as
`@1.2.3-beta.4`.

If a bare install spec matches a bundled plugin id (for example `diffs`), Velaclaw
...
```bash
velaclaw plugins install clawhub:velaclaw-codex-app-server
velaclaw plugins install clawhub:velaclaw-codex-app-server@1.2.3
```

Velaclaw now also prefers ClawHub for bare npm-safe plugin specs. It only falls
back to npm if ClawHub does not have that package or version:

```bash
velaclaw plugins install velaclaw-codex-app-server
```

Velaclaw downloads the package archive from ClawHub, checks the advertised
...
```bash
velaclaw plugins marketplace list <marketplace-name>
velaclaw plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
velaclaw plugins install <plugin-name> --marketplace <marketplace-name>
velaclaw plugins install <plugin-name> --marketplace <owner/repo>
velaclaw plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
velaclaw plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Velaclaw accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, Velaclaw auto-detects:

- native Velaclaw plugins (`velaclaw.plugin.json`)
...
```bash
velaclaw plugins install -l ./my-plugin
```
...
```bash
velaclaw plugins uninstall <id>
velaclaw plugins uninstall <id> --dry-run
velaclaw plugins uninstall <id> --keep-files
```
...
```bash
velaclaw plugins update <id-or-npm-spec>
velaclaw plugins update --all
velaclaw plugins update <id-or-npm-spec> --dry-run
velaclaw plugins update @velaclaw/voice-call@beta
velaclaw plugins update velaclaw-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Velaclaw reuses the recorded install spec for that
...
or exact version. Velaclaw resolves that package name back to the tracked plugin
record, updates that installed plugin, and records the new npm spec for future
id-based updates.

When a stored integrity hash exists and the fetched artifact hash changes,
Velaclaw prints a warning and asks for confirmation before proceeding. Use
...
```bash
velaclaw plugins marketplace list <source>
velaclaw plugins marketplace list <source> --json
```
