---
summary: "Velaclaw CLI reference for `velaclaw` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.velaclaw-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.velaclaw-<name>`.
- `--container <name>`: target a named container for execution.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `velaclaw update` (source installs only).
...
Velaclaw uses a lobster palette for CLI output.
...
```
velaclaw [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `velaclaw voicecall`).

## Security

- `velaclaw security audit` — audit config + local state for common security foot-guns.
- `velaclaw security audit --deep` — best-effort live Gateway probe.
- `velaclaw security audit --fix` — tighten safe defaults and state/config permissions.
...
- `velaclaw plugins list` — discover plugins (use `--json` for machine output).
- `velaclaw plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `velaclaw plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`; use `--force` to overwrite an existing install target).
- `velaclaw plugins marketplace list <marketplace>` — list marketplace entries before install.
- `velaclaw plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `velaclaw plugins doctor` — report plugin load errors.
...
- `velaclaw memory status` — show index stats; use `--deep` for vector + embedding readiness checks or `--fix` to repair stale recall/promotion artifacts.
- `velaclaw memory index` — reindex memory files.
- `velaclaw memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
- `velaclaw memory promote` — rank short-term recalls and optionally append top entries into `MEMORY.md`.
...
- `--install` writes an `Velaclaw Completion` block into your shell profile and points it at the cached script under the Velaclaw state directory.
...
- `--workspace <dir>`: agent workspace path (default `~/.velaclaw/workspace`).
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `velaclaw config` with no
...
- `config set --dry-run`: validate assignments without writing `velaclaw.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `velaclaw.json`, including propagated field `title` / `description` docs metadata across nested object, wildcard, array-item, and composition branches, plus best-effort live plugin/channel schema metadata.
...
- `velaclaw --update` rewrites to `velaclaw update`.

### `backup`

Create and verify local backup archives for Velaclaw state.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs live per-account probe/audit checks when the gateway is reachable; if not, it falls back to config-only channel summaries. Use `velaclaw health` or `velaclaw status --deep` for broader gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `velaclaw doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Velaclaw promotes account-scoped values into the channel account map before writing the new account. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```bash
velaclaw channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
velaclaw channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
velaclaw channels remove --channel discord --account work --delete
velaclaw channels status --probe
velaclaw status --deep
```

### `directory`

Look up self, peer, and group IDs for channels that expose a directory surface. See [`velaclaw directory`](/cli/directory).
...
Tip: use `velaclaw skills search`, `velaclaw skills install`, and `velaclaw skills update` for ClawHub-backed skills.
...
Generate a mobile pairing QR and setup code from the current Gateway config. See [`velaclaw qr`](/cli/qr).
...
- After scanning, approve the request with `velaclaw devices list` / `velaclaw devices approve <requestId>`.

### `clawbot`

Legacy alias namespace. Currently supports `velaclaw clawbot qr`, which maps to [`velaclaw qr`](/cli/qr).
...
- `hooks enable <name>`
- `hooks disable <name>`
- `hooks install <path-or-spec>` (deprecated alias for `velaclaw plugins install`)
- `hooks update [id]` (deprecated alias for `velaclaw plugins update`)
...
- Plugin-managed hooks cannot be enabled or disabled through `velaclaw hooks`; enable or disable the owning plugin instead.
...
- `setup` configures the Gmail watch plus the Velaclaw-facing push path.
...
- Without `--apply`, this is a planning helper that prints the recommended Velaclaw + Tailscale DNS config.
...
- `velaclaw message send --target +15555550123 --message "Hi"`
- `velaclaw message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Running `velaclaw agents` with no subcommand is equivalent to `velaclaw agents list`.
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Velaclaw may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Manage saved MCP server definitions and expose Velaclaw channels over MCP stdio.

#### `mcp serve`

Expose routed Velaclaw channel conversations over MCP stdio.
...
- `--gateway`
- `--json`
- node RPC options from `velaclaw nodes`
...
Velaclaw can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `velaclaw status --usage` (prints full provider breakdown)
...
- MiniMax note: raw `usage_percent` / `usagePercent` means remaining quota, so Velaclaw inverts it before display; count-based fields still win when present. `model_remains` responses prefer the chat-model entry, derive the window label from timestamps when needed, and include the model name in the plan label.
- Usage auth comes from provider-specific hooks when available; otherwise Velaclaw falls back to matching OAuth/API-key credentials from auth profiles, env, or config. If none resolve, usage is hidden.
...
Legacy docs shortcut. Flow commands live under `velaclaw tasks flow`:
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Velaclaw services are treated as first-class and aren't flagged as "extra".
...
velaclaw logs --limit 200
velaclaw logs --plain
velaclaw logs --json
velaclaw logs --no-color
```
...
Anthropic note: Anthropic staff told us Velaclaw-style Claude CLI usage is
allowed again, so Velaclaw treats Claude CLI reuse and `claude -p` usage as
...
Anthropic setup-token remains available as a supported token-auth path, but Velaclaw now prefers Claude CLI reuse and `claude -p` when available.

### `models` (root)

`velaclaw models` is an alias for `models status`.
...
- Anthropic `setup-token` / `paste-token` remain available as a supported Velaclaw token path, but Velaclaw now prefers Claude CLI reuse and `claude -p` when available.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `VELACLAW_GATEWAY_TOKEN` / `VELACLAW_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `VELACLAW_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`velaclaw browser`](/cli/browser) and the [Browser tool](/tools/browser).
...
Plugin-provided voice-call utilities. Only appears when the voice-call plugin is installed and enabled. See [`velaclaw voicecall`](/cli/voicecall).
...
Search the live Velaclaw docs index.
