---
summary: "CLI reference for `velaclaw wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `velaclaw wiki`
title: "wiki"
---
...
Use `velaclaw wiki` when you want a compiled knowledge vault with:
...
- `.velaclaw-wiki/cache/agent-digest.json`
- `.velaclaw-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `velaclaw memory search` when the
...
```bash
velaclaw wiki get entity.alpha
velaclaw wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`velaclaw wiki` behavior is shaped by:
