---
summary: "CLI reference for `velaclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `velaclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
velaclaw secrets reload
velaclaw secrets reload --json
velaclaw secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Velaclaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `velaclaw.json` refs)
...
```bash
velaclaw secrets audit
velaclaw secrets audit --check
velaclaw secrets audit --json
velaclaw secrets audit --allow-exec
```
...
```bash
velaclaw secrets configure
velaclaw secrets configure --plan-out /tmp/velaclaw-secrets-plan.json
velaclaw secrets configure --apply --yes
velaclaw secrets configure --providers-only
velaclaw secrets configure --skip-provider-setup
velaclaw secrets configure --agent ops
velaclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `velaclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Velaclaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
velaclaw secrets apply --from /tmp/velaclaw-secrets-plan.json --json
```
...
- `velaclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.velaclaw/.env` known secret keys whose values were migrated
