---
summary: "Velaclaw Gateway CLI (`velaclaw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Velaclaw’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `velaclaw gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.Zavianx/velaclaw-dev.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
- `velaclaw onboard --mode local` and `velaclaw setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
- `--token <token>`: token override (also sets `VELACLAW_GATEWAY_TOKEN` for the process).
...
```bash
velaclaw gateway health --url ws://127.0.0.1:18789
```
...
```bash
velaclaw gateway usage-cost
velaclaw gateway usage-cost --days 7
velaclaw gateway usage-cost --json
```
...
velaclaw gateway status --require-rpc
```
...
```bash
velaclaw gateway call status
velaclaw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
- For password auth on `gateway run`, prefer `VELACLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `VELACLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
- If both `gateway.auth.token` and `gateway.auth.password` are configured and `gateway.auth.mode` is unset, install is blocked until mode is set explicitly.
- Lifecycle commands accept `--json` for scripting.

## Discover gateways (Bonjour)

`gateway discover` scans for Gateway beacons (`_velaclaw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `velaclaw.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
velaclaw gateway discover --timeout 4000
velaclaw gateway discover --json | jq '.beacons[].wsUrl'
```
