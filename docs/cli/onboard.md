---
summary: "CLI reference for `velaclaw onboard` (interactive onboarding)"
read_when:
...
```bash
velaclaw onboard
velaclaw onboard --flow quickstart
velaclaw onboard --flow manual
velaclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`VELACLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `velaclaw gateway run`.
...
- Fastest first chat: `velaclaw dashboard` (Control UI, no channel setup).
