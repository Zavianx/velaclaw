---
summary: "Move (migrate) an Velaclaw install from one machine to another"
read_when:
  - You are moving Velaclaw to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Velaclaw to a New Machine

This guide moves an Velaclaw gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.velaclaw/` by default) and your **workspace**, you preserve:

- **Config** -- `velaclaw.json` and all gateway settings
...
<Tip>
Run `velaclaw status` on the old machine to confirm your state directory path.
Custom profiles use `~/.velaclaw-<profile>/` or a path set via `VELACLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf velaclaw-state.tgz .velaclaw
    ```

    If you use multiple profiles (e.g. `~/.velaclaw-work`), archive each separately.
...
- [ ] `velaclaw status` shows the gateway running
