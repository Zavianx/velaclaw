---
summary: "Automated, hardened Velaclaw installation with Ansible, Tailscale VPN, and firewall isolation"
read_when:
...
Deploy Velaclaw to production servers with **[velaclaw-ansible](https://github.com/Zavianx/velaclaw-dev-ansible)** -- an automated installer with security-first architecture.

<Info>
The [velaclaw-ansible](https://github.com/Zavianx/velaclaw-dev-ansible) repo is the source of truth for Ansible deployment. This page is a quick overview.
</Info>
...
```bash
curl -fsSL https://raw.githubusercontent.com/Zavianx/velaclaw-dev-ansible/main/install.sh | bash
```
...
5. **Velaclaw** -- host-based, not containerized
...
<Steps>
  <Step title="Switch to the velaclaw user">
    ```bash
    sudo -i -u velaclaw
    ```
  </Step>
...
# Restart gateway
sudo systemctl restart velaclaw

# Provider login (run as velaclaw user)
sudo -i -u velaclaw
velaclaw channels login
```
...
<Steps>
  ...
  <Step title="Run the playbook">
    ...
    # Then run: /tmp/velaclaw-setup.sh
    ```
...
The Ansible installer sets up Velaclaw for manual updates. See [Updating](/install/updating) for the standard update flow.
...
<AccordionGroup>
  ...
  <Accordion title="Service will not start">
    ```bash
    # Check logs
    sudo journalctl -u velaclaw -n 100

    # Verify permissions
    sudo ls -la /opt/velaclaw
  ...
  <Accordion title="Docker sandbox issues">
    ...
    # Check sandbox image
    sudo docker images | grep velaclaw-sandbox

    # Build sandbox image if missing
    cd /opt/Zavianx/velaclaw-dev
    sudo -u velaclaw ./scripts/sandbox-setup.sh
    ```

  </Accordion>
  <Accordion title="Provider login fails">
    Make sure you are running as the `velaclaw` user:
    ```bash
    sudo -i -u velaclaw
    velaclaw channels login
    ```
...
For detailed security architecture and troubleshooting, see the velaclaw-ansible repo:

- [Security Architecture](https://github.com/Zavianx/velaclaw-dev-ansible/blob/main/docs/security.md)
- [Technical Details](https://github.com/Zavianx/velaclaw-dev-ansible/blob/main/docs/architecture.md)
- [Troubleshooting Guide](https://github.com/Zavianx/velaclaw-dev-ansible/blob/main/docs/troubleshooting.md)

## Related

- [velaclaw-ansible](https://github.com/Zavianx/velaclaw-dev-ansible) -- full deployment guide
