---
summary: "Run Velaclaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Velaclaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Velaclaw on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Velaclaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Velaclaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Velaclaw Gateway in Docker
- Persist `~/.velaclaw` + `~/.velaclaw/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.velaclaw` state includes `velaclaw.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-velaclaw-project --name="Velaclaw Gateway"
    gcloud config set project my-velaclaw-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create velaclaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh velaclaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    VELACLAW_IMAGE=velaclaw:latest
    VELACLAW_GATEWAY_TOKEN=change-me-now
    VELACLAW_GATEWAY_BIND=lan
    VELACLAW_GATEWAY_PORT=18789

    VELACLAW_CONFIG_DIR=/home/$USER/.velaclaw
    VELACLAW_WORKSPACE_DIR=/home/$USER/.velaclaw/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.velaclaw
    ```
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `VELACLAW_GATEWAY_TOKEN`.
    ...
    services:
      velaclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - VELACLAW_GATEWAY_BIND=${VELACLAW_GATEWAY_BIND}
          - VELACLAW_GATEWAY_PORT=${VELACLAW_GATEWAY_PORT}
          - VELACLAW_GATEWAY_TOKEN=${VELACLAW_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${VELACLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`VELACLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm velaclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh velaclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop velaclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type velaclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start velaclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create velaclaw-deploy \
     --display-name="Velaclaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-velaclaw-project \
     --member="serviceAccount:velaclaw-deploy@my-velaclaw-project.iam.gserviceaccount.com" \
