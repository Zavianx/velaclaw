---
summary: "Deploy Velaclaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Velaclaw on a Kubernetes cluster
  - You want to test Velaclaw in a Kubernetes environment
title: "Kubernetes"
---

# Velaclaw on Kubernetes

A minimal starting point for running Velaclaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Velaclaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/velaclaw 18789:18789 -n velaclaw
...
```bash
kubectl get secret velaclaw-secrets -n velaclaw -o jsonpath='{.data.VELACLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: velaclaw (configurable via VELACLAW_NAMESPACE)
├── Deployment/velaclaw        # Single pod, init container + gateway
├── Service/velaclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/velaclaw-config  # velaclaw.json + AGENTS.md
└── Secret/velaclaw-secrets    # Gateway token + API keys
```
...
Edit `velaclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret velaclaw-secrets -n velaclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/velaclaw -n velaclaw
```

### Custom namespace

```bash
VELACLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
└── manifests/
    ...
    ├── configmap.yaml          # velaclaw.json + AGENTS.md
