---
summary: "Optional Docker-based setup and onboarding for Velaclaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Velaclaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export VELACLAW_IMAGE="ghcr.io/Zavianx/velaclaw-dev:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/Zavianx/velaclaw-dev/pkgs/container/velaclaw).
  ...
  <Step title="Complete onboarding">
    ...
    `velaclaw-gateway` directly. `velaclaw-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm velaclaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm velaclaw-cli channels login

    # Telegram
    docker compose run --rm velaclaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm velaclaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t velaclaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node velaclaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node velaclaw-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d velaclaw-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `VELACLAW_EXTRA_MOUNTS`
or `VELACLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `velaclaw-cli` shares `velaclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d velaclaw-gateway`, run onboarding
and setup-time config writes through `velaclaw-gateway` with
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `VELACLAW_IMAGE`               | Use a remote image instead of building locally                   |
| `VELACLAW_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `VELACLAW_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `VELACLAW_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `VELACLAW_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `VELACLAW_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `VELACLAW_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec velaclaw-gateway node dist/index.js health --token "$VELACLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `VELACLAW_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `VELACLAW_CONFIG_DIR` to `/home/node/.velaclaw` and
`VELACLAW_WORKSPACE_DIR` to `/home/node/.velaclaw/workspace`, so those paths
survive container replacement.

That mounted config directory is where Velaclaw keeps:

- `velaclaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `VELACLAW_GATEWAY_TOKEN`

For full persistence details on VM deployments, see
[Docker VM Runtime - What persists where](/install/docker-vm-runtime#what-persists-where).

**Disk growth hotspots:** watch `media/`, session JSONL files, `cron/runs/*.jsonl`,
and rolling file logs under `/tmp/velaclaw/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/Zavianx/velaclaw-dev/main/scripts/clawdock/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export VELACLAW_SANDBOX=1
    export VELACLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm velaclaw-cli gateway probe
    docker compose run -T --rm velaclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `velaclaw-cli` uses `network_mode: "service:velaclaw-gateway"` so CLI
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.velaclaw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/velaclaw-config /path/to/velaclaw-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export VELACLAW_HOME_VOLUME="velaclaw_home"`
    2. **Bake system deps**: `export VELACLAW_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `VELACLAW_HOME_VOLUME` or `VELACLAW_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/Zavianx/velaclaw-dev/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm velaclaw-cli dashboard --no-open
    docker compose run --rm velaclaw-cli devices list
    docker compose run --rm velaclaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm velaclaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm velaclaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping Velaclaw up to date
