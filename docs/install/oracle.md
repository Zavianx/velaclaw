---
summary: "Host Velaclaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Velaclaw on Oracle Cloud
  - Looking for free VPS hosting for Velaclaw
...
Run a persistent Velaclaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@velaclaw`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=velaclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `velaclaw doctor --non-interactive` and check logs with `journalctl --user -u velaclaw-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep Velaclaw up to date
