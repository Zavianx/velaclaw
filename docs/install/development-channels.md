...
Velaclaw ships three update channels:
...
```bash
velaclaw update --channel stable
...
- **`dev`**: ensures a git checkout (default `~/velaclaw`, override with
  `VELACLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
velaclaw update --tag 2026.4.1-beta.1

# Install from the beta dist-tag (one-off, does not persist)
velaclaw update --tag beta

# Install from GitHub main branch (npm tarball)
velaclaw update --tag main

# Install a specific npm package spec
velaclaw update --tag velaclaw@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `velaclaw update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Velaclaw prompts for confirmation (skip with `--yes`).
...
Preview what `velaclaw update` would do without making changes:

```bash
velaclaw update --dry-run
velaclaw update --channel beta --dry-run
velaclaw update --tag 2026.4.1-beta.1 --dry-run
velaclaw update --dry-run --json
```
...
When you switch channels with `velaclaw update`, Velaclaw also syncs plugin
sources:
