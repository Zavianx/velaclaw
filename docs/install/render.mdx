---
summary: "Deploy Velaclaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying Velaclaw to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Velaclaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/Zavianx/velaclaw)
...
repository configures everything needed to run Velaclaw:
...
services:
  - type: web
    ...
    envVars:
      - key: VELACLAW_GATEWAY_PORT
        value: "8080"
      - key: VELACLAW_STATE_DIR
        value: /data/.velaclaw
      - key: VELACLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: VELACLAW_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Velaclaw state
...
`VELACLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you deploy directly from the upstream repository instead of your own fork, Render will not auto-deploy your service. To update it, run a manual Blueprint sync from the dashboard.
...
For Velaclaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Velaclaw state plus any configured
...
- Missing `VELACLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `VELACLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `velaclaw backup create` in the Render shell.
