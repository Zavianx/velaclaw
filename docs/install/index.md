---
summary: "Install Velaclaw — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Velaclaw, and launches onboarding.
...
Use this when you want Velaclaw and Node kept under a local prefix such as
`~/.velaclaw`, without depending on a system-wide Node install:

```bash
curl -fsSL https://velaclaw.ai/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g velaclaw@latest
```
...
Or skip the link and use `pnpm velaclaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
velaclaw --version      # confirm the CLI is available
velaclaw doctor         # check for config issues
velaclaw gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `velaclaw onboard --install-daemon` or `velaclaw gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Velaclaw on a cloud server or VPS:
...
## Troubleshooting: `velaclaw` not found

If the install succeeded but `velaclaw` is not found in your terminal:
