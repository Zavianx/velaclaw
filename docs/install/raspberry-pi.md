---
summary: "Host Velaclaw on a Raspberry Pi for always-on self-hosting"
read_when:
...
**Service will not start** -- Check logs with `journalctl --user -u velaclaw-gateway.service --no-pager -n 100` and run `velaclaw doctor --non-interactive`. If this is a headless Pi, also verify lingering is enabled: `sudo loginctl enable-linger "$(whoami)"`.
