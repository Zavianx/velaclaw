...
```bash
velaclaw nodes describe --node <idOrNameOrIp>
velaclaw nodes canvas snapshot --node <idOrNameOrIp>
velaclaw logs --follow
```
...
velaclaw nodes status
velaclaw approvals get --node <idOrNameOrIp>
velaclaw approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`velaclaw approvals get --node ...`), not in the gateway pairing record.
