...
Velaclaw can **summarize inbound media** (image/audio/video) before the reply pipeline runs. It auto‑detects when local tools or provider keys are available, and can be disabled or customized. If understanding is off, models still receive the original files/URLs as usual.

Vendor-specific media behavior is registered by vendor plugins, while Velaclaw
...
- If the active primary image model already supports vision natively, Velaclaw
  skips the `[Image]` summary block and passes the original image into the
  model instead.
- If `<capability>.enabled: true` but no models are configured, Velaclaw tries the
  **active reply model** when its provider supports the capability.

### Auto-detect media understanding (default)

If `tools.media.<capability>.enabled` is **not** set to `false` and you haven’t
configured models, Velaclaw auto-detects in this order and **stops at the first
working option**:
...
When provider-based **audio** and **video** media understanding is enabled, Velaclaw
...
If the proxy value is malformed, Velaclaw logs a warning and falls back to direct
fetch.

## Capabilities (optional)

If you set `capabilities`, the entry only runs for those media types. For shared
lists, Velaclaw can infer defaults:
...
## Provider support matrix (Velaclaw integrations)
...
- `parakeet-mlx` note: with `--output-dir`, Velaclaw reads `<output-dir>/<media-basename>.txt` when output format is `txt` (or unspecified); non-`txt` formats fall back to stdout.
...
- If a file has no extractable text, Velaclaw injects `[No extractable text]`.
