---
summary: "How Velaclaw builds prompt context and reports token usage + costs"
read_when:
...
Velaclaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Velaclaw assembles its own system prompt on every run. It includes:
...
For images, Velaclaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Velaclaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `velaclaw status --usage` and `velaclaw channels list` show
...
available; otherwise Velaclaw falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, Velaclaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Velaclaw can
...
Anthropic's 1M context window is currently beta-gated. Velaclaw can inject the
...
Velaclaw skips the `context-1m-*` beta header because Anthropic currently
