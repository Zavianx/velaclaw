...
This page lists every configuration knob for Velaclaw memory search. For
...
If Velaclaw runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.velaclaw/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
