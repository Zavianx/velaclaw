...
Velaclaw features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, Velaclaw reads usage from
...
Anthropic still does not expose a per-message dollar estimate that Velaclaw can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `velaclaw status --usage` and `velaclaw channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Velaclaw inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Velaclaw prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Velaclaw falls back to matching OAuth/API-key
...
Velaclaw can pick up credentials from:
...
Velaclaw's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Velaclaw Claude-login path with **Extra Usage** enabled.
...
- `velaclaw status --usage`
- `velaclaw models status --json`
...
`velaclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
