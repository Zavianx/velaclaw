...
Velaclaw has three public release lanes:
...
- Every Velaclaw release ships the npm package and macOS app together
...
- Release checks now run in a separate manual workflow:
  `Velaclaw Release Checks`
- Cross-OS install and upgrade runtime validation is dispatched from the
  private caller workflow
  `velaclaw/releases-private/.github/workflows/velaclaw-cross-os-release-checks.yml`,
  which invokes the reusable public workflow
  `.github/workflows/velaclaw-cross-os-release-checks-reusable.yml`
...
- `Velaclaw NPM Release` validation-only preflight also accepts the current
...
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/velaclaw-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/velaclaw-npm-postpublish-verify.ts YYYY.M.D`
...
`Velaclaw NPM Release` accepts these operator-controlled inputs:
...
`Velaclaw Release Checks` accepts these operator-controlled inputs:
...
1. Run `Velaclaw NPM Release` with `preflight_only=true`
...
3. Run `Velaclaw Release Checks` separately with the same tag or the
...
5. Run `Velaclaw NPM Release` again with `preflight_only=false`, the same
