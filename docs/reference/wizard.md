...
This is the full reference for `velaclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.Zavianx/velaclaw-dev.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `velaclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though Velaclaw now prefers Claude CLI reuse when available.
    - **OpenAI Code (Codex) subscription (Codex CLI)**: if `~/.codex/auth.json` exists, onboarding can reuse it. Reused Codex CLI credentials stay managed by Codex CLI; on expiry Velaclaw re-reads that source first and, when the provider can refresh it, writes the refreshed credential back to Codex storage instead of taking ownership itself.
    ...
    - Auth profiles live in `~/.velaclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.velaclaw/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.velaclaw/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$VELACLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
  ...
  <Step title="Workspace">
    - Default `~/.velaclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `velaclaw pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `velaclaw configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `velaclaw health`.
    - Tip: `velaclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
```bash
export VELACLAW_GATEWAY_TOKEN="your-token"
velaclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env VELACLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.velaclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.Zavianx/velaclaw-dev.json`:
...
`velaclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.velaclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.velaclaw/agents/<agentId>/sessions/`.
...
- Providers: [WhatsApp](/channels/whatsapp), [Telegram](/channels/telegram), [Discord](/channels/discord), Google Chat, Signal, BlueBubbles (iMessage), iMessage (legacy)
