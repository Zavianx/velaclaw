...
This document explains how Velaclaw manages sessions end-to-end:
...
Velaclaw is designed around a single **Gateway process** that owns session state.
...
Velaclaw persists sessions in two layers:
...
- Transcripts: `~/.velaclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Velaclaw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Velaclaw reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
velaclaw sessions cleanup --dry-run
velaclaw sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.velaclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Velaclaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Velaclaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Velaclaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Velaclaw preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Velaclaw consumes the events, but Pi decides when to compact).
...
Velaclaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Velaclaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Velaclaw leaves it alone.
...
- If the provider fails or returns an empty result, Velaclaw falls back to built-in LLM summarization automatically.
...
- `velaclaw status` (CLI)
- `velaclaw sessions` / `sessions --json`
...
Velaclaw supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Velaclaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Velaclaw also suppresses **draft/typing streaming** when a
...
Velaclaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Velaclaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `velaclaw status`.
