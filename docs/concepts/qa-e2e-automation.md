...
The private QA stack is meant to exercise Velaclaw in a more realistic,
...
```bash
pnpm velaclaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
disposable server. It requires `VELACLAW_QA_TELEGRAM_GROUP_ID`,
`VELACLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
`VELACLAW_QA_TELEGRAM_SUT_BOT_TOKEN`, plus two distinct bots in the same
...
```bash
pnpm velaclaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Velaclaw
...
```bash
pnpm velaclaw qa character-eval \
