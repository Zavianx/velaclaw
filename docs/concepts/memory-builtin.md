...
Velaclaw indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.velaclaw/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `velaclaw memory index --force`
...
**Memory search disabled?** Check `velaclaw memory status`. If no provider is
detected, set one explicitly or add an API key.

**Stale results?** Run `velaclaw memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Velaclaw falls back to in-process cosine similarity
