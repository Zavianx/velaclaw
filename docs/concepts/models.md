...
Velaclaw selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Velaclaw can use (plus aliases).
...
- If you omit the provider, Velaclaw resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, Velaclaw
...
velaclaw models set <provider/model>
velaclaw models set-image <provider/model>

velaclaw models aliases list
velaclaw models aliases add <alias> <provider/model>
velaclaw models aliases remove <alias>

velaclaw models fallbacks list
velaclaw models fallbacks add <provider/model>
velaclaw models fallbacks remove <provider/model>
velaclaw models fallbacks clear

velaclaw models image-fallbacks list
velaclaw models image-fallbacks add <provider/model>
velaclaw models image-fallbacks remove <provider/model>
velaclaw models image-fallbacks clear
```

`velaclaw models` (no subcommand) is a shortcut for `models status`.
...
`velaclaw models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.velaclaw/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Velaclaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Velaclaw regenerates `models.json`, including command-driven paths like `velaclaw agent`.
