...
read_when:
  - You want to understand how Velaclaw assembles model context
...
A **context engine** controls how Velaclaw builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Velaclaw ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.Zavianx/velaclaw-dev.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Velaclaw plugin. Install
...
velaclaw plugins install -l ./my-context-engine
```
...
Every time Velaclaw runs a model prompt, the context engine participates at
...
Velaclaw currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Velaclaw
...
The built-in `legacy` engine preserves Velaclaw's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Velaclaw uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Velaclaw disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Velaclaw automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `velaclaw/plugin-sdk/core` to use
  Velaclaw's built-in compaction behavior.
...
Velaclaw resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Velaclaw's built-in summarization. Plugin engines can implement
...
- Use `velaclaw doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Velaclaw
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `velaclaw plugins install -l ./my-engine` to link a
