...
alongside Velaclaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Velaclaw falls back to the
...
Velaclaw creates a self-contained QMD home under
`~/.velaclaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Velaclaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). If a mode fails, Velaclaw retries with `qmd query`.
- If QMD fails entirely, Velaclaw falls back to the builtin SQLite engine.
...
process, so you can tune QMD globally without adding new Velaclaw config:
...
collection under `~/.velaclaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Velaclaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Velaclaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Velaclaw uses.
...
Velaclaw's builtin symlink rules. Keep temporary monorepo checkouts under
