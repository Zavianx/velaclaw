...
summary: "How Velaclaw remembers things across sessions"
read_when:
...
Velaclaw remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.velaclaw/workspace`).
...
<Info>
Velaclaw auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Velaclaw
...
```bash
velaclaw memory rem-backfill --rollback
velaclaw memory rem-backfill --rollback-short-term
```

## CLI

```bash
velaclaw memory status          # Check index status and provider
velaclaw memory search "query"  # Search from the command line
velaclaw memory index --force   # Rebuild the index
```
