...
Start with this in `velaclaw.json`:
...
With those enabled, Velaclaw can show:
...
`config.promptOverride` replaces the default Active Memory prompt. Velaclaw
...
5. Verify memory search itself works with `velaclaw memory status --deep`.
...
If `memorySearch.provider` is unset, Velaclaw auto-detects the first available
embedding provider.
...
- if an embedding provider is resolved and then fails at runtime, Velaclaw does
...
- run `velaclaw memory status --deep` to inspect the current memory-search
