...
[Honcho](https://honcho.dev) adds AI-native memory to Velaclaw. It persists
...
```bash
velaclaw plugins install @honcho-ai/velaclaw-honcho
velaclaw honcho setup
...
Settings live under `plugins.entries["velaclaw-honcho"].config`:
    ...
    entries: {
      "velaclaw-honcho": {
        config: {
          ...
          workspaceId: "velaclaw", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `velaclaw honcho setup` detects and
...
```bash
velaclaw honcho setup                        # Configure API key and migrate files
velaclaw honcho status                       # Check connection status
velaclaw honcho ask <question>               # Query Honcho about the user
velaclaw honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/velaclaw-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Velaclaw integration guide](https://docs.honcho.dev/v3/guides/integrations/velaclaw)
- [Memory](/concepts/memory) -- Velaclaw memory overview
