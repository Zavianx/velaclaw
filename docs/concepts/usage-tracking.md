...
- `/usage cost` in chats: local cost summary aggregated from Velaclaw session logs.
- CLI: `velaclaw status --usage` prints a full per-provider breakdown.
- CLI: `velaclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Velaclaw treats
  ...
  quota, so Velaclaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Velaclaw prefers the
...
can supply plugin-specific usage auth logic; otherwise Velaclaw falls back to
