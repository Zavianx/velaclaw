...
- **Session store** (chat history + routing state) under `~/.velaclaw/agents/<agentId>/sessions`.
...
`~/.velaclaw/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.velaclaw` (or `VELACLAW_STATE_DIR`)
- Workspace: `~/.velaclaw/workspace` (or `~/.velaclaw/workspace-<agentId>`)
- Agent dir: `~/.velaclaw/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.velaclaw/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Velaclaw runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.velaclaw/workspace` (or `~/.velaclaw/workspace-<profile>` when `VELACLAW_PROFILE` is set).
- State defaults to `~/.velaclaw/agents/main/agent`.
...
```bash
velaclaw agents add coding
velaclaw agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.velaclaw/agents/<agentId>`.
...
```bash
velaclaw channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.velaclaw/workspace-alex" },
      { id: "mia", workspace: "~/.velaclaw/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, Velaclaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Velaclaw falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.velaclaw/workspace-main" },
      { id: "coding", workspace: "~/.velaclaw/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.velaclaw/workspace-main" },
      { id: "alerts", workspace: "~/.velaclaw/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
velaclaw channels login --channel whatsapp --account personal
velaclaw channels login --channel whatsapp --account biz
```

`~/.Zavianx/velaclaw-dev.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.velaclaw/workspace-home",
        agentDir: "~/.velaclaw/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.velaclaw/workspace-work",
        agentDir: "~/.velaclaw/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.velaclaw/credentials/whatsapp/personal
          // authDir: "~/.velaclaw/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.velaclaw/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.velaclaw/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.velaclaw/workspace-opus",
