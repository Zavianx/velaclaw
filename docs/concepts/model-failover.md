---
summary: "How Velaclaw rotates auth profiles and falls back across models"
read_when:
...
Velaclaw handles failures in two stages:
...
For a normal text run, Velaclaw evaluates candidates in this order:
...
Inside each candidate, Velaclaw tries auth-profile failover before advancing to
...
Velaclaw uses **auth profiles** for both API keys and OAuth tokens.

- Secrets live in `~/.velaclaw/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.velaclaw/agent/auth-profiles.json`).
- Runtime auth-routing state lives in `~/.velaclaw/agents/<agentId>/agent/auth-state.json`.
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.velaclaw/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
...
Profiles live in `~/.velaclaw/agents/<agentId>/agent/auth-profiles.json` under `profiles`.

## Rotation order

When a provider has multiple profiles, Velaclaw chooses an order like this:

1. **Explicit config**: `auth.order[provider]` (if set).
2. **Configured profiles**: `auth.profiles` filtered by provider.
3. **Stored profiles**: entries in `auth-profiles.json` for the provider.

If no explicit order is configured, Velaclaw uses a round‑robin order:
...
Velaclaw **pins the chosen auth profile per session** to keep provider caches warm.
...
they are tried first, but Velaclaw may rotate to another profile on rate limits/timeouts.
User‑pinned profiles stay locked to that profile; if it fails and model fallbacks
are configured, Velaclaw moves to the next model instead of switching profiles.
...
like rate limiting), Velaclaw marks it in cooldown and moves to the next profile.
...
- Velaclaw records `cooldownModel` for rate-limit failures when the failing
  model id is known.
...
Billing/credit failures (for example “insufficient credits” / “credit balance too low”) are treated as failover‑worthy, but they’re usually not transient. Instead of a short cooldown, Velaclaw marks the profile as **disabled** (with a longer backoff) and rotates to the next profile/provider.

Not every billing-shaped response is `402`, and not every HTTP `402` lands
here. Velaclaw keeps explicit billing text in the billing lane even when a
...
If all profiles for a provider fail, Velaclaw moves to the next model in
`agents.defaults.model.fallbacks`. This applies to auth failures, rate limits, and
timeouts that exhausted profile rotation (other errors do not advance fallback).

Overloaded and rate-limit errors are handled more aggressively than billing
cooldowns. By default, Velaclaw allows one same-provider auth-profile retry,
...
Velaclaw builds the candidate list from the currently requested `provider/model`
...
- If the current run is already on a configured fallback in the same provider
  family, Velaclaw keeps using the full configured chain.
- If the current run is on a different provider than config and that current
  model is not already part of the configured fallback chain, Velaclaw does not
...
When every auth profile for a provider is already in cooldown, Velaclaw does
...
When every candidate fails, Velaclaw throws `FallbackSummaryError`. The outer
...
- if the remaining block is a matching model-scoped rate limit, Velaclaw
