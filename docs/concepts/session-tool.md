...
Velaclaw gives agents tools to work across sessions, inspect status, and
...
After the target responds, Velaclaw can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned Velaclaw
...
the completion origin only identifies a channel Velaclaw can still reuse the
