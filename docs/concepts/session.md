---
summary: "How Velaclaw manages conversation sessions"
read_when:
...
Velaclaw organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `velaclaw security audit`.
...
Velaclaw automatically bounds session storage over time. By default, it runs
...
Preview with `velaclaw sessions cleanup --dry-run`.

## Inspecting sessions

- `velaclaw status` -- session store path and recent activity.
- `velaclaw sessions --json` -- all sessions (filter with `--active <minutes>`).
