...
read_when:
  - Generating or reviewing `velaclaw secrets apply` plans
...
This page defines the strict contract enforced by `velaclaw secrets apply`.

If a target does not match these rules, apply fails before mutating configuration.

## Plan file shape

`velaclaw secrets apply --from <plan.json>` expects a `targets` array of plan targets:
...
- `secrets apply` writes supported `velaclaw.json` targets, supported `auth-profiles.json` targets, and optional scrub targets.
...
If apply fails with an invalid target path message, regenerate the plan with `velaclaw secrets configure` or fix the target path to a supported shape above.
