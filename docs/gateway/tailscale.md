...
Velaclaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Velaclaw requires a shared password.
...
- `token` (default when `VELACLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `VELACLAW_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Velaclaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Velaclaw only treats a request as Serve when it arrives from loopback with
...
Prefer `VELACLAW_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
velaclaw gateway --tailscale serve
velaclaw gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Velaclaw to undo `tailscale serve`
