...
- `velaclaw gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `velaclaw doctor` reports no blocking config/service issues.
- `velaclaw channels status --probe` shows live per-account transport status and,
...
velaclaw config get agents.defaults.models
```
...
- Velaclaw model runs fail only on normal agent turns
...
velaclaw infer model run --model <provider/model> --prompt "hi" --json
velaclaw logs --follow
```

Look for:

- direct tiny calls succeed, but Velaclaw runs fail only on larger prompts
...
- direct tiny requests succeed, but Velaclaw agent runs fail with backend/model
  crashes (for example Gemma on some `inferrs` builds) → Velaclaw transport is
...
2. Set `compat.supportsTools: false` for models/backends that cannot handle
   Velaclaw's tool schema surface reliably.
3. Lower prompt pressure where possible: smaller workspace bootstrap, shorter
   ...
   support.
4. If tiny direct requests keep passing while Velaclaw agent turns still crash
...
velaclaw status
velaclaw channels status --probe
velaclaw pairing list --channel <channel> [--account <id>]
velaclaw config get channels
velaclaw logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `velaclaw config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `velaclaw devices list` then `velaclaw devices approve <requestId>`.                                                                                                                                                                                            |
...
If `velaclaw devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `velaclaw devices rotate --scope ...` can only request operator scopes that
...
velaclaw logs --follow
velaclaw doctor
velaclaw gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `velaclaw onboard --mode local` / `velaclaw setup` to restamp the expected local-mode config. If you are running Velaclaw via Podman, the default config path is `~/.Zavianx/velaclaw-dev.json`.
...
Use this when `velaclaw gateway probe` reaches something, but still prints a warning block.

```bash
velaclaw gateway probe
velaclaw gateway probe --json
velaclaw gateway probe --ssh user@gateway-host
```
...
```bash
velaclaw cron status
velaclaw cron list
velaclaw cron runs --id <jobId> --limit 20
velaclaw system heartbeat last
velaclaw logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so Velaclaw skips the model call.
...
velaclaw approvals get --node <idOrNameOrIp>
velaclaw logs --follow
velaclaw status
```
...
velaclaw browser start --browser-profile velaclaw
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `velaclaw browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
velaclaw gateway status
velaclaw config get gateway.mode
velaclaw config get gateway.remote.url
velaclaw config get gateway.auth.mode
```
...
```bash
velaclaw config get gateway.bind
velaclaw config get gateway.auth.mode
velaclaw config get gateway.auth.token
velaclaw gateway status
velaclaw logs --follow
```
