...
velaclaw gateway --port 18789 --verbose
# force-kill listener on selected port, then start
velaclaw gateway --force
```
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `VELACLAW_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `VELACLAW_GATEWAY_TOKEN` / `VELACLAW_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

Velaclaw’s highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `velaclaw`, `velaclaw/default`, and `velaclaw/<agentId>`.
- `velaclaw/default` is the stable alias that always maps to the configured default agent.
- Use `x-velaclaw-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `VELACLAW_GATEWAY_PORT` → `gateway.port` → `18789` |
...
```bash
velaclaw gateway status
velaclaw gateway status --deep   # adds a system-level service scan
velaclaw gateway status --json
velaclaw gateway install
velaclaw gateway restart
velaclaw gateway stop
velaclaw secrets reload
velaclaw logs --follow
velaclaw doctor
```
...
LaunchAgent labels are `ai.velaclaw.gateway` (default) or `ai.velaclaw.<profile>` (named profile). `velaclaw doctor` audits and repairs service config drift.
...
```bash
velaclaw gateway install
systemctl --user enable --now velaclaw-gateway[-<profile>].service
velaclaw gateway status
```
...
```ini
[Unit]
Description=Velaclaw Gateway
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/velaclaw gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `Velaclaw Gateway`
(or `Velaclaw Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, Velaclaw falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now velaclaw-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/velaclaw-gateway[-<profile>].service` and adjust
`ExecStart=` if your `velaclaw` binary lives elsewhere.
...
- Unique `gateway.port`
- Unique `VELACLAW_CONFIG_PATH`
- Unique `VELACLAW_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
VELACLAW_CONFIG_PATH=~/.velaclaw/a.json VELACLAW_STATE_DIR=~/.velaclaw-a velaclaw gateway --port 19001
VELACLAW_CONFIG_PATH=~/.velaclaw/b.json VELACLAW_STATE_DIR=~/.velaclaw-b velaclaw gateway --port 19002
```
...
```bash
velaclaw --dev setup
velaclaw --dev gateway --allow-unconfigured
velaclaw --dev status
```
