...
Velaclaw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`velaclaw agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-velaclaw-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-velaclaw-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "velaclaw"`, `model: "velaclaw/default"`, `model: "velaclaw/<agentId>"`, or `x-velaclaw-agent-id`
- use `x-velaclaw-model` when you want to override the selected agent's backend model
- use `x-velaclaw-session-key` for explicit session routing
- use `x-velaclaw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Velaclaw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Velaclaw normalizes common OpenAI-style aliases before those counters reach
