...
read_when:
  - Setting up Velaclaw for the first time
...
Velaclaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.Zavianx/velaclaw-dev.json`.

If the file is missing, Velaclaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `velaclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  ...
  <Tab title="CLI (one-liners)">
    ```bash
    velaclaw config get agents.defaults.workspace
...
<Warning>
Velaclaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `velaclaw config schema` prints the same JSON Schema family used by Control UI
  and config validation.
- Treat that schema output as the canonical machine-readable contract for
  `velaclaw.json`; this overview and the configuration reference summarize it.
...
- Only diagnostic commands work (`velaclaw doctor`, `velaclaw logs`, `velaclaw health`, `velaclaw status`)
- Run `velaclaw doctor` to see exact issues
- Run `velaclaw doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `velaclaw.json`.
    ...
    ```bash
    velaclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `VELACLAW_APNS_RELAY_BASE_URL` and `VELACLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `VELACLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-velaclaw-token`); query-string tokens are rejected.
...
The Gateway watches `~/.Zavianx/velaclaw-dev.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    ```bash
    velaclaw gateway call config.get --params '{}'  # capture payload.hash
    velaclaw gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.velaclaw/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    velaclaw gateway call config.patch --params '{
...
Velaclaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.velaclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Velaclaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `VELACLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${VELACLAW_GATEWAY_TOKEN}" } },
