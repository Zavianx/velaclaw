...
- During heartbeat runs, Velaclaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `velaclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
- If only alert delivery is disabled, Velaclaw can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
...
If **all three** are false, Velaclaw skips the heartbeat run entirely (no model call).
...
- Velaclaw parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
velaclaw system event --text "Check for urgent follow-ups" --mode now
```
