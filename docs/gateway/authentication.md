...
Velaclaw supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Velaclaw also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `velaclaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.velaclaw/.env` so the daemon can read it:

```bash
cat >> ~/.velaclaw/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `velaclaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.velaclaw/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Velaclaw as a supported token
path. Anthropic staff has since told us that Velaclaw-style Claude CLI usage is
...
```bash
velaclaw models auth paste-token --provider openrouter
```
...
- If auth exists but Velaclaw cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Velaclaw integration path is allowed again.
- Velaclaw therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `VELACLAW_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Velaclaw retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `velaclaw models status --probe` shows omitted
...
Run `velaclaw models status` to confirm which profile is expiring. If an
