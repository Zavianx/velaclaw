...
read_when:
  - Running Velaclaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Velaclaw
...
- You run Velaclaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Velaclaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Velaclaw extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Velaclaw can verify.
...
- Velaclaw can stay on loopback HTTP behind the proxy.
...
If Velaclaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://velaclaw.example.com
    to: http://velaclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
velaclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy velaclaw:18789 {
...
location / {
    ...
    proxy_pass http://velaclaw:18789;
...
Velaclaw rejects ambiguous configurations where both a `gateway.auth.token` (or `VELACLAW_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-velaclaw-scopes`.

Examples:

- `x-velaclaw-scopes: operator.read`
- `x-velaclaw-scopes: operator.read,operator.write`
- `x-velaclaw-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Velaclaw honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-velaclaw-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-velaclaw-scopes` explicitly when you want a trusted-proxy request to
...
`velaclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
Velaclaw rejected a loopback-source trusted-proxy request.
...
3. Update Velaclaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `velaclaw security audit` and review findings
