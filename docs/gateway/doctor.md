...
`velaclaw doctor` is the repair + migration tool for Velaclaw. It fixes stale
...
```bash
velaclaw doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
velaclaw doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
velaclaw doctor --non-interactive
```
...
- Extra workspace dir detection (`~/velaclaw`).
...
doctor-style RPC methods, but they are **not** part of `velaclaw doctor` CLI
repair/migration.
...
```bash
velaclaw memory rem-backfill --path ./memory --stage-short-term
```
...
you to run `velaclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.Zavianx/velaclaw-dev.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `velaclaw doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.velaclaw/sessions/` to `~/.velaclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.velaclaw/agent/` to `~/.velaclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.velaclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.velaclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `velaclaw doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.velaclaw/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.velaclaw` folders exist across
  home directories or when `VELACLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.Zavianx/velaclaw-dev.json` is
...
that re-auth is required and prints the exact `velaclaw models auth login --provider ...`
command to run.
...
Discord plugin runtime packages) are present in the Velaclaw install root.
If any are missing, doctor reports the packages and installs them in
`velaclaw doctor --fix` / `velaclaw doctor --repair` mode.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the Velaclaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Velaclaw gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`velaclaw doctor` without `--fix`) this check
is skipped entirely.
...
- **Legacy workspace dirs**: warns when `~/velaclaw` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(velaclaw completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `velaclaw completion --write-state` to regenerate the cache manually.
...
- `velaclaw doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `velaclaw doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `velaclaw memory status --deep` to verify embedding readiness at runtime.
...
- `velaclaw doctor` prompts before rewriting supervisor config.
- `velaclaw doctor --yes` accepts the default repair prompts.
- `velaclaw doctor --repair` applies recommended fixes without prompts.
- `velaclaw doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `velaclaw gateway install --force`.
