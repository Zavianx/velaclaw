...
Velaclaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `VELACLAW_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Velaclaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Velaclaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Velaclaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/velaclaw"],
...
- Velaclaw resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__VELACLAW_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `velaclaw.json` refs)
...
When reload-time activation fails after a healthy state, Velaclaw enters degraded secrets state.
...
- Strict command paths (for example `velaclaw memory` remote-memory paths and `velaclaw qr --remote` when it needs remote shared-secret refs) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `velaclaw status`, `velaclaw status --all`, `velaclaw channels status`, `velaclaw channels resolve`, `velaclaw security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `velaclaw secrets reload`.
...
```bash
velaclaw secrets audit --check
velaclaw secrets configure
velaclaw secrets audit --check
```
...
- plaintext values at rest (`velaclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `velaclaw.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `velaclaw secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `velaclaw.json` plus `auth-profiles.json` for one agent scope
...
- `velaclaw secrets configure --providers-only`
- `velaclaw secrets configure --skip-provider-setup`
- `velaclaw secrets configure --agent <id>`
...
```bash
velaclaw secrets apply --from /tmp/velaclaw-secrets-plan.json
velaclaw secrets apply --from /tmp/velaclaw-secrets-plan.json --allow-exec
velaclaw secrets apply --from /tmp/velaclaw-secrets-plan.json --dry-run
velaclaw secrets apply --from /tmp/velaclaw-secrets-plan.json --dry-run --allow-exec
```
...
Velaclaw intentionally does not write rollback backups containing historical plaintext secret values.
