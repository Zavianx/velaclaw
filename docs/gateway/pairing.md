...
velaclaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.velaclaw`):

- `~/.velaclaw/nodes/paired.json`
- `~/.velaclaw/nodes/pending.json`

If you override `VELACLAW_STATE_DIR`, the `nodes/` folder moves with it.
