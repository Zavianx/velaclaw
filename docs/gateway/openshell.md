---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Velaclaw agents"
read_when:
...
OpenShell is a managed sandbox backend for Velaclaw. Instead of running Docker
containers locally, Velaclaw delegates sandbox lifecycle to the `openshell` CLI,
...
- Velaclaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Velaclaw creates an OpenShell
...
- You edit files locally outside Velaclaw and want those changes visible in the
...
- When the sandbox is first created, Velaclaw seeds the remote workspace from
...
- Velaclaw does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Velaclaw after the initial seed,
the remote sandbox does **not** see those changes. Use
`velaclaw sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"velaclaw"`  | Sandbox source for first-time create                  |
...
1. Velaclaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Velaclaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `velaclaw sandbox` commands
