---
summary: "How Velaclaw sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Velaclaw can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`velaclaw-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Velaclaw emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
If you deploy the Velaclaw Gateway itself as a Docker container, it orchestrates sibling sandbox containers using the host's Docker socket (DooD). This introduces a specific path mapping constraint:

- **Config Requires Host Paths**: The `velaclaw.json` `workspace` configuration MUST contain the **Host's absolute path** (e.g. `/home/user/.velaclaw/workspaces`), not the internal Gateway container path. When Velaclaw asks the Docker daemon to spawn a sandbox, the daemon evaluates paths relative to the Host OS namespace, not the Gateway namespace.
- **FS Bridge Parity (Identical Volume Map)**: The Velaclaw Gateway native process also writes heartbeat and bridge files to the `workspace` directory. Because the Gateway evaluates the exact same string (the host path) from within its own containerized environment, the Gateway deployment MUST include an identical volume map linking the host namespace natively (`-v /home/user/.velaclaw:/home/user/.velaclaw`).

If you map paths internally without absolute host parity, Velaclaw natively throws an `EACCES` permission error attempting to write its heartbeat inside the container environment because the fully qualified path string doesn't exist natively.

### SSH backend

Use `backend: "ssh"` when you want Velaclaw to sandbox `exec`, file tools, and media reads on
...
- Velaclaw creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, Velaclaw seeds that remote workspace from the local workspace once.
...
- Velaclaw does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. Velaclaw resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside Velaclaw after the seed step are not visible remotely until you recreate the sandbox.
- `velaclaw sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
- Browser sandboxing is not supported on the SSH backend.
- `sandbox.docker.*` settings do not apply to the SSH backend.

### OpenShell backend

Use `backend: "openshell"` when you want Velaclaw to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. Velaclaw syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. Velaclaw seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- Velaclaw asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- Before `exec`, Velaclaw syncs the local workspace into the OpenShell sandbox.
- After `exec`, Velaclaw syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

Use this when:

- you edit files locally outside Velaclaw and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, Velaclaw seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- Velaclaw does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside Velaclaw after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `velaclaw sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `velaclaw sandbox recreate` deletes the current runtime and lets Velaclaw recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.velaclaw/sandboxes`.
...
Velaclaw mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Velaclaw blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- Velaclaw also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. Velaclaw normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
Default Docker image: `velaclaw-sandbox:bookworm-slim`
...
`velaclaw-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `VELACLAW_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `VELACLAW_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `VELACLAW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `VELACLAW_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `VELACLAW_DOCKER_SOCKET`. Full setup and env
...
- Use `velaclaw sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
