...
Velaclaw can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Velaclaw tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
velaclaw agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, Velaclaw now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Velaclaw will
...
2. **Builds a system prompt** using the same Velaclaw prompt + workspace context.
...
told us Velaclaw-style Claude CLI usage is allowed again, so Velaclaw treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes Velaclaw's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so Velaclaw writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the Velaclaw skills snapshot
two ways: the compact Velaclaw skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that Velaclaw would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by Velaclaw to the
...
- Velaclaw drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
Velaclaw will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Velaclaw appends the
...
- For Gemini CLI JSON output, Velaclaw reads reply text from `response` and
...
- `stats.cached` is normalized into Velaclaw `cacheRead`.
- If `stats.input` is missing, Velaclaw derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Velaclaw handles
...
CLI backends do **not** receive Velaclaw tool calls directly, but a backend can
...
When bundle MCP is enabled, Velaclaw:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`VELACLAW_MCP_TOKEN`)
...
If no MCP servers are enabled, Velaclaw still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct Velaclaw tool calls.** Velaclaw does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. Velaclaw sessions still work
  normally.
