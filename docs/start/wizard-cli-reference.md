...
read_when:
  - You need detailed behavior for velaclaw onboard
...
This page is the full reference for `velaclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.Zavianx/velaclaw-dev.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `velaclaw doctor` before continuing.
  ...
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first
      - If task creation is denied, VelaClaw falls back to a per-user Startup-folder login item and starts the gateway immediately.
      - Scheduled Tasks remain preferred because they provide better supervisor status.
    - Runtime selection: Node (recommended; required for WhatsApp and Telegram). Bun is not recommended.

  </Step>
  <Step title="Health check">
    - Starts gateway (if needed) and runs `velaclaw health`.
    - `velaclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported.
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI Code subscription (Codex CLI reuse)">
    ...
    Reused Codex CLI credentials stay managed by Codex CLI; on expiry VelaClaw
...
- Auth profiles (API keys + OAuth): `~/.velaclaw/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`velaclaw onboard`](/cli/onboard)
