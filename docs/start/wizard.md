...
CLI onboarding is the **recommended** way to set up VelaClaw on macOS,
...
`velaclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`velaclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.velaclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `velaclaw doctor` first.
</Note>
...
Use `velaclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.velaclaw/workspace-<agentId>`.
...
- CLI command reference: [`velaclaw onboard`](/cli/onboard)
