---
summary: "Scripted onboarding and agent setup for the VelaClaw CLI"
read_when:
...
Use `--non-interactive` to automate `velaclaw onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but VelaClaw now prefers Claude CLI reuse when available.
