...
This is handy when you need to toggle obscure settings without editing `velaclaw.json`.
...
extension `package.json` and `velaclaw.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.velaclaw-dev` and
...
```bash
pnpm gateway:dev
VELACLAW_PROFILE=dev velaclaw tui
```

If you don’t have a global install yet, run the CLI via `pnpm velaclaw ...`.
...
```bash
VELACLAW_PROFILE=dev velaclaw gateway --dev --reset
```
...
Velaclaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.velaclaw/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
VELACLAW_RAW_STREAM=1
VELACLAW_RAW_STREAM_PATH=~/.velaclaw/logs/raw-stream.jsonl
```

Default file:

`~/.velaclaw/logs/raw-stream.jsonl`
