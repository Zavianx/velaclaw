---
summary: "Symptom first troubleshooting hub for Velaclaw"
read_when:
  - Velaclaw is not working and you need the fastest path to a fix
...
- `velaclaw status` → shows configured channels and no obvious auth errors.
- `velaclaw status --all` → full report is present and shareable.
- `velaclaw gateway probe` → expected gateway target is reachable (`Reachable: yes`). `RPC: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `velaclaw gateway status` → `Runtime: running` and `RPC probe: ok`.
- `velaclaw doctor` → no blocking config/service errors.
- `velaclaw channels status --probe` → reachable gateway returns live per-account
...
- `velaclaw logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in Velaclaw

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `velaclaw infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Velaclaw agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Velaclaw prompts crash the
...
## Plugin install fails with missing velaclaw extensions

If install fails with `package.json missing velaclaw.extensions`, the plugin package
is using an old shape that Velaclaw no longer accepts.

Fix in the plugin package:

1. Add `velaclaw.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `velaclaw plugins install <package>` again.

Example:

```json
{
  "name": "@velaclaw/my-plugin",
  "version": "1.2.3",
  "velaclaw": {
...
```mermaid
flowchart TD
  A[Velaclaw is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `velaclaw gateway status`
    - `RPC probe: ok`
  ...
  <Accordion title="Cron or heartbeat did not fire or did not deliver">
    ...
    <Accordion title="Exec suddenly asks for approval">
      ```bash
      velaclaw config get tools.exec.host
      velaclaw config get tools.exec.security
      velaclaw config get tools.exec.ask
    ...
    <Accordion title="Browser tool fails">
      ...
      - `velaclaw` starts, or `user` can see local Chrome tabs.
      ...
      - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `velaclaw browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
