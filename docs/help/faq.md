---
summary: "Frequently asked questions about Velaclaw setup, configuration, and usage"
read_when:
...
5. **Tail the latest log**
   ...
   ```bash
   velaclaw logs --follow
   ```

   If RPC is down, fall back to:

   ```bash
   tail -f "$(ls -t /tmp/Zavianx/velaclaw-dev-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   ```bash
   velaclaw health --json
   velaclaw health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://velaclaw.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Velaclaw **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/Zavianx/velaclaw-dev/issues](https://github.com/Zavianx/velaclaw-dev/issues)
    [https://github.com/Zavianx/velaclaw-dev/pulls](https://github.com/Zavianx/velaclaw-dev/pulls)

    Start with these commands (share outputs when asking for help):

    ```bash
    velaclaw status
    velaclaw models status
    velaclaw doctor
    ```

    What they do:

    - `velaclaw status`: quick snapshot of gateway/agent health + basic config.
    - `velaclaw models status`: checks provider auth + model availability.
    - `velaclaw doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `velaclaw status --all`, `velaclaw logs --follow`,
    `velaclaw gateway status`, `velaclaw health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Velaclaw">
    ...
    If you don't have a global install yet, run it via `pnpm velaclaw onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `VELACLAW_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `VELACLAW_GATEWAY_PASSWORD`).
    - If no shared secret is configured yet, generate a token with `velaclaw doctor --generate-gateway-token`.

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `velaclaw gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes trusted gateway host); HTTP APIs still require shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `velaclaw gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, then paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - If a supported native channel can infer approvers safely, Velaclaw now auto-enables DM-first native approvals when `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Velaclaw on the new machine.
    2. Copy `$VELACLAW_STATE_DIR` (default: `~/.velaclaw`) from the old machine.
    3. Copy your workspace (default: `~/.velaclaw/workspace`).
    4. Run `velaclaw doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.velaclaw/` (for example `~/.velaclaw/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/Zavianx/velaclaw-dev/blob/main/CHANGELOG.md](https://github.com/Zavianx/velaclaw-dev/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.velaclaw.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.velaclaw.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.velaclaw.ai`, then retry.
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/Zavianx/velaclaw-dev/tree/main/docs](https://github.com/Zavianx/velaclaw-dev/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://velaclaw.ai/install.sh | bash -s -- --beta
    ```
    ...
    [https://velaclaw.ai/install.ps1](https://velaclaw.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://velaclaw.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://velaclaw.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://velaclaw.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or velaclaw not recognized">
    ...
    **2) velaclaw is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Velaclaw, track/report it in:

    - [Issue #30640](https://github.com/Zavianx/velaclaw-dev/issues/30640)
  ...
  <Accordion title="How do I install Velaclaw on Linux?">
  ...
  <Accordion title="How do I install Velaclaw on a VPS?">
  ...
  <Accordion title="Can I ask Velaclaw to update itself?">
    ...
    ```bash
    velaclaw update
    velaclaw update status
    velaclaw update --channel stable|beta|dev
    velaclaw update --tag <dist-tag|version>
    velaclaw update --no-restart
    ```

    If you must automate from an agent:

    ```bash
    velaclaw update --yes --no-restart
    velaclaw gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `velaclaw onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Velaclaw with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    Pro/Max or OpenAI Codex) are optional ways to authenticate those providers.

    For Anthropic in Velaclaw, the practical split is:

    - **Anthropic API key**: normal Anthropic API billing
    - **Claude CLI / Claude subscription auth in Velaclaw**: Anthropic staff
      told us this usage is allowed again, and Velaclaw is treating `claude -p`
    ...
    Velaclaw also supports other hosted subscription-style options including
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    ...
    Anthropic staff told us Velaclaw-style Claude CLI usage is allowed again, so
    Velaclaw treats Claude subscription auth and `claude -p` usage as sanctioned
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic staff told us this usage is allowed again, so Velaclaw treats
    Claude CLI reuse and `claude -p` usage as sanctioned for this integration
    unless Anthropic publishes a new policy.

    Anthropic setup-token is still available as a supported Velaclaw token path, but Velaclaw now prefers Claude CLI reuse and `claude -p` when available.
    ...
    options in Velaclaw, see [OpenAI](/providers/openai), [Qwen / Model
...
for usage/billing and raise limits as needed.
    ...
    Velaclaw Claude-login path with Extra Usage enabled).

    Tip: set a **fallback model** so Velaclaw can keep replying while a provider is rate-limited.
  ...
  <Accordion title="Why does ChatGPT GPT-5.4 not unlock openai/gpt-5.4 in Velaclaw?">
    Velaclaw treats the two routes separately:

    - `openai-codex/gpt-5.4` = ChatGPT/Codex OAuth
    - `openai/gpt-5.4` = direct OpenAI Platform API

    In Velaclaw, ChatGPT/Codex sign-in is wired to the `openai-codex/*` route,
    not the direct `openai/*` route. If you want the direct API path in
    Velaclaw, set `OPENAI_API_KEY` (or the equivalent OpenAI provider config).
    If you want ChatGPT/Codex sign-in in Velaclaw, use `openai-codex/*`.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    Velaclaw can show the currently visible provider usage/quota windows in
    `velaclaw models status`, but it does not invent or normalize ChatGPT-web
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Velaclaw fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Velaclaw. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `velaclaw.json`.
    ...
    2. Enable the plugin: `velaclaw plugins enable google`
    3. Login: `velaclaw models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="If I buy a Mac mini to run Velaclaw, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `velaclaw nodes status` / `velaclaw nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but Velaclaw authorization uses numeric IDs only.

    Safer (no third-party bot):

    - DM your bot, then run `velaclaw logs --follow` and read `from.id`.
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Velaclaw via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the Velaclaw code install. Your state
    (`~/.velaclaw`) and workspace (`~/.velaclaw/workspace`) stay untouched.
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Velaclaw-specific note:** WhatsApp/Telegram/Slack/Mattermost/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Velaclaw on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Velaclaw is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Velaclaw in a VM and what are the requirements?">
...
<AccordionGroup>
  ...
  <Accordion title="Value proposition">
    Velaclaw is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
  ...
  <Accordion title="What are the top five everyday use cases for Velaclaw?">
  ...
  <Accordion title="Can Velaclaw help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Velaclaw draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Velaclaw is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Velaclaw when you
    ...
    Showcase: [https://velaclaw.ai/showcase](https://velaclaw.ai/showcase)
...
<AccordionGroup>
  ...
  <Accordion title="A subagent finished, but the completion update went to the wrong place or never posted. What should I check?">
    ...
    - If the completion origin only carries a channel, Velaclaw falls back to the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) so direct delivery can still succeed.
    ...
    - If the child's last visible assistant reply is the exact silent token `NO_REPLY` / `no_reply`, or exactly `ANNOUNCE_SKIP`, Velaclaw intentionally suppresses the announce instead of posting stale earlier progress.
    ...
    ```bash
    velaclaw tasks show <runId-or-sessionKey>
    ```
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    - Confirm cron is enabled (`cron.enabled`) and `VELACLAW_SKIP_CRON` is not set.
    ...
    ```bash
    velaclaw cron run <jobId>
    velaclaw cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `velaclaw skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.ai](https://clawhub.ai).

    ```bash
    velaclaw skills search "calendar"
    velaclaw skills search --limit 20
    velaclaw skills install <skill-slug>
    velaclaw skills install <skill-slug> --version <version>
    velaclaw skills install <skill-slug> --force
    velaclaw skills update --all
    velaclaw skills list --eligible
    velaclaw skills check
    ```

    Native `velaclaw skills install` writes into the active workspace `skills/`
    ...
    `~/.velaclaw/skills` and use `agents.defaults.skills` or
    `agents.list[].skills` if you want to narrow which agents can see it.

  </Accordion>

  <Accordion title="Can Velaclaw run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.velaclaw.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.velaclaw/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "velaclaw": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.velaclaw/skills/<name>/SKILL.md`. If only some agents should see a shared install, configure `agents.defaults.skills` or `agents.list[].skills`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills), [Skills config](/tools/skills-config), and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Velaclaw?">
    ...
    velaclaw browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Docker feels limited - how do I enable full features?">
    ...
    - Persist `/home/node` with `VELACLAW_HOME_VOLUME` so caches survive.
    - Bake system deps into the image with `VELACLAW_DOCKER_APT_PACKAGES`.
  ...
  <Accordion title="How do I bind a host folder into the sandbox?">
    ...
    Velaclaw validates bind sources against both the normalized path and the canonical path resolved through the deepest existing ancestor. That means symlink-parent escapes still fail closed even when the last path segment does not exist yet, and allowed-root checks still apply after symlink resolution.
  ...
  <Accordion title="How does memory work?">
    Velaclaw memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Velaclaw also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Velaclaw auto-selects a provider when it
    ...
    configured and present, Velaclaw
...
<AccordionGroup>
  <Accordion title="Is all data used with Velaclaw saved locally?">
    No - **Velaclaw's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.velaclaw` + your workspace directory).
  ...
  <Accordion title="Where does Velaclaw store its data?">
    Everything lives under `$VELACLAW_STATE_DIR` (default: `~/.velaclaw`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$VELACLAW_STATE_DIR/velaclaw.json`                             | Main config (JSON5)                                                |
    | `$VELACLAW_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
    | `$VELACLAW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
    | `$VELACLAW_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
    | `$VELACLAW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
    | `$VELACLAW_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
    | `$VELACLAW_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
    | `$VELACLAW_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
    | `$VELACLAW_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

    Legacy single-agent path: `~/.velaclaw/agent/*` (migrated by `velaclaw doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.velaclaw/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.velaclaw`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.velaclaw`)**: config, channel/provider state, auth profiles, sessions, logs,
      and shared skills (`~/.velaclaw/skills`).

    Default workspace is `~/.velaclaw/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.velaclaw/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.velaclaw` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Velaclaw repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Velaclaw reads an optional **JSON5** config from `$VELACLAW_CONFIG_PATH` (default: `~/.Zavianx/velaclaw-dev.json`):

    ```
    $VELACLAW_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.velaclaw/workspace`).
  ...
  <Accordion title='I set gateway.bind: "lan" (or "tailnet") and now nothing listens / the UI says unauthorized'>
    ...
    - For password auth, set `gateway.auth.mode: "password"` plus `gateway.auth.password` (or `VELACLAW_GATEWAY_PASSWORD`) instead.
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Velaclaw enforces gateway auth by default, including loopback. In the normal default path that means token auth: if no explicit auth path is configured, gateway startup resolves to token mode and auto-generates one, saving it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you prefer a different auth path, you can explicitly choose password mode (or, for non-loopback identity-aware reverse proxies, `trusted-proxy`). If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `velaclaw doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Velaclaw.` every time.
    - `random`: rotating funny/seasonal taglines (default behavior).
    - If you want no banner at all, set env `VELACLAW_HIDE_BANNER=1`.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `velaclaw configure --section web` and choose a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Velaclaw auto-detects the first ready fetch fallback provider from available credentials. Today the bundled provider is Firecrawl.
    - Daemons read env vars from `~/.velaclaw/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - Restore from backup (git or a copied `~/.Zavianx/velaclaw-dev.json`).
    - If you have no backup, re-run `velaclaw doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `velaclaw config set` for small changes.
    - Use `velaclaw configure` for interactive edits.
  ...
  <Accordion title="Can the Velaclaw browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="How can my agent access my computer if the Gateway is hosted remotely?">
    ...
    5. Approve the node on the Gateway:
       ...
       ```bash
       velaclaw devices list
       velaclaw devices approve <requestId>
       ```
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `velaclaw gateway status`
    - Gateway health: `velaclaw status`
    - Channel health: `velaclaw channels status`
  ...
  <Accordion title="Can two Velaclaw instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `velaclaw agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    velaclaw agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    velaclaw gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does Velaclaw load environment variables?">
    Velaclaw reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.velaclaw/.env` (aka `$VELACLAW_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.velaclaw/.env` so they're picked up even when the service doesn't inherit your shell env.
    ...
    `VELACLAW_LOAD_SHELL_ENV=1`, `VELACLAW_SHELL_ENV_TIMEOUT_MS=15000`.

  </Accordion>

  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `velaclaw models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Velaclaw won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.velaclaw/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Velaclaw instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Velaclaw but keep it installed?">
    ...
    - If you used profiles (`--profile` / `VELACLAW_PROFILE`), reset each state dir (defaults are `~/.velaclaw-<profile>`).
    - Dev reset: `velaclaw gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Velaclaw skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Velaclaw runs on **your own account**, so if you're in the group, Velaclaw can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    velaclaw directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Velaclaw not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.velaclaw/agents/<agentId>/sessions/`.
    ...
    - Use `velaclaw doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Velaclaw's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `openai/gpt-5.4`). If you omit the provider, Velaclaw first tries an alias, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider as a deprecated compatibility path. If that provider no longer exposes the configured default model, Velaclaw falls back to the first configured provider/model instead of surfacing a stale removed-provider default. You should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `velaclaw models set ...` (updates just model config)
    - `velaclaw configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.Zavianx/velaclaw-dev.json`
    ...
    for partial updates.
    If you did overwrite config, restore from backup or re-run `velaclaw doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `velaclaw onboard` and choose `Ollama`
    ...
    - for manual switching, use `velaclaw models list` and `velaclaw models set ollama/<model>`
  ...
  <Accordion title="What do Velaclaw, Flawd, and Krill use for models?">
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Velaclaw release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Velaclaw ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.velaclaw/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `velaclaw agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Velaclaw can keep responding even when a provider is rate-limited or temporarily failing.

    The rate-limit bucket includes more than plain `429` responses. Velaclaw
    ...
    explicit billing text on `401` or `403`, Velaclaw can still keep that in
    ...
    `organization spending limit exceeded`), Velaclaw treats it as
    ...
    unknown/error in it". Velaclaw does treat provider-scoped transient shapes
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.velaclaw/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.velaclaw/agent/*` (migrated by `velaclaw doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.velaclaw/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `velaclaw models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use Claude CLI**
      - Run `velaclaw models auth login --provider anthropic --method cli --set-default` on the gateway host.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.velaclaw/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        velaclaw models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Velaclaw will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Velaclaw now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    Velaclaw uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Velaclaw may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `velaclaw models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
    ...
    velaclaw models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    velaclaw models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    velaclaw models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    velaclaw models auth order set --provider anthropic --agent main anthropic:default
    ```

    To verify what will actually be tried, use:

    ```bash
    velaclaw models status --probe
    ```
...
<AccordionGroup>
  <Accordion title="What port does the Gateway use?">
    ...
    ```
    --port > VELACLAW_GATEWAY_PORT > gateway.port > default 18789
    ```

  </Accordion>

  <Accordion title='Why does velaclaw gateway status say "Runtime: running" but "RPC probe: failed"?'>
    Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

    Use `velaclaw gateway status` and trust these lines:
  ...
  <Accordion title='Why does velaclaw gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `VELACLAW_STATE_DIR` mismatch).

    Fix:

    ```bash
    velaclaw gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Velaclaw enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `velaclaw gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Velaclaw in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `velaclaw gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `velaclaw dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `velaclaw doctor --generate-gateway-token`.
    - If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
    - Shared-secret mode: set `gateway.auth.token` / `VELACLAW_GATEWAY_TOKEN` or `gateway.auth.password` / `VELACLAW_GATEWAY_PASSWORD`, then paste the matching secret in Control UI settings.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `velaclaw devices list`
      - `velaclaw devices rotate --device <id> --role operator`
    ...
    - Still stuck? Run `velaclaw status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - `VELACLAW_CONFIG_PATH` (per-instance config)
    - `VELACLAW_STATE_DIR` (per-instance state)
    - `agents.defaults.workspace` (workspace isolation)
    - `gateway.port` (unique ports)

    Quick setup (recommended):

    - Use `velaclaw --profile <name> ...` per instance (auto-creates `~/.velaclaw-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `velaclaw --profile <name> gateway install`.

    Profiles also suffix service names (`ai.velaclaw.<profile>`; legacy `com.velaclaw.*`, `velaclaw-gateway-<profile>.service`, `Velaclaw Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    velaclaw tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    ```
    /tmp/Zavianx/velaclaw-dev-YYYY-MM-DD.log
    ```
    ...
    - macOS: `$VELACLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.velaclaw/logs/...`; profiles use `~/.velaclaw-<profile>/logs/...`)
    - Linux: `journalctl --user -u velaclaw-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Velaclaw Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `velaclaw gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Velaclaw?">
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `velaclaw gateway status`
    2. Is the Gateway healthy? `velaclaw status`
    3. Does the UI have the right token? `velaclaw dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    velaclaw channels status
    velaclaw channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Velaclaw already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="ELI5: velaclaw gateway restart vs velaclaw gateway">
    - `velaclaw gateway restart`: restarts the **background service** (launchd/systemd).
    - `velaclaw gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `velaclaw gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Velaclaw assistant setup](/start/velaclaw) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    velaclaw message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Velaclaw to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `velaclaw pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `velaclaw pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `velaclaw doctor` to surface risky DM policies.
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Velaclaw only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    velaclaw pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    velaclaw pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Velaclaw blocks **cross-provider** messaging by default. If a tool call is bound
...
Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/Zavianx/velaclaw-dev/discussions).
