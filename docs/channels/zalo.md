...
Zalo ships as a bundled plugin in current Velaclaw releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `velaclaw plugins install @velaclaw/zalo`
- Or from a source checkout: `velaclaw plugins install ./path/to/local/zalo-plugin`
...
This page reflects current Velaclaw behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `velaclaw pairing list zalo`
  - `velaclaw pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in Velaclaw.
...
- Example: `velaclaw message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `velaclaw channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `velaclaw logs --follow`
