...
- Onboarding (`velaclaw onboard`) and `velaclaw channels add --channel whatsapp`
  prompt to install the WhatsApp plugin the first time you select it.
- `velaclaw channels login --channel whatsapp` also offers the install flow when
  the plugin is not present yet.
- Dev channel + git checkout: defaults to the local plugin path.
- Stable/Beta: defaults to the npm package `@velaclaw/whatsapp`.

Manual install stays available:

```bash
velaclaw plugins install @velaclaw/whatsapp
```
...
<Note>
Velaclaw recommends running WhatsApp on a separate number when possible. (The channel metadata and setup flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for Velaclaw
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current Velaclaw channel architecture.
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[velaclaw]`
...
<AccordionGroup>
  ...
  <Accordion title="Logout behavior">
    `velaclaw channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
...
<AccordionGroup>
  ...
  <Accordion title="Group messages unexpectedly ignored">
    ...
    - duplicate keys in `velaclaw.json` (JSON5): later entries override earlier ones, so keep a single `groupPolicy` per scope
