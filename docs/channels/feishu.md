...
> **Requires Velaclaw 2026.4.10 or above.** Run `velaclaw --version` to check. Upgrade with `velaclaw update`.
...
```bash
velaclaw pairing list feishu
velaclaw pairing approve feishu <CODE>
```
...
4. Check logs: `velaclaw logs --follow`
...
5. Ensure the gateway is running: `velaclaw gateway status`
6. Check logs: `velaclaw logs --follow`

### App Secret leaked

1. Reset the App Secret in Feishu Open Platform / Lark Developer
2. Update the value in your config
3. Restart the gateway: `velaclaw gateway restart`
