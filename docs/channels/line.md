...
read_when:
  - You want to connect Velaclaw to LINE
...
LINE connects to Velaclaw via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current Velaclaw releases, so normal
...
```bash
velaclaw plugins install @velaclaw/line
```

Local checkout (when running from a git repo):

```bash
velaclaw plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Velaclaw applies strict pre-auth body limits and timeout before verification.
- Velaclaw processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
velaclaw pairing list line
velaclaw pairing approve line <CODE>
```
