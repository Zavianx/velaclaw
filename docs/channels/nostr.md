...
read_when:
  - You want Velaclaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Velaclaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Velaclaw releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`velaclaw onboard`) and `velaclaw channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
velaclaw plugins install @velaclaw/nostr
```

Use a local checkout (dev workflows):

```bash
velaclaw plugins install --link <path-to-local-nostr-plugin>
```
...
velaclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "velaclaw",
        displayName: "Velaclaw",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "velaclaw@example.com",
        lud16: "velaclaw@example.com",
      },
    },
  },
}
