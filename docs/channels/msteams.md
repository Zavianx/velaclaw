...
Microsoft Teams ships as a bundled plugin in current Velaclaw releases, so no
...
```bash
velaclaw plugins install @velaclaw/msteams
```

Local checkout (when running from a git repo):

```bash
velaclaw plugins install ./path/to/local/msteams-plugin
```
...
3. Configure Velaclaw with those credentials.
...
- Talk to Velaclaw via Teams DMs, group chats, or channels.
...
- On startup, Velaclaw resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.Zavianx/velaclaw-dev.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Velaclaw, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `velaclaw-msteams` (must be unique) |
...
For production deployments, Velaclaw supports **federated authentication** as a more secure alternative to client secrets. Two methods are available:
...
3. At runtime, Velaclaw uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint (`169.254.169.254`).
...
**Default behavior:** When `authType` is not set, Velaclaw defaults to client secret authentication. Existing configurations continue to work without changes.
...
1. **Ensure the Microsoft Teams plugin is available**
   ...
   - Older/custom installs can add it manually:
     - From npm: `velaclaw plugins install @velaclaw/msteams`
     - From a local checkout: `velaclaw plugins install ./path/to/local/msteams-plugin`
...
Velaclaw exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "Velaclaw in Teams", full: "Velaclaw in Teams" },
...
Velaclaw handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Velaclaw only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/VelaclawShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Velaclaw sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `velaclaw message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.velaclaw/msteams-polls.json`.
...
```bash
velaclaw message send --channel msteams \
...
velaclaw message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
velaclaw message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
velaclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
velaclaw message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
