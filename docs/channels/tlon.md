...
Tlon is a decentralized messenger built on Urbit. Velaclaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Velaclaw releases, so normal packaged
...
```bash
velaclaw plugins install @velaclaw/tlon
```

Local checkout (when running from a git repo):

```bash
velaclaw plugins install ./path/to/local/tlon-plugin
```
...
By default, Velaclaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `velaclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Velaclaw replies in-thread.
