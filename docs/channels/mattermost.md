---
summary: "Mattermost bot setup and Velaclaw config"
read_when:
...
Mattermost ships as a bundled plugin in current Velaclaw releases, so normal
...
```bash
velaclaw plugins install @velaclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
velaclaw plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Velaclaw and start the gateway.
...
Native slash commands are opt-in. When enabled, Velaclaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Velaclaw derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when Velaclaw registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Velaclaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Velaclaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Velaclaw, not `404`.
...
- Approve via:
  - `velaclaw pairing list mattermost`
  - `velaclaw pairing approve mattermost <CODE>`
...
Use these target formats with `velaclaw message send` or cron/webhooks:
...
Velaclaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Velaclaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Velaclaw sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Velaclaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Velaclaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="velaclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"velaclaw-mattermost-interactions",
...
`velaclaw message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: Velaclaw
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as Velaclaw. Set an
