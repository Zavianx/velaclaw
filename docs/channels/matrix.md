...
Matrix is a bundled channel plugin for Velaclaw.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Bundled plugin

Matrix ships as a bundled plugin in current Velaclaw releases, so normal
...
```bash
velaclaw plugins install @velaclaw/matrix
```

Install from a local checkout:

```bash
velaclaw plugins install ./path/to/local/matrix-plugin
```
...
1. Ensure the Matrix plugin is available.
   - Current packaged Velaclaw releases already bundle it.
...
```bash
velaclaw channels add
velaclaw configure --section channels
```
...
- To resolve room names before saving, use `velaclaw channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "Velaclaw Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.velaclaw/credentials/matrix/`.
The default account uses `credentials.json`; named accounts use `credentials-<account>.json`.
When cached credentials exist there, Velaclaw treats Matrix as configured for setup, doctor, and channel-status discovery even if current auth is not set directly in config.
...
`autoJoin` applies to all Matrix invites, including DM-style invites. Velaclaw cannot reliably
...
Set `channels.matrix.streaming` to `"partial"` when you want Velaclaw to send a single live preview
...
- `streaming: "off"` is the default. Velaclaw waits for the final reply and sends it once.
...
- If the preview no longer fits in one Matrix event, Velaclaw stops preview streaming and falls back to normal final delivery.
- Media replies still send attachments normally. If a stale preview can no longer be reused safely, Velaclaw redacts it before sending the final media reply.
...
- bot user = the Velaclaw Matrix account that sends the reply
...
1. Configure Velaclaw to use quiet previews:
...
Velaclaw marks finalized text-only preview edits with:

```json
{
  "com.velaclaw.finalized_preview": true
}
```
...
```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/velaclaw-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.velaclaw\\.finalized_preview",
        "value": true
      },
...
- `velaclaw-finalized-preview-botname`: a rule ID unique to this bot for this receiving user
- `@bot:example.org`: your Velaclaw Matrix bot MXID, not the receiving user's MXID
...
- If one receiving user should notify for multiple Velaclaw Matrix bot accounts, create one rule per bot with a unique rule ID for each sender match.
- A simple pattern is `velaclaw-finalized-preview-<botname>`, such as `velaclaw-finalized-preview-ops` or `velaclaw-finalized-preview-support`.

The rule is evaluated against the event sender:

- authenticate with the receiving user's token
- match `sender` against the Velaclaw bot MXID
...
- This only affects text-only preview edits that Velaclaw can safely finalize in place. Media fallbacks and stale-preview fallbacks still use normal Matrix delivery.
...
- No special `homeserver.yaml` change is required for finalized Velaclaw preview notifications.
...
By default, Matrix messages from other configured Velaclaw Matrix accounts are ignored.
...
- Velaclaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; Velaclaw treats "bot-authored" as "sent by another configured Matrix account on this Velaclaw gateway".
...
```bash
velaclaw matrix verify status --include-recovery-key --json
```
...
```bash
velaclaw matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
velaclaw matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
velaclaw matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
velaclaw matrix verify backup restore
```

Verbose restore diagnostics:

```bash
velaclaw matrix verify backup restore --verbose
```
...
```bash
velaclaw matrix verify status --account assistant
velaclaw matrix verify backup restore --account assistant
velaclaw matrix devices list --account assistant
```
...
Velaclaw treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `velaclaw matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for Velaclaw to treat the device as fully verified.

### What bootstrap does

`velaclaw matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, Velaclaw tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
backup baseline for future messages, use `velaclaw matrix verify backup reset --yes`.
Do this only when you accept that unrecoverable old encrypted history will stay
unavailable and that Velaclaw may recreate secret storage if the current backup
...
If startup still finds broken bootstrap state, Velaclaw can attempt a guarded repair path even when `channels.matrix.password` is not configured.
If the homeserver requires password-based UIA for that repair, Velaclaw logs a warning and keeps startup non-fatal instead of aborting the bot.
If the current device is already owner-signed, Velaclaw preserves that identity instead of resetting it automatically.
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by Velaclaw.
For self-verification flows, Velaclaw also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, Velaclaw auto-accepts the request and then waits for the SAS flow to proceed normally.
...
Velaclaw does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old Velaclaw-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
velaclaw matrix devices list
```

Remove stale Velaclaw-managed devices with:

```bash
velaclaw matrix devices prune-stale
```
...
`~/.velaclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, Velaclaw reuses the best existing
...
```bash
velaclaw matrix profile set --name "Velaclaw Assistant"
velaclaw matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

Add `--account <id>` when you want to target a named Matrix account explicitly.

Matrix accepts `mxc://` avatar URLs directly. When you pass an `http://` or `https://` avatar URL, Velaclaw uploads it to Matrix first and stores the resolved `mxc://` URL back into `channels.matrix.avatarUrl` (or the selected account override).
...
- Same-session DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise Velaclaw falls back to normal user-scoped routing.
- When Velaclaw sees a Matrix DM room collide with another DM room on the same shared Matrix DM session, it posts a one-time `m.notice` in that room with the `/focus` escape hatch when thread bindings are enabled and the `dm.sessionScope` hint.
...
- `threadBindings.spawnAcpSessions` is only required for `/acp spawn --thread auto|here`, where Velaclaw needs to create or bind a child Matrix thread.
...
Ack reactions use the standard Velaclaw resolution order:
...
- Matrix room history is pending-only: Velaclaw buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
velaclaw pairing list matrix
velaclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, Velaclaw reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state gets out of sync, Velaclaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
velaclaw matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
velaclaw matrix direct repair --user-id @alice:example.org
```
...
Partial shared auth defaults do not create a separate implicit default account by themselves. Velaclaw only synthesizes the top-level `default` account when that default has fresh auth (`homeserver` plus `accessToken`, or `homeserver` plus `userId` and `password`); named accounts can still stay discoverable from `homeserver` plus `userId` when cached credentials satisfy auth later.
...
Set `defaultAccount` when you want Velaclaw to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If multiple Matrix accounts are configured and one account id is `default`, Velaclaw uses that account implicitly even when `defaultAccount` is unset.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `velaclaw matrix verify ...` and `velaclaw matrix devices ...` when you want to override that implicit selection for one command.
...
By default, Velaclaw blocks private/internal Matrix homeservers for SSRF protection unless you
...
Velaclaw uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere Velaclaw asks you for a room or user target:
...
- `allowBots`: allow messages from other configured Velaclaw Matrix accounts (`true` or `"mentions"`).
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; Velaclaw does not trust alias state claimed by the invited room.
- `dm`: DM policy block (`enabled`, `policy`, `allowFrom`, `sessionScope`, `threadReplies`).
- `dm.policy`: controls DM access after Velaclaw has joined the room and classified it as a DM. It does not change whether an invite is auto-joined.
