...
Current Velaclaw releases bundle BlueBubbles, so normal packaged builds do not
need a separate `velaclaw plugins install` step.

## Overview

- Runs on macOS via the BlueBubbles helper app ([bluebubbles.app](https://bluebubbles.app)).
- Recommended/tested: macOS Sequoia (15). macOS Tahoe (26) works; edit is currently broken on Tahoe, and group icon updates may report success but not sync.
- Velaclaw talks to it through its REST API (`GET /api/v1/ping`, `POST /message/text`, `POST /chat/:id/*`).
...
3. Run `velaclaw onboard` and select BlueBubbles, or configure manually:
...
- Webhook authentication is always required. Velaclaw rejects BlueBubbles webhook requests unless they include a password/guid that matches `channels.bluebubbles.password` (for example `?password=<password>` or `x-password`), regardless of loopback/proxy topology.
...
```
velaclaw channels add bluebubbles --http-url http://192.168.1.100:1234 --password <password>
```
...
- Approve via:
  - `velaclaw pairing list bluebubbles`
  - `velaclaw pairing approve bluebubbles <CODE>`
...
- **Typing indicators**: Velaclaw sends typing start events; BlueBubbles clears typing automatically on send or timeout (manual stop via DELETE is unreliable).
...
Velaclaw may surface _short_ message IDs (e.g., `1`, `2`) to save tokens.
...
- Direct handles: `+15555550123`, `user@example.com`
  - If a direct handle does not have an existing DM chat, Velaclaw will create one via `POST /api/v1/chat/new`. This requires the BlueBubbles Private API to be enabled.
...
- Pairing codes expire after one hour; use `velaclaw pairing list bluebubbles` and `velaclaw pairing approve bluebubbles <code>`.
- Reactions require the BlueBubbles private API (`POST /api/v1/message/react`); ensure the server version exposes it.
- Edit/unsend require macOS 13+ and a compatible BlueBubbles server version. On macOS 26 (Tahoe), edit is currently broken due to private API changes.
- Group icon updates can be flaky on macOS 26 (Tahoe): the API may return success but the new icon does not sync.
- Velaclaw auto-hides known-broken actions based on the BlueBubbles server's macOS version. If edit still appears on macOS 26 (Tahoe), disable it manually with `channels.bluebubbles.actions.edit=false`.
- For status/health info: `velaclaw status --all` or `velaclaw status --deep`.
