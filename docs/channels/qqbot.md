...
read_when:
  - You want to connect Velaclaw to QQ
...
QQ Bot connects to Velaclaw via the official QQ Bot API (WebSocket gateway). The
...
Current Velaclaw releases bundle QQ Bot, so normal packaged builds do not need
a separate `velaclaw plugins install` step.
...
```bash
velaclaw channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `velaclaw channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Velaclaw instance:
...
```bash
velaclaw channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
| `/bot-version` | Show the Velaclaw framework version  |
