...
read_when:
  - Setting up Zalo Personal for Velaclaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Velaclaw.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current Velaclaw releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `velaclaw plugins install @velaclaw/zalouser`
- Or from a source checkout: `velaclaw plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `velaclaw channels login --channel zalouser`
...
```bash
velaclaw directory self --channel zalouser
velaclaw directory peers list --channel zalouser --query "name"
velaclaw directory groups list --channel zalouser --query "work"
```
...
- `velaclaw pairing list zalouser`
- `velaclaw pairing approve zalouser <code>`
...
- On startup, Velaclaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Velaclaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Velaclaw state. Example:
...
- Velaclaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Velaclaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `velaclaw channels logout --channel zalouser && velaclaw channels login --channel zalouser`
...
- The channel now runs fully in Velaclaw without external CLI binaries.
