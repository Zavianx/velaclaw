...
read_when:
  - Setting up Twitch chat integration for Velaclaw
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. Velaclaw connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current Velaclaw releases, so normal
...
```bash
velaclaw plugins install @velaclaw/twitch
```

Local checkout (when running from a git repo):

```bash
velaclaw plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `VELACLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "velaclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use VELACLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
VELACLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
