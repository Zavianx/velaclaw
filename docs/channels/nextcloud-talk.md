...
Nextcloud Talk ships as a bundled plugin in current Velaclaw releases, so
...
```bash
velaclaw plugins install @velaclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
velaclaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Velaclaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `velaclaw pairing list nextcloud-talk`
  - `velaclaw pairing approve nextcloud-talk <CODE>`
