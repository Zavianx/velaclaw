...
read_when:
  ...
  - Reviewing Velaclaw security posture
title: "Pairing"
---

# Pairing

“Pairing” is Velaclaw’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `velaclaw-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.velaclaw/credentials/`:
...
3. On your phone, open the Velaclaw iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `velaclaw nodes pending|approve|reject|rename`) is a
