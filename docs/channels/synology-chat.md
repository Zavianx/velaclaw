---
summary: "Synology Chat webhook setup and Velaclaw config"
read_when:
  - Setting up Synology Chat with Velaclaw
...
Synology Chat ships as a bundled plugin in current Velaclaw releases, so normal
...
```bash
velaclaw plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `velaclaw onboard` now shows Synology Chat in the same channel setup list as `velaclaw channels add`.
   - Non-interactive setup: `velaclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Velaclaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Velaclaw.
   - Guided: `velaclaw onboard`
   - Direct: `velaclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Velaclaw accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `velaclaw pairing list synology-chat`
  - `velaclaw pairing approve synology-chat <CODE>`
...
```bash
velaclaw message send --channel synology-chat --target 123456 --text "Hello from Velaclaw"
velaclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Velaclaw now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Velaclaw
- `Rate limit exceeded`:
