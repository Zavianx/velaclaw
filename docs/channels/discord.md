...
You will need to create a new application with a bot, add the bot to your server, and pair it to Velaclaw. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "Velaclaw".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your Velaclaw agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to Velaclaw in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with Velaclaw. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running Velaclaw before messaging your agent.

```bash
export DISCORD_BOT_TOKEN="YOUR_BOT_TOKEN"
velaclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
velaclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
velaclaw config set channels.discord.enabled true --strict-json
velaclaw gateway
```

    If Velaclaw is already running as a background service, restart it via the Velaclaw Mac app or by stopping and restarting the `velaclaw gateway run` process.

  </Step>

  <Step title="Configure Velaclaw and pair">
...
```bash
velaclaw pairing list discord
velaclaw pairing approve discord <CODE>
```
...
Discord forum and media channels only accept thread posts. Velaclaw supports two ways to create them:
...
- Use `velaclaw message thread create` to create a thread directly. Do not pass `--message-id` for forum channels.

Example: send to forum parent to create a thread

```bash
velaclaw message send --channel discord --target channel:<forumId> \
  --message "Topic title\nBody of the post"
```
...
```bash
velaclaw message thread create --channel discord --target channel:<forumId> \
...
Velaclaw supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- Velaclaw adds a trigger button automatically
...
<Tabs>
  ...
  <Tab title="Guild policy">
    ...
    - names/tags are supported for `users`, but IDs are safer; `velaclaw security audit` warns when name/tag entries are used
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in Velaclaw config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces Velaclaw auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    Velaclaw can stream draft replies by sending a temporary message and editing it as text arrives.
...
```
    ...
    enabled for Discord, Velaclaw skips the preview stream to avoid double streaming.
...
```
    ...
    - `spawnAcpSessions` is only required when Velaclaw needs to create/bind a child thread via `--thread auto|here`. It is not required for `/acp spawn ... --bind here` in the current channel.
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while Velaclaw is processing an inbound message.
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/velaclaw",
    },
  },
}
```
...
```
  ...
  <Accordion title="Approvals in Discord">
    ...
    When those buttons are present, they are the primary approval UX; Velaclaw
    ...
    - env-first local auth (`VELACLAW_GATEWAY_TOKEN` / `VELACLAW_GATEWAY_PASSWORD` then `gateway.auth.*`)
...
Velaclaw uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires constructing a component payload via the discord tool), while legacy `embeds` remain available but are not recommended.
...
Velaclaw can join Discord voice channels for realtime, continuous conversations. This is separate from voice message attachments.
...
- Velaclaw also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. Velaclaw generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; Velaclaw converts to OGG/Opus when needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep Velaclaw current (`velaclaw update`) so the Discord voice receive recovery logic is present
...
- If command deploy/state is stale, restart gateway and re-check with `velaclaw channels status --probe`.
