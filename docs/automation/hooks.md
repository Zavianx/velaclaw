...
Hooks are small scripts that run when something happens inside the Gateway. They are automatically discovered from directories and can be inspected with `velaclaw hooks`.
...
- **Webhooks**: external HTTP endpoints that let other systems trigger work in Velaclaw. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `velaclaw hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "velaclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
**Metadata fields** (`metadata.velaclaw`):
...
1. **Bundled hooks**: shipped with Velaclaw
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.velaclaw/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
Hook packs are npm packages that export hooks via `velaclaw.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.velaclaw/logs/commands.log`  |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
velaclaw hooks enable <hook-name>
```
...
# Enable/disable
velaclaw hooks enable <hook-name>
velaclaw hooks disable <hook-name>
```
...
1. Verify the hook is enabled: `velaclaw hooks list`
