...
- `velaclaw tasks list` shows all tasks; `velaclaw tasks audit` surfaces issues.
...
velaclaw tasks list --runtime acp
velaclaw tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
velaclaw tasks show <lookup>

# Cancel a running task (kills the child session)
velaclaw tasks cancel <lookup>

# Change notification policy for a task
velaclaw tasks notify <lookup> state_changes

# Run a health audit
velaclaw tasks audit

# Preview or apply maintenance
velaclaw tasks maintenance
velaclaw tasks maintenance --apply

# Inspect TaskFlow state
velaclaw tasks flow list
...
| CLI operations         | `cli`        | `velaclaw agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `velaclaw tasks cancel`                        |
...
When a task reaches a terminal state, Velaclaw notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, Velaclaw also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
velaclaw tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
velaclaw tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
velaclaw tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `velaclaw status` when issues are detected.
...
```bash
velaclaw tasks maintenance [--json]
velaclaw tasks maintenance --apply [--json]
```
...
```bash
velaclaw tasks flow list [--status <status>] [--json]
velaclaw tasks flow show <lookup> [--json]
velaclaw tasks flow cancel <lookup>
```
...
For the full operator ledger, use the CLI: `velaclaw tasks list`.

## Status integration (task pressure)

`velaclaw status` includes an at-a-glance task summary:
...
```
$VELACLAW_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `velaclaw tasks` to inspect individual task records and `velaclaw tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.velaclaw/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
