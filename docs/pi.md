...
summary: "Architecture of Velaclaw's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Velaclaw
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Velaclaw integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Velaclaw uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Velaclaw directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Velaclaw's local TUI mode)                                             |
...
├── pi-tools.ts                    # createVelaclawCodingTools()
...
├── velaclaw-tools.ts              # Velaclaw-specific tools
...
Image injection is prompt-local: Velaclaw loads image refs from the current prompt and
...
2. **Custom Replacements**: Velaclaw replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Velaclaw Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Velaclaw's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Velaclaw CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Velaclaw wraps this with `guardSessionManager()` for tool result safety.
...
Velaclaw maintains an auth profile store with multiple API keys per provider:
...
Velaclaw loads custom pi extensions for specialized behavior:
...
Velaclaw also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Velaclaw Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Velaclaw tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.velaclaw/agents/<agentId>/sessions/` (or `$VELACLAW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `VELACLAW_LIVE_TEST=1`)
