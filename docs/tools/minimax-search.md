...
Velaclaw supports MiniMax as a `web_search` provider through the MiniMax
...
Velaclaw also accepts `MINIMAX_CODING_API_KEY` as an env alias. `MINIMAX_API_KEY`
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Velaclaw resolves
...
- `query`
- `count` (Velaclaw trims the returned result list to the requested count)
