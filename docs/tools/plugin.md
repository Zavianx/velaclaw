---
summary: "Install, configure, and manage Velaclaw plugins"
read_when:
...
Plugins extend Velaclaw with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with Velaclaw), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ```bash
    # From npm
    velaclaw plugins install @velaclaw/voice-call

    # From a local directory or archive
    velaclaw plugins install ./my-plugin
    velaclaw plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@velaclaw/voice-call
...
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `velaclaw.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to Velaclaw features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `velaclaw plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
watch + in-process restart enabled (the default `velaclaw gateway` path), that
...
Velaclaw scans for plugins in this order (first match wins):
...
```bash
velaclaw plugins list                       # compact inventory
velaclaw plugins list --enabled            # only loaded plugins
velaclaw plugins list --verbose            # per-plugin detail lines
velaclaw plugins list --json               # machine-readable inventory
velaclaw plugins inspect <id>              # deep detail
velaclaw plugins inspect <id> --json       # machine-readable
velaclaw plugins inspect --all             # fleet-wide table
velaclaw plugins info <id>                 # inspect alias
velaclaw plugins doctor                    # diagnostics

velaclaw plugins install <package>         # install (ClawHub first, then npm)
velaclaw plugins install clawhub:<pkg>     # install from ClawHub only
velaclaw plugins install <spec> --force    # overwrite existing install
velaclaw plugins install <path>            # install from local path
velaclaw plugins install -l <path>         # link (no copy) for dev
velaclaw plugins install <plugin> --marketplace <source>
velaclaw plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
velaclaw plugins install <spec> --pin      # record exact resolved npm spec
velaclaw plugins install <spec> --dangerously-force-unsafe-install
velaclaw plugins update <id>             # update one plugin
velaclaw plugins update <id> --dangerously-force-unsafe-install
velaclaw plugins update --all            # update all
velaclaw plugins uninstall <id>          # remove config/install records
...
Bundled plugins ship with Velaclaw. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `velaclaw plugins enable <id>`.
...
See [`velaclaw plugins` CLI reference](/cli/plugins) for full details.
...
Velaclaw loads the entry object and calls `register(api)` during plugin
