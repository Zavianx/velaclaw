...
- an Ollama host that is reachable from Velaclaw
- `ollama signin`
...
If no explicit Ollama base URL is set, Velaclaw uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Velaclaw reuses
...
- If the Ollama host is auth-protected, Velaclaw reuses the normal Ollama
  provider API key when present.
- Velaclaw warns during setup if Ollama is unreachable or not signed in, but
