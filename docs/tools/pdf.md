...
The tool is only registered when Velaclaw can resolve a PDF-capable model config for the agent:
...
- The fallback chain is auth-aware. A configured `provider/model` only counts if
  Velaclaw can actually authenticate that provider for the agent.
...
- If text extraction succeeds but image extraction would require vision on a
  text-only model, Velaclaw drops the rendered images and continues with the
  extracted text.
