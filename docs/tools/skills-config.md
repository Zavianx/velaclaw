...
`~/.Zavianx/velaclaw-dev.json`. Agent-specific skill visibility lives under
...
- Built-in skill roots always include `~/.velaclaw/skills`, `~/.agents/skills`,
...
- `install.nodeManager`: node installer preference (`npm` | `pnpm` | `yarn` | `bun`, default: npm).
  ...
  - `velaclaw setup --node-manager` is narrower and currently accepts `npm`,
...
- Keys under `entries` map to the skill name by default. If a skill defines
  `metadata.velaclaw.skillKey`, use that key instead.
- Load precedence is `<workspace>/skills` → `<workspace>/.agents/skills` →
  `~/.agents/skills` → `~/.velaclaw/skills` → bundled skills →
