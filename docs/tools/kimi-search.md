...
Velaclaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Velaclaw reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.velaclaw/.env`.

If you omit `baseUrl`, Velaclaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Velaclaw defaults to `kimi-k2.5`.
