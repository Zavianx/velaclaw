...
read_when:
  - Running Velaclaw Gateway in WSL2 while Chrome lives on Windows
...
- Velaclaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/Zavianx/velaclaw-dev/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Velaclaw and Chrome are on the same machine
...
- Velaclaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Velaclaw is not the problem yet.
...
Fix that before touching Velaclaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Velaclaw at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Velaclaw to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Velaclaw to succeed
...
```bash
velaclaw browser open https://example.com --browser-profile remote
velaclaw browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `velaclaw browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `velaclaw browser stop --browser-profile remote`
...
3. Velaclaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Velaclaw config or Control UI auth
