...
Velaclaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Velaclaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `velaclaw configure --section web`. It stores the key in
`~/.Zavianx/velaclaw-dev.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.velaclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
