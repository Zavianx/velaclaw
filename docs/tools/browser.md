...
read_when:
  ...
  - Debugging why velaclaw is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Velaclaw-managed)"
---

# Browser (velaclaw-managed)

Velaclaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `velaclaw` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **velaclaw** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`velaclaw`, `work`, `remote`, ...).
...
```bash
velaclaw browser --browser-profile velaclaw status
...
If `velaclaw browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`velaclaw browser`),
...
If `velaclaw browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "velaclaw"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.Zavianx/velaclaw-dev.json`.
...
- If you override the Gateway port (`gateway.port` or `VELACLAW_GATEWAY_PORT`),
...
- Default profile is `velaclaw` (Velaclaw-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `velaclaw` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
Velaclaw uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
velaclaw config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Velaclaw will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `velaclaw browser stop` stops the browser process that
  Velaclaw launched
- attach-only and remote CDP profiles: `velaclaw browser stop` closes the active
  ...
  though no browser process was launched by Velaclaw
...
Velaclaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Velaclaw can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Velaclaw treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Velaclaw can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Velaclaw
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Velaclaw supports both:

- **HTTP(S) endpoints** — Velaclaw calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — Velaclaw connects directly,
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-velaclaw-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Velaclaw
  auto-generates `gateway.auth.token` on startup and persists it to config.
- Velaclaw does **not** auto-generate that token when `gateway.auth.mode` is
...
Velaclaw supports multiple named profiles (routing configs). Profiles can be:

- **velaclaw-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `velaclaw` profile is auto-created if missing.
...
Velaclaw can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Velaclaw attaches.
...
```bash
velaclaw browser --browser-profile user start
velaclaw browser --browser-profile user status
velaclaw browser --browser-profile user tabs
velaclaw browser --browser-profile user snapshot --format ai
```
...
- `velaclaw doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `velaclaw` profile because it can
  act inside your signed-in browser session.
- Velaclaw does not launch the browser for this driver; it attaches to an
  existing session only.
- Velaclaw uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, Velaclaw passes it through to target that explicit
...
When launching locally, Velaclaw picks the first available:
...
- `x-velaclaw-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `velaclaw` browser when a per-tab CDP
...
```bash
docker compose run --rm velaclaw-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`VELACLAW_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `velaclaw browser snapshot --interactive --compact --depth 6`
- `velaclaw browser snapshot --efficient`
- `velaclaw browser snapshot --labels`
- `velaclaw browser snapshot --selector "#main" --interactive`
- `velaclaw browser snapshot --frame "iframe#main" --interactive`
- `velaclaw browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `velaclaw browser stop` is still the
...
- `velaclaw browser requests --filter api --clear`
- `velaclaw browser pdf`
- `velaclaw browser responsebody "**/api" --max-chars 5000`
...
- `velaclaw browser storage local set theme dark`
- `velaclaw browser storage session clear`
- `velaclaw browser set offline on`
- `velaclaw browser set headers --headers-json '{"X-Debug":"1"}'`
- `velaclaw browser set credentials user pass`
- `velaclaw browser set credentials --clear`
- `velaclaw browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `velaclaw browser set geo --clear`
- `velaclaw browser set media dark`
- `velaclaw browser set timezone America/New_York`
- `velaclaw browser set locale en-US`
...
- Download and trace output paths are constrained to Velaclaw temp roots:
  - traces: `/tmp/velaclaw` (fallback: `${os.tmpdir()}/velaclaw`)
  - downloads: `/tmp/velaclaw/downloads` (fallback: `${os.tmpdir()}/velaclaw/downloads`)
- Upload paths are constrained to an Velaclaw temp uploads root:
  - uploads: `/tmp/velaclaw/uploads` (fallback: `${os.tmpdir()}/velaclaw/uploads`)
...
Velaclaw supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `velaclaw browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `velaclaw browser click 12`, `velaclaw browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `velaclaw browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `velaclaw browser click e12`, `velaclaw browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `velaclaw browser wait --url "**/dash"`
- Wait for load state:
  - `velaclaw browser wait --load networkidle`
- Wait for a JS predicate:
  - `velaclaw browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `velaclaw browser wait "#main"`
...
1. `velaclaw browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `velaclaw browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `velaclaw browser trace start`
   - reproduce the issue
   - `velaclaw browser trace stop` (prints `TRACE:<path>`)
...
```bash
velaclaw browser status --json
velaclaw browser snapshot --interactive --json
velaclaw browser requests --filter api --json
velaclaw browser cookies --json
```
...
- The velaclaw browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `velaclaw browser evaluate` and `wait --fn`
...
- **CDP startup or readiness failure** means Velaclaw cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "velaclaw" is not reachable after start`
...
- For the local loopback `velaclaw` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Velaclaw's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (velaclaw, chrome, or remote CDP).
