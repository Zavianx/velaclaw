...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.velaclaw/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Velaclaw picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    velaclaw agent --message "give me a greeting"
    ```
...
| `metadata.velaclaw.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.velaclaw.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.velaclaw.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `velaclaw agent --message "..."` to test before sharing
...
| `~/.velaclaw/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Velaclaw) | Low        | Global                |
