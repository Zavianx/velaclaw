...
Velaclaw can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere Velaclaw can send audio.
...
When `messages.tts.provider` is unset, Velaclaw picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `velaclaw.json`.
...
- If `provider` is **unset**, Velaclaw uses the first configured speech provider in registry auto-select order.
...
`~/.velaclaw/settings/tts.json`, override with `VELACLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Velaclaw retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, Velaclaw:
...
Discord note: `/tts` is a built-in Discord command, so Velaclaw registers
