---
summary: "ClawHub guide: public registry, native Velaclaw install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Velaclaw skills and plugins**.

- Use native `velaclaw` commands to search/install/update skills and install
...
```bash
velaclaw plugins install clawhub:<package>
...
Native `velaclaw` commands install into your active workspace and persist source
...
4. Users browse, download, and install skills in Velaclaw.
...
If you want to add new capabilities to your Velaclaw agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `velaclaw skills install <skill-slug>`
3. Start a new Velaclaw session so it picks up the new skill.
...
A skill is a versioned bundle of files that teaches Velaclaw how to perform a
...
Interested in becoming a moderator? Ask in the Velaclaw Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Velaclaw workspace).
...
Code plugins must include the required Velaclaw metadata in `package.json`:
