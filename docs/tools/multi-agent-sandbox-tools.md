...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `velaclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `velaclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=velaclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${VELACLAW_STATE_DIR:-$HOME/.velaclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
