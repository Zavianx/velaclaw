...
- `commands.config` (default `false`) enables `/config` (reads/writes `velaclaw.json`).
- `commands.mcp` (default `false`) enables `/mcp` (reads/writes Velaclaw-managed MCP config under `mcp.servers`).
...
- `/claw create|use|leave|list|current|edit|remove` manages named claws under the current bot.
...
- `/config show|get|set|unset` reads or writes `velaclaw.json`. Owner-only. Requires `commands.config: true`.
- `/mcp show|get|set|unset` reads or writes Velaclaw-managed MCP server config under `mcp.servers`. Owner-only. Requires `commands.mcp: true`.
...
- `/restart` restarts Velaclaw when enabled. Default: enabled; set `commands.restart: false` to disable it.
...
- `/claw create <name> [role text]` creates an isolated named claw and activates it for the current sender/conversation. `/new claw <name> ...` and `/claw new <name> ...` are aliases. Use `/claw leave` or `/claw use main` to return to the main claw. Use `/claw remove <name> [--purge]` to delete a claw; `--purge` also removes its stored session. Tmux-style forms such as `new-session -s`, `attach-session -t`, `list-sessions`, and `kill-session -t` remain accepted as compatibility aliases but are not the recommended UI.
- For full provider usage breakdown, use `velaclaw status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Velaclaw session logs.
- `/restart` is enabled by default; set `commands.restart: false` to disable it.
- `/plugins install <spec>` accepts the same plugin specs as `velaclaw plugins install`: local path/archive, npm package, or `clawhub:<pkg>`.
...
- If a run is already active, Velaclaw marks a live switch as pending and only restarts into the new model at a clean retry point.
...
- **Provider usage/quota** (example: “Claude 80% left”) shows up in `/status` for the current model provider when usage tracking is enabled. Velaclaw normalizes provider windows to `% left`; for MiniMax, remaining-only percent fields are inverted before display, and `model_remains` responses prefer the chat-model entry plus a model-tagged plan label.
...
```
/debug show
/debug set messages.responsePrefix="[velaclaw]"
...
- Overrides apply immediately to new config reads, but do **not** write to `velaclaw.json`.
...
`/config` writes to your on-disk config (`velaclaw.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[velaclaw]"
...
`/mcp` writes Velaclaw-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
- `/mcp` stores config in Velaclaw config, not Pi-owned project settings.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/velaclaw`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
