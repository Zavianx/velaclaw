...
`ask: "always"` in `~/.velaclaw/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `velaclaw approvals get`, `velaclaw approvals get --gateway`, or
`velaclaw approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `velaclaw exec-policy show` exposes the same merged view and
`velaclaw exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`velaclaw exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `velaclaw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Velaclaw also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.velaclaw/exec-approvals.sock",
...
- requested exec policy in Velaclaw config (`tools.exec.*`)
- host-local approvals policy in `~/.velaclaw/exec-approvals.json`
...
- In YOLO mode, Velaclaw does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
velaclaw config set tools.exec.host gateway
velaclaw config set tools.exec.security full
velaclaw config set tools.exec.ask off
...
```bash
velaclaw approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `velaclaw exec-policy` does not synchronize node approvals
- `velaclaw exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `velaclaw approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, Velaclaw treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.velaclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `velaclaw approvals allowlist ...`).
- `velaclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `velaclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Velaclaw still rejects the `env` builtin in safe-bin
...
`~/.velaclaw/exec-approvals.json` directly.

CLI: `velaclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Velaclaw sends a followup `agent` turn to the same session.
...
topics, Velaclaw preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Velaclaw prevents the agent from reusing
