---
summary: "Velaclaw tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools Velaclaw provides
...
Velaclaw has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). Velaclaw ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Velaclaw), others are **external** (published on npm by the community).
...
These tools ship with Velaclaw and are available without installing any plugins:
...
| `research_task` / `web_search` / `x_search` / `web_fetch` | Structured research, web search, X search, page fetching              | [Web](/tools/web)                           |
...
| `group:web`        | research_task, web_search, x_search, web_fetch                                                            |
...
| `group:velaclaw`   | All built-in Velaclaw tools (excludes plugin tools)                                                       |
