...
For shared provider-backed agent sessions, Velaclaw starts music generation as a
...
Not all providers support all parameters. Velaclaw still validates hard limits
such as input counts before submission. When a provider supports duration but
uses a shorter maximum than the requested value, Velaclaw automatically clamps
...
Tool results report the applied settings. When Velaclaw clamps duration during provider fallback, the returned `durationSeconds` reflects the submitted value and `details.normalization.durationSeconds` shows the requested-to-applied mapping.
...
- Task tracking: use `velaclaw tasks list` or `velaclaw tasks show <taskId>` to inspect queued, running, and terminal status for the generation.
- Completion wake: Velaclaw injects an internal completion event back into the same session so the model can write the user-facing follow-up itself.
...
When generating music, Velaclaw tries providers in this order:
...
```bash
VELACLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
...
```bash
VELACLAW_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts
```
