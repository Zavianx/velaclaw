...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of Velaclaw's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, Velaclaw keeps the normal managed `web_search` behavior.
...
If no `provider` is set, Velaclaw checks providers in this order and uses the
...
- or omit that field and let Velaclaw auto-detect the first ready web-fetch
  provider from available credentials
- today the bundled web-fetch provider is Firecrawl, configured under
  `plugins.entries.firecrawl.config.webFetch.*`

When you choose **Kimi** during `velaclaw onboard` or
...
<Tabs>
  <Tab title="Config file">
    Run `velaclaw configure --section web` or set the key directly:
