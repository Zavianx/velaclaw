---
summary: "Use ACP runtime sessions for Codex, Claude Code, Cursor, Gemini CLI, Velaclaw ACP, and other harness agents"
read_when:
...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Velaclaw run external coding harnesses (for example Pi, Claude Code, Codex, Cursor, Copilot, Velaclaw ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses) through an ACP backend plugin.

If you ask Velaclaw in plain language to "run this in Codex" or "start Claude Code in a thread", Velaclaw should route that request to the ACP runtime (not the native sub-agent runtime). Each ACP session spawn is tracked as a [background task](/automation/tasks).

If you want Codex or Claude Code to connect as an external MCP client directly
to existing Velaclaw channel conversations, use [`velaclaw mcp serve`](/cli/mcp)
instead of ACP.
...
| ---------------------------------------------------------------------------------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Run Codex, Claude Code, Gemini CLI, or another external harness _through_ Velaclaw | This page: ACP agents                 | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls |
| Expose an Velaclaw Gateway session _as_ an ACP server for an editor or client      | [`velaclaw acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to Velaclaw over stdio/WebSocket                                          |
| Reuse a local AI CLI as a text-only fallback model                                 | [CLI Backends](/gateway/cli-backends) | Not ACP. No Velaclaw tools, no ACP controls, no harness runtime                                             |
...
- On startup, Velaclaw probes that binary and self-repairs it if needed.
...
- `/acp spawn codex`: Velaclaw should be ready to bootstrap `acpx`, but the Codex ACP adapter may still need a first-run fetch.
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Velaclaw-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Velaclaw native sub-agent runtime  |
...
1. Velaclaw ACP session control plane
...
- Velaclaw keeps owning the channel transport, auth, safety, and delivery.
...
- If you spawn to a different ACP agent and do not pass `--cwd`, Velaclaw inherits the **target agent's** workspace by default, not the requester's.
- If that inherited workspace path is missing (`ENOENT`/`ENOTDIR`), Velaclaw falls back to the backend default cwd instead of silently reusing the wrong tree.
...
- ACP session: the durable Codex/Claude/Gemini runtime state Velaclaw routes to
...
- `/acp spawn codex --thread auto`: Velaclaw may create a child thread/topic and bind the ACP session there
...
- Generic current-conversation binds use the shared Velaclaw binding store and survive normal gateway restarts.

Notes:

- `--bind here` and `--thread ...` are mutually exclusive on `/acp spawn`.
- On Discord, `--bind here` binds the current channel or thread in place. `spawnAcpSessions` is only required when Velaclaw needs to create a child thread for `--thread auto|here`.
- If the active channel does not expose current-conversation ACP bindings, Velaclaw returns a clear unsupported message.
...
- Velaclaw binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Velaclaw returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning Velaclaw agent id.
...
- Velaclaw ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, Velaclaw inherits the target agent workspace from agent config.
...
- If `agentId` is omitted, Velaclaw uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Velaclaw may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Velaclaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the Velaclaw sandbox.
...
If no target resolves, Velaclaw returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Velaclaw returns a clear unsupported-control error.
...
When Velaclaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Velaclaw `agentId` path).
...
```bash
velaclaw plugins install acpx
velaclaw config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
velaclaw plugins install ./path/to/local/acpx-plugin
```
...
- Relative paths resolve from Velaclaw workspace directory.
...
- Velaclaw startup remains non-blocking while the backend health check runs.

See [Plugins](/tools/plugin).

### Automatic dependency install

When you install Velaclaw globally with `npm install -g velaclaw`, the acpx
...
normally and reports the missing dependency through `velaclaw acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Velaclaw plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Velaclaw plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
velaclaw config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `velaclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Velaclaw
  plugins.
...
```bash
velaclaw config set plugins.entries.acpx.config.timeoutSeconds 180
```
...
These ACPX harness permissions are separate from Velaclaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
velaclaw config set plugins.entries.acpx.config.permissionMode approve-all
velaclaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** Velaclaw currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
