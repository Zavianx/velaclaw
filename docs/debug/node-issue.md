...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Velaclaw
...
Running Velaclaw via Node with `tsx` fails at startup with:

```
[velaclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `velaclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch velaclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node velaclaw.mjs status` works on Node 25.
