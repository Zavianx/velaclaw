...
read_when:
  - You want to place an outbound voice call from Velaclaw
...
Voice calls for Velaclaw via a plugin. Supports outbound notifications and
...
- Use `velaclaw voicecall ...` or the `voice_call` tool
...
velaclaw plugins install "$PLUGIN_SRC"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `velaclaw doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `velaclaw doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `velaclaw doctor --fix`:
...
```bash
velaclaw voicecall call --to "+15555550123" --message "Hello from Velaclaw"
velaclaw voicecall start --to "+15555550123"   # alias for call
velaclaw voicecall continue --call-id <id> --message "Any questions?"
velaclaw voicecall speak --call-id <id> --message "One moment"
velaclaw voicecall end --call-id <id>
velaclaw voicecall status --call-id <id>
velaclaw voicecall tail
velaclaw voicecall latency                     # summarize turn latency from logs
velaclaw voicecall expose --mode funnel
```
