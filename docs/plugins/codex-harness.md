---
title: "Codex Harness"
summary: "Run Velaclaw embedded agent turns through the bundled Codex app-server harness"
read_when:
...
The bundled `codex` plugin lets Velaclaw run embedded agent turns through the
...
Velaclaw still owns chat channels, session files, model selection, tools,
...
force `embeddedHarness.runtime: "codex"` or `VELACLAW_AGENT_RUNTIME=codex`.
...
Velaclaw has separate routes for OpenAI and Codex-shaped access:

| Model ref              | Runtime path                                 | Use when                                                                |
| ---------------------- | -------------------------------------------- | ----------------------------------------------------------------------- |
| `openai/gpt-5.4`       | OpenAI provider through Velaclaw/PI plumbing | You want direct OpenAI Platform API access with `OPENAI_API_KEY`.       |
...
- Velaclaw with the bundled `codex` plugin available.
...
Velaclaw on the protocol surface it has been tested against.
...
With fallback disabled, Velaclaw fails early if the Codex plugin is disabled,
...
Velaclaw session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the Velaclaw session binding for that thread.
...
- `VELACLAW_CODEX_APP_SERVER_BIN`
- `VELACLAW_CODEX_APP_SERVER_ARGS`
- `VELACLAW_CODEX_APP_SERVER_APPROVAL_POLICY`
- `VELACLAW_CODEX_APP_SERVER_SANDBOX`
- `VELACLAW_CODEX_APP_SERVER_GUARDIAN=1`
...
Model switching stays Velaclaw-controlled. When an Velaclaw session is attached
...
generic and works on any channel that supports Velaclaw text commands.
...
- `/codex resume <thread-id>` attaches the current Velaclaw session to an existing Codex thread.
...
normal turns. On the next message, Velaclaw resumes that Codex thread, passes the
currently selected Velaclaw `codex/*` model into app-server, and keeps extended
history enabled.
...
Velaclaw still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal Velaclaw delivery path.

When the selected model uses the Codex harness, native thread compaction is
delegated to Codex app-server. Velaclaw keeps a transcript mirror for channel
...
**Velaclaw falls back to PI:** set `embeddedHarness.fallback: "none"` or
`VELACLAW_AGENT_HARNESS_FALLBACK=none` while testing.
