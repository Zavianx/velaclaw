...
summary: "Step-by-step guide to building a messaging channel plugin for Velaclaw"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Velaclaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Velaclaw to a
...
<Info>
  If you have not built any Velaclaw plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Velaclaw keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `velaclaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `velaclaw/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `velaclaw/plugin-sdk/approval-auth-runtime`
- `velaclaw/plugin-sdk/approval-client-runtime`
- `velaclaw/plugin-sdk/approval-delivery-runtime`
...
- `velaclaw/plugin-sdk/approval-native-runtime`
- `velaclaw/plugin-sdk/approval-reply-runtime`
- `velaclaw/plugin-sdk/channel-runtime-context`

Likewise, prefer `velaclaw/plugin-sdk/setup-runtime`,
`velaclaw/plugin-sdk/setup-adapter-runtime`,
`velaclaw/plugin-sdk/reply-runtime`,
`velaclaw/plugin-sdk/reply-dispatch-runtime`,
`velaclaw/plugin-sdk/reply-reference`, and
`velaclaw/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `velaclaw/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `velaclaw/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `velaclaw/plugin-sdk/channel-setup` covers the optional-install setup
...
- use the broader `velaclaw/plugin-sdk/setup` seam only when you also need the
...
- `velaclaw/plugin-sdk/account-core`,
  `velaclaw/plugin-sdk/account-id`,
  `velaclaw/plugin-sdk/account-resolution`, and
  `velaclaw/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `velaclaw/plugin-sdk/inbound-envelope` and
  `velaclaw/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `velaclaw/plugin-sdk/messaging-targets` for target parsing/matching
- `velaclaw/plugin-sdk/outbound-media` and
  `velaclaw/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates
- `velaclaw/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `velaclaw/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `velaclaw/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `velaclaw/plugin-sdk/channel-inbound` for the shared layer.
...
`velaclaw/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#velaclaw-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/velaclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "velaclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Velaclaw to Acme Chat."
        }
      }
    }
    ```

    ```json velaclaw.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    } from "velaclaw/plugin-sdk/channel-core";
    import type { VelaclawConfig } from "velaclaw/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "velaclaw/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Velaclaw
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "velaclaw/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    Velaclaw loads this instead of the full entry when the channel is disabled
    ...
    `velaclaw/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Velaclaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Velaclaw.
...
├── package.json              # velaclaw.channel metadata
├── velaclaw.plugin.json      # Manifest with config schema
