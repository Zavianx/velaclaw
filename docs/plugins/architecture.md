...
read_when:
  - Building or debugging native Velaclaw plugins
...
This page covers the internal architecture of the Velaclaw plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Velaclaw. Every
native Velaclaw plugin registers against one or more capability types:
...
Velaclaw classifies every loaded plugin into a shape based on its actual
...
Use `velaclaw plugins inspect <id>` to see a plugin's shape and capability
...
When you run `velaclaw doctor` or `velaclaw plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `velaclaw status --all` and `velaclaw plugins doctor`.

## Architecture overview

Velaclaw's plugin system has four layers:

1. **Manifest + discovery**
   Velaclaw finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `velaclaw.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Velaclaw plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Velaclaw reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Velaclaw
...
That split lets Velaclaw validate config, explain missing/disabled plugins, and
...
normal chat actions. Velaclaw keeps one shared `message` tool in core, and
...
Velaclaw treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Velaclaw-facing
  surfaces
...
So if Velaclaw adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Velaclaw has shared
...
```ts
import type { VelaclawPluginDefinition } from "velaclaw/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "velaclaw/plugin-sdk/media-understanding";

const plugin: VelaclawPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Velaclaw already treats image/audio/video understanding as one shared
...
`VelaclawPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Velaclaw can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Velaclaw knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `VelaclawPluginApi` or
  `api.runtime`
...
Native Velaclaw plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Velaclaw process

Compatible bundles are safer by default because Velaclaw currently treats them
...
name: `@velaclaw/<id>` by default, or an approved typed suffix such as
...
Velaclaw exports capabilities, not implementation convenience.
...
At startup, Velaclaw does roughly this:
...
The manifest is the control-plane source of truth. Velaclaw uses it to:
...
Velaclaw keeps short in-process caches for:
...
- Set `VELACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `VELACLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `VELACLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `VELACLAW_PLUGIN_MANIFEST_CACHE_MS`.
...
Velaclaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Velaclaw calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | Velaclaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on Velaclaw's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Velaclaw's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Velaclaw adds those
...
- Velaclaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Velaclaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-velaclaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-velaclaw-scopes` only when the header is explicitly present
  - if `x-velaclaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-velaclaw-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `velaclaw/plugin-sdk` import when
authoring plugins:

- `velaclaw/plugin-sdk/plugin-entry` for plugin registration primitives.
- `velaclaw/plugin-sdk/core` for the generic shared plugin-facing contract.
- `velaclaw/plugin-sdk/config-schema` for the root `velaclaw.json` Zod schema
  export (`VelaclawSchema`).
- Stable channel primitives such as `velaclaw/plugin-sdk/channel-setup`,
  `velaclaw/plugin-sdk/setup-runtime`,
  `velaclaw/plugin-sdk/setup-adapter-runtime`,
  `velaclaw/plugin-sdk/setup-tools`,
  `velaclaw/plugin-sdk/channel-pairing`,
  `velaclaw/plugin-sdk/channel-contract`,
  `velaclaw/plugin-sdk/channel-feedback`,
  `velaclaw/plugin-sdk/channel-inbound`,
  `velaclaw/plugin-sdk/channel-lifecycle`,
  `velaclaw/plugin-sdk/channel-reply-pipeline`,
  `velaclaw/plugin-sdk/command-auth`,
  `velaclaw/plugin-sdk/secret-input`, and
  `velaclaw/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `velaclaw/plugin-sdk/channel-config-helpers`,
  `velaclaw/plugin-sdk/allow-from`,
  `velaclaw/plugin-sdk/channel-config-schema`,
  `velaclaw/plugin-sdk/telegram-command-config`,
  `velaclaw/plugin-sdk/channel-policy`,
  `velaclaw/plugin-sdk/approval-gateway-runtime`,
  `velaclaw/plugin-sdk/approval-handler-adapter-runtime`,
  `velaclaw/plugin-sdk/approval-handler-runtime`,
  `velaclaw/plugin-sdk/approval-runtime`,
  `velaclaw/plugin-sdk/config-runtime`,
  `velaclaw/plugin-sdk/infra-runtime`,
  `velaclaw/plugin-sdk/agent-runtime`,
  `velaclaw/plugin-sdk/lazy-runtime`,
  `velaclaw/plugin-sdk/reply-history`,
  `velaclaw/plugin-sdk/routing`,
  `velaclaw/plugin-sdk/status-helpers`,
  `velaclaw/plugin-sdk/text-runtime`,
  `velaclaw/plugin-sdk/runtime-store`, and
  `velaclaw/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `velaclaw/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `velaclaw/plugin-sdk/*` subpaths. Velaclaw core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  Velaclaw is not yet serving a runtime snapshot.
...
- Avoid the root `velaclaw/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `velaclaw/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `velaclaw/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `velaclaw/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `velaclaw/plugin-sdk/channel-targets`; channel-specific
...
`velaclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Velaclaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Velaclaw's
...
- if both `catalog` and `discovery` are registered, Velaclaw uses `catalog`
...
- Read-only command paths such as `velaclaw status`, `velaclaw status --all`,
  `velaclaw channels status`, `velaclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `velaclaw.extensions`:
...
Security guardrail: every `velaclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `velaclaw plugins install` installs plugin dependencies with
...
Optional: `velaclaw.setupEntry` can point at a lightweight setup-only module.
When Velaclaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `velaclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Velaclaw load the
...
Channel plugins can advertise setup/discovery metadata via `velaclaw.channel` and
install hints via `velaclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@velaclaw/nextcloud-talk",
  "velaclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@velaclaw/nextcloud-talk",
...
Useful `velaclaw.channel` fields beyond the minimal example:
...
Velaclaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.velaclaw/mpm/plugins.json`
- `~/.velaclaw/mpm/catalog.json`
- `~/.velaclaw/plugins/catalog.json`

Or point `VELACLAW_PLUGIN_CATALOG_PATHS` (or `VELACLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "velaclaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "velaclaw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `VelaclawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Velaclaw stays opinionated without becoming hardcoded to one
