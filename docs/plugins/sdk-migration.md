...
read_when:
  - You see the VELACLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the VELACLAW_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Velaclaw plugin
---

# Plugin SDK Migration

Velaclaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`velaclaw/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`velaclaw/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`velaclaw/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `velaclaw/plugin-sdk/slack`, `velaclaw/plugin-sdk/discord`,
`velaclaw/plugin-sdk/signal`, `velaclaw/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`velaclaw/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `velaclaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `velaclaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "velaclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "velaclaw/plugin-sdk/compat";
    ...
    import { resolveControlCommandGate } from "velaclaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "velaclaw/extension-api";
...
```bash
VELACLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 velaclaw gateway run
VELACLAW_SUPPRESS_EXTENSION_API_WARNING=1 velaclaw gateway run
```
