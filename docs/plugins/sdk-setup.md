...
read_when:
  ...
  - You are defining plugin config schemas or package.json velaclaw metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`velaclaw.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `velaclaw` field that tells the plugin system what
...
```json
{
  "name": "@myorg/velaclaw-my-channel",
  "version": "1.0.0",
  "type": "module",
  "velaclaw": {
    "extensions": ["./index.ts"],
...
```json velaclaw-clawhub-package.json
{
  "name": "@myorg/velaclaw-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "velaclaw": {
    "extensions": ["./index.ts"],
    ...
    "build": {
      "velaclawVersion": "2026.3.24-beta.2",
...
`velaclaw.channel` is cheap package metadata for channel discovery and setup
...
`velaclaw.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported Velaclaw version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `velaclaw doctor --fix`.
...
When enabled, Velaclaw loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `velaclaw.plugin.json` in the package root.
Velaclaw uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Velaclaw",
  "configSchema": {
    "type": "object",
...
Velaclaw loads when it only needs setup surfaces (onboarding, config repair,
...
`velaclaw/plugin-sdk/channel-entry-contract` instead of
...
**When Velaclaw uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `velaclaw/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Velaclaw validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "velaclaw/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `velaclaw onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "velaclaw/plugin-sdk/channel-setup";
...
helpers from `velaclaw/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`velaclaw/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `velaclaw/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "velaclaw/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/velaclaw-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
velaclaw plugins install @myorg/velaclaw-my-plugin
```

Velaclaw tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
velaclaw plugins install clawhub:@myorg/velaclaw-my-plugin   # ClawHub only
```
...
<Info>
  For npm-sourced installs, `velaclaw plugins install` runs
