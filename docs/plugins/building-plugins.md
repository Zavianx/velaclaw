...
summary: "Create your first Velaclaw plugin in minutes"
read_when:
  - You want to create a new Velaclaw plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Velaclaw
---
...
You do not need to add your plugin to the Velaclaw repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`velaclaw plugins install <package-name>`. Velaclaw tries ClawHub first and
...
`velaclaw/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json velaclaw.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Velaclaw",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    velaclaw plugins install clawhub:@myorg/velaclaw-my-plugin
    ```

    Velaclaw also checks ClawHub before npm for bare package specs like
    `@myorg/velaclaw-my-plugin`.
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, Velaclaw retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `velaclaw/plugin-sdk/error-runtime`
...
Always import from focused `velaclaw/plugin-sdk/<subpath>` paths:

```typescript
import { definePluginEntry } from "velaclaw/plugin-sdk/plugin-entry";
import { createPluginRuntimeStore } from "velaclaw/plugin-sdk/runtime-store";

// Wrong: monolithic root (deprecated, will be removed)
import { ... } from "velaclaw/plugin-sdk";
```
...
package-root seam instead of promoting it into `velaclaw/plugin-sdk/*`.

Some generated `velaclaw/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `velaclaw` metadata</Check>
<Check>**velaclaw.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [Zavianx/velaclaw-dev](https://github.com/Zavianx/velaclaw-dev/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Velaclaw X account [@velaclaw](https://x.com/velaclaw) for release announcements.
