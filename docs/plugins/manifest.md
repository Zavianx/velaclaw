...
read_when:
  - You are building an Velaclaw plugin
...
This page is for the **native Velaclaw plugin manifest** only.
...
Velaclaw auto-detects those bundle layouts too, but they are not validated
against the `velaclaw.plugin.json` schema described here.
...
- static capability ownership snapshots used for bundled compat wiring and
  contract coverage
- cheap QA runner metadata that the shared `velaclaw qa` host can inspect
...
| `providerAuthEnvVars`               | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Velaclaw can inspect without loading plugin code.                                                                                                                      |
| `providerAuthAliases`               | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                   |
| `channelEnvVars`                    | No       | `Record<string, string[]>`       | Cheap channel env metadata that Velaclaw can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                     |
...
| `qaRunners`                         | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `velaclaw qa` host before plugin runtime loads.                                                                                                               |
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, Velaclaw falls back to `choiceId`.                                        |
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `velaclaw qa`, for example `matrix`.    |
...
Use `contracts` only for static capability ownership metadata that Velaclaw can
...
Use `modelSupport` when Velaclaw should infer your provider plugin from
...
Legacy top-level capability keys are deprecated. Use `velaclaw doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `velaclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `velaclaw` block used for entrypoints, install gating, setup, or catalog metadata |
...
| Field                                                             | What it means                                                                                                                                |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `velaclaw.extensions`                                             | Declares native plugin entrypoints.                                                                                                          |
| `velaclaw.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding and deferred channel startup.                                                       |
| `velaclaw.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                         |
| `velaclaw.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime. |
| `velaclaw.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.       |
| `velaclaw.install.npmSpec` / `velaclaw.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                           |
| `velaclaw.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                          |
| `velaclaw.install.minHostVersion`                                 | Minimum supported Velaclaw host version, using a semver floor like `>=2026.3.22`.                                                            |
| `velaclaw.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                               |
| `velaclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                         |
