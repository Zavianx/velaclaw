...
summary: "Step-by-step guide to building a model provider plugin for Velaclaw"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Velaclaw
...
(LLM) to Velaclaw. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to Velaclaw's normal inference loop. If the model
...
<Steps>
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "velaclaw/plugin-sdk/provider-auth";
    ...
    If the upstream provider uses different control tokens than Velaclaw, add a
    ...
    Velaclaw parses its own control markers or channel delivery.
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "velaclaw/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "velaclaw/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "velaclaw/plugin-sdk/provider-tools";
    ...
    example: `@velaclaw/anthropic-provider` exports
    ...
    `velaclaw/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@velaclaw/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@velaclaw/openrouter-provider`: `api.ts` exports the provider builder
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Velaclaw classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # velaclaw.providers metadata
├── velaclaw.plugin.json      # Manifest with provider auth metadata
