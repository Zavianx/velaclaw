...
read_when:
  ...
  - You want a reference for all registration methods on VelaclawPluginApi
...
prefer `velaclaw/plugin-sdk/channel-core`; keep `velaclaw/plugin-sdk/core` for
...
| `plugin-sdk/config-schema`  | `VelaclawSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `velaclaw.json` Zod schema export (`VelaclawSchema`) |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, Velaclaw tool bridge helpers, and attempt result utilities |
...
The `register(api)` callback receives an `VelaclawPluginApi` object with these
methods:
...
- User config still wins. Velaclaw merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `velaclaw/plugin-sdk/memory-host-core` instead of reaching into a specific
...
| `api.config`             | `VelaclawConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `velaclaw/plugin-sdk/<your-plugin>`
...
- `@velaclaw/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `velaclaw/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `velaclaw/plugin-sdk/speech`, `.../provider-model-shared`, or another
