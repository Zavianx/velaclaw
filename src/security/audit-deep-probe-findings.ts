...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("velaclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("velaclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set VELACLAW_GATEWAY_TOKEN/VELACLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("velaclaw security audit --deep")}".`,
    });
  }
  return findings;
}
