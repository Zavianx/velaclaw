...
function summarizeGroupPolicy(cfg: VelaclawConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: VelaclawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: VelaclawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: VelaclawConfig): boolean {
...
function isBrowserEnabled(cfg: VelaclawConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: VelaclawConfig): SecurityAuditFinding[] {
