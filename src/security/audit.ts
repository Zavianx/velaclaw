...
import type { ConfigFileSnapshot, VelaclawConfig } from "../config/config.js";
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Velaclaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Velaclaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.VELACLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.VELACLAW_GATEWAY_PASSWORD);
...
export function collectLoggingFindings(cfg: VelaclawConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: VelaclawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExecRuntimeFindings(cfg: VelaclawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: VelaclawConfig): string[] {
