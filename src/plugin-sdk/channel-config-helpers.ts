...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`velaclaw pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`velaclaw pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends VelaclawConfig = VelaclawConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends VelaclawConfig = VelaclawConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends VelaclawConfig = VelaclawConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: VelaclawConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends VelaclawConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends VelaclawConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends VelaclawConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
