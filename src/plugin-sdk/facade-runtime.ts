...
const VELACLAW_PACKAGE_ROOT =
  resolveLoaderPackageRoot({
    modulePath: fileURLToPath(import.meta.url),
    moduleUrl: import.meta.url,
  }) ?? fileURLToPath(new URL("../..", import.meta.url));
const CURRENT_MODULE_PATH = fileURLToPath(import.meta.url);
const VELACLAW_SOURCE_EXTENSIONS_ROOT = path.resolve(VELACLAW_PACKAGE_ROOT, "extensions");
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  if (preferSource) {
    const modulePath =
      resolveBundledPluginSourcePublicSurfacePath({
        ...params,
        sourceRoot: bundledPluginsDir ?? path.resolve(VELACLAW_PACKAGE_ROOT, "extensions"),
      }) ??
      resolveBundledPluginPublicSurfacePath({
        rootDir: VELACLAW_PACKAGE_ROOT,
        env: params.env,
...
function buildFacadeActivationCheckParams(
  ...
  return {
    ...params,
    location,
    sourceExtensionsRoot: VELACLAW_SOURCE_EXTENSIONS_ROOT,
    resolutionKey: createFacadeResolutionKey(params),
  };
}
