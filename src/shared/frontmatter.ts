...
export function resolveVelaclawManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type VelaclawManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveVelaclawManifestRequires(
  metadataObj: Record<string, unknown>,
): VelaclawManifestRequires | undefined {
  const requiresRaw =
...
export function resolveVelaclawManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveVelaclawManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedVelaclawManifestInstallBase = {
...
export function parseVelaclawManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedVelaclawManifestInstallBase | undefined {
  ...
  const spec: ParsedVelaclawManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyVelaclawManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedVelaclawManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
