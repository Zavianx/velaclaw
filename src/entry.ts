...
import { ensureVelaclawExecMarkerOnProcess } from "./infra/velaclaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "velaclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "velaclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  process.title = "velaclaw";
  ensureVelaclawExecMarkerOnProcess();
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.VELACLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureCliRespawnReady(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[velaclaw] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Velaclaw ${VERSION} (${commit})` : `Velaclaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[velaclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsedContainer.ok) {
      console.error(`[velaclaw] ${parsedContainer.error}`);
      process.exit(2);
    }
    ...
    if (!parsed.ok) {
      ...
      console.error(`[velaclaw] ${parsed.error}`);
      process.exit(2);
    }

    const containerTargetName = resolveCliContainerTarget(process.argv);
    if (containerTargetName && parsed.profile) {
      console.error("[velaclaw] --container cannot be combined with --profile/--dev");
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[velaclaw] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
