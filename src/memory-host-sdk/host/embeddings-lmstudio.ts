...
// LMStudio extension removed — stub imports
const LMSTUDIO_PROVIDER_ID = "lmstudio" as const;
const LMSTUDIO_DEFAULT_EMBEDDING_MODEL = "nomic-embed-text-v1.5";
function buildLmstudioAuthHeaders(..._args: unknown[]): Record<string, string> {
  return {};
}
function ensureLmstudioModelLoaded(..._args: unknown[]): Promise<void> {
  return Promise.resolve();
}
function resolveLmstudioInferenceBase(..._args: unknown[]): string {
  return "http://127.0.0.1:1234";
}
function resolveLmstudioProviderHeaders(..._args: unknown[]): Record<string, string> {
  return {};
}
function resolveLmstudioRuntimeApiKey(..._args: unknown[]): string | undefined {
  return undefined;
}
...
async function resolveLmstudioApiKey(
  ...
  try {
    return resolveLmstudioRuntimeApiKey({
...
export async function createLmstudioEmbeddingProvider(
  ...
  const providerHeaders = resolveLmstudioProviderHeaders({
    config: options.config,
    env: process.env,
