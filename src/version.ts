...
declare const __VELACLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "velaclaw";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["VELACLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["VELACLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["VELACLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.VELACLAW_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Velaclaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __VELACLAW_VERSION__ === "string" ? __VELACLAW_VERSION__ : undefined,
  bundledVersion: process.env.VELACLAW_BUNDLED_VERSION,
});
