...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Velaclaw Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# Velaclaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(velaclaw completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "velaclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
