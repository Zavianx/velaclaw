...
// TTS removed: canonicalizeSpeechProviderId, listSpeechProviders from ../tts/provider-registry.js
// TTS removed: getTtsProvider, listSpeechVoices, resolveExplicitTtsOverrides, resolveTtsConfig,
//              resolveTtsPrefsPath, setTtsEnabled, setTtsProvider, textToSpeech from ../tts/tts.js
...
function getAuthProfileIdsForProvider(cfg: VelaclawConfig, providerId: string): string[] {
...
async function writeOutputAsset(params: {
  ...
  const requestedMime = normalizeMimeType(detectMime({ filePath: resolvedOutput }));
    ...
    params.file && params.file.length > 0
      ? await Promise.all(
          (await readInputFiles(params.file)).map(async (entry) => ({
            ...
            mimeType: detectMime({ buffer: entry.buffer, filePath: entry.path }) ?? "image/png",
          })),
        )
      : undefined;
...
async function runTtsConvert(params: {
  ...
  // TTS removed -- local TTS subsystem no longer available
  throw new Error("Local TTS is not available (TTS subsystem removed).");
}
...
async function runTtsProviders(transport: CapabilityTransport) {
  ...
  // TTS removed -- local TTS provider listing no longer available
  return { providers: [], active: undefined };
}

async function runTtsVoices(_providerRaw?: string) {
  // TTS removed -- no voices available
  return [];
}
...
async function runTtsStateMutation(params: {
  ...
  // TTS removed -- local TTS state mutation no longer available
  throw new Error("Local TTS is not available (TTS subsystem removed).");
}
...
export function registerCapabilityCli(program: Command) {
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.velaclaw.ai/cli/infer")}\n`,
    );
