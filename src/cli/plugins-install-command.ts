...
async function loadConfigFromSnapshotForInstall(
  ...
  if (resolvePluginInstallInvalidConfigPolicy(request) !== "allow-bundled-recovery") {
    throw buildInvalidPluginInstallConfigError(
      "Config invalid; run `velaclaw doctor --fix` before installing plugins.",
    );
  }
  ...
  if (!snapshot.exists || Object.keys(parsed).length === 0) {
    throw buildInvalidPluginInstallConfigError(
      "Config file could not be parsed; run `velaclaw doctor` to repair it.",
    );
  }
  if (
    ...
    throw buildInvalidPluginInstallConfigError(
      `Config invalid outside the bundled recovery path for ${pluginLabel}; run \`velaclaw doctor --fix\` before reinstalling it.`,
    );
  }
