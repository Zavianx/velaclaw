...
export function resolveMissingPluginCommandMessage(
  ...
  if (parentPluginId) {
    ...
    if (config?.plugins?.entries?.[parentPluginId]?.enabled === false) {
      return (
        `The \`velaclaw ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`velaclaw ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.VELACLAW_CONTAINER) ?? null;
  ...
  try {
    ...
    process.on("uncaughtException", (error) => {
      console.error("[velaclaw] Uncaught exception:", formatUncaughtError(error));
