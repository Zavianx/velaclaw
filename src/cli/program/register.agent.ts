...
${formatHelpExamples([
  ['velaclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['velaclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'velaclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'velaclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['velaclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'velaclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.velaclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.velaclaw.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'velaclaw agents set-identity --agent main --name "Velaclaw" --emoji "⛵🦞"',
    "Set name + emoji.",
  ],
  ["velaclaw agents set-identity --agent main --avatar avatars/velaclaw.png", "Set avatar path."],
  [
    "velaclaw agents set-identity --workspace ~/.velaclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "velaclaw agents set-identity --identity-file ~/.velaclaw/workspace/IDENTITY.md --agent main",
