...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["velaclaw status", "Show channel health + session summary."],
          ["velaclaw status --all", "Full diagnosis (read-only)."],
          ["velaclaw status --json", "Machine-readable output."],
          ["velaclaw status --usage", "Show model provider usage/quota snapshots."],
          [
            "velaclaw status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["velaclaw status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.velaclaw.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.velaclaw.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["velaclaw sessions", "List all sessions."],
          ["velaclaw sessions --agent work", "List sessions for one agent."],
          ["velaclaw sessions --all-agents", "Aggregate sessions across agents."],
          ["velaclaw sessions --active 120", "Only last 2 hours."],
          ["velaclaw sessions --json", "Machine-readable output."],
          ["velaclaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.velaclaw.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["velaclaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "velaclaw sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["velaclaw sessions cleanup --enforce", "Apply maintenance now."],
          ["velaclaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["velaclaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "velaclaw sessions cleanup --enforce --store ./tmp/sessions.json",
