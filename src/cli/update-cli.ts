...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Velaclaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["velaclaw update", "Update a source checkout (git)"],
        ["velaclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["velaclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["velaclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["velaclaw update --tag main", "One-off package install from GitHub main"],
        ["velaclaw update --dry-run", "Preview actions without changing anything"],
        ["velaclaw update --no-restart", "Update without restarting the service"],
        ["velaclaw update --json", "Output result as JSON"],
        ["velaclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["velaclaw update wizard", "Interactive update wizard"],
        ["velaclaw --update", "Shorthand for velaclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run velaclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.velaclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["velaclaw update status", "Show channel + version status."],
          ["velaclaw update status --json", "JSON output."],
          ["velaclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.velaclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
