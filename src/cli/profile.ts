...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.velaclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.VELACLAW_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.VELACLAW_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.VELACLAW_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.VELACLAW_CONFIG_PATH)) {
    env.VELACLAW_CONFIG_PATH = path.join(stateDir, "velaclaw.json");
  }

  if (profile === "dev" && !env.VELACLAW_GATEWAY_PORT?.trim()) {
    env.VELACLAW_GATEWAY_PORT = "19001";
  }
}
