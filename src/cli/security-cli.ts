...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["velaclaw security audit", "Run a local security audit."],
          ["velaclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["velaclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "velaclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["velaclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["velaclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.velaclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Velaclaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("velaclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("velaclaw security audit --fix")}`));
        if (!fixResult) {
