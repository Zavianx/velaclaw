...
import { velaClawRootFs, velaClawRootFsSync } from "./velaclaw-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["velaclaw"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  try {
    return parsePackageName(await velaClawRootFs.readFile(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function readPackageNameSync(dir: string): string | null {
  try {
    return parsePackageName(
      velaClawRootFsSync.readFileSync(path.join(dir, "package.json"), "utf-8"),
    );
  } catch {
    return null;
  }
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = velaClawRootFsSync.realpathSync(normalized);
...
export async function resolveVelaclawPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveVelaclawPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
