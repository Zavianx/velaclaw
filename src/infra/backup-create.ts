...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Velaclaw config file was found to back up."
        : "No local Velaclaw state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "velaclaw-backup-"));
