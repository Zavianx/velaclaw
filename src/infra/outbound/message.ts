...
function buildMessagePollResult(params: {
  ...
  return {
    ...
    maxSelections: params.normalized.maxSelections ?? 1,
...
function resolveRequiredPlugin(channel: string, cfg: VelaclawConfig) {
...
async function resolveMessageConfig(cfg?: VelaclawConfig): Promise<VelaclawConfig> {
  if (cfg) {
    return cfg;
  }
