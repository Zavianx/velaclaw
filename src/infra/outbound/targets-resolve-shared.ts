import { mapAllowFromEntries } from "velaclaw/plugin-sdk/channel-config-helpers";
...
function buildWebChatDeliveryError(): Error {
  return new Error(
    `Delivering to WebChat is not supported via \`${formatCliCommand("velaclaw agent")}\`; use WhatsApp/Telegram or run with --deliver=false.`,
  );
}
