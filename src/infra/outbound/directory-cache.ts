...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: VelaclawConfig | null = null;
  ...
  get(key: string, cfg: VelaclawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: VelaclawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: VelaclawConfig): void {
