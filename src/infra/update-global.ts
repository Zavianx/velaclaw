...
const PRIMARY_PACKAGE_NAME = "velaclaw";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const VELACLAW_MAIN_PACKAGE_SPEC = "github:Zavianx/velaclaw-dev#main";
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "velaclaw.plugin.json")))
      ) {
        expectedFiles.add(relativePath);
      }
    }),
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Velaclaw", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.VELACLAW_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.VELACLAW_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return VELACLAW_MAIN_PACKAGE_SPEC;
  }
