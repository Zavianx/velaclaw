...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[velaclaw] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  ...
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[velaclaw] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[velaclaw] FATAL unhandled rejection:", formatUncaughtError(reason));
      exitWithTerminalRestore("fatal unhandled rejection");
      return;
    }

    if (isConfigError(reason)) {
      console.error("[velaclaw] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
    ...
    if (isTransientUnhandledRejectionError(reason)) {
      console.warn(
        "[velaclaw] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[velaclaw] Unhandled promise rejection:", formatUncaughtError(reason));
