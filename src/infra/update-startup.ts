...
import { resolveVelaclawPackageRoot } from "./velaclaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: VelaclawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: VelaclawConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveVelaclawPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("velaclaw update")}`,
      );
