...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __velaclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__velaclawDiagnosticEventsState) {
    globalStore.__velaclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__velaclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: VelaclawConfig): boolean {
