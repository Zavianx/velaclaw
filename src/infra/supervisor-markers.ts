const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "VELACLAW_LAUNCHD_LABEL"],
  systemd: ["VELACLAW_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["VELACLAW_WINDOWS_TASK_NAME"],
} as const;
...
export const SUPERVISOR_HINT_ENV_VARS = [
  ...
  "VELACLAW_SERVICE_MARKER",
  "VELACLAW_SERVICE_KIND",
] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.VELACLAW_SERVICE_MARKER?.trim();
    const serviceKind = env.VELACLAW_SERVICE_KIND?.trim();
