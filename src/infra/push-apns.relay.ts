...
const GATEWAY_DEVICE_ID_HEADER = "x-velaclaw-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-velaclaw-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-velaclaw-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.VELACLAW_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "VELACLAW_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or VELACLAW_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.VELACLAW_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require VELACLAW_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.VELACLAW_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
