...
export const POSIX_VELACLAW_TMP_DIR = "/tmp/velaclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredVelaclawTmpDirOptions = {
...
export function resolvePreferredVelaclawTmpDir(
  options: ResolvePreferredVelaclawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "velaclaw" : `velaclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[velaclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Velaclaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Velaclaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_VELACLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_VELACLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_VELACLAW_TMP_DIR)) {
      return POSIX_VELACLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_VELACLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_VELACLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_VELACLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_VELACLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_VELACLAW_TMP_DIR;
