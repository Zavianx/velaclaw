 ...
 * returning control to the caller (typically `triggerVelaclawRestart` →
...
/**
 * Parse velaclaw gateway PIDs from lsof -Fpc stdout.
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (
        currentPid != null &&
        currentCmd &&
        normalizeLowercaseStringOrEmpty(currentCmd).includes("velaclaw")
      ) {
...
/**
 * Windows: find listening PIDs on the port, then verify each is an velaclaw
 ...
 * Returns only PIDs that belong to velaclaw gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only velaclaw gateway processes.
