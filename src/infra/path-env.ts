...
function candidateBinDirs(opts: EnsureVelaclawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Velaclaw process is on.
  try {
  ...
  // Bundled macOS app: `velaclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "velaclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.VELACLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "velaclaw"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `velaclaw`
...
/**
 * Best-effort PATH bootstrap so skills that require the `velaclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureVelaclawCliOnPath(opts: EnsureVelaclawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.VELACLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.VELACLAW_PATH_BOOTSTRAPPED = "1";
