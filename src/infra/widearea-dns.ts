...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.VELACLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*velaclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "velaclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "velaclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "velaclaw-gw");
  ...
  records.push(`_velaclaw-gw._tcp IN PTR ${instanceLabel}._velaclaw-gw._tcp`);
  records.push(`${instanceLabel}._velaclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._velaclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; velaclaw-content-hash: ${contentHash}\n${contentBody}`;
}
