...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open Velaclaw to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "velaclaw.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.VELACLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.VELACLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set VELACLAW_APNS_TEAM_ID and VELACLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.VELACLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.VELACLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.VELACLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set VELACLAW_APNS_PRIVATE_KEY_P8 or VELACLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading VELACLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
