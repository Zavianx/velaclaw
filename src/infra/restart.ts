...
export function triggerVelaclawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.VELACLAW_SYSTEMD_UNIT,
      process.env.VELACLAW_PROFILE,
    );
  ...
  const label =
    process.env.VELACLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.VELACLAW_PROFILE);
