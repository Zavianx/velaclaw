...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.VELACLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Velaclaw. You can keep running the web gateway without it: \`${formatCliCommand("velaclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
