...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: VelaclawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: VelaclawConfig) {
...
function resolveHeartbeatAgents(cfg: VelaclawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: VelaclawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: VelaclawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: VelaclawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
