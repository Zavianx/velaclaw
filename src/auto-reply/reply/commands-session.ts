...
import { scheduleGatewaySigusr1Restart, triggerVelaclawRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Velaclaw in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerVelaclawRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Velaclaw via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
