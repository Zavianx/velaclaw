...
type AgentDefaults = NonNullable<NonNullable<VelaclawConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.velaclaw/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
// TTS subsystem removed -- voice mode line is always null.
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `⛵ Velaclaw 🦞 ${VERSION}${commit ? ` (${commit})` : ""}`;
