import type { VelaclawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "velaclaw-bundled": {
    ...
    canOverride: ["velaclaw-bundled"],
    canBeOverriddenBy: ["velaclaw-managed", "velaclaw-plugin"],
  },
  "velaclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["velaclaw-bundled", "velaclaw-plugin"],
    canBeOverriddenBy: ["velaclaw-managed"],
  },
  "velaclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["velaclaw-bundled", "velaclaw-managed", "velaclaw-plugin"],
    canBeOverriddenBy: ["velaclaw-managed"],
  },
  "velaclaw-workspace": {
    precedence: 40,
    ...
    canOverride: ["velaclaw-workspace"],
    canBeOverriddenBy: ["velaclaw-workspace"],
  },
};
...
export function resolveHookConfig(
  config: VelaclawConfig | undefined,
  hookKey: string,
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "velaclaw-plugin") {
    return { enabled: true };
  }
