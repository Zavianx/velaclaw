...
This directory contains hooks that ship with Velaclaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.velaclaw/workspace`)
...
homepage: https://docs.velaclaw.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.velaclaw/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.Zavianx/velaclaw-dev.json`:
...
2. Restart gateway: `pkill -9 -f 'velaclaw.*gateway' && pnpm velaclaw gateway`
3. Enable the hook: `velaclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.velaclaw.ai/automation/hooks
