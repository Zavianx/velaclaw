...
homepage: https://docs.velaclaw.ai/automation/hooks#command-logger
metadata:
  {
    "velaclaw":
      {
        "emoji": "📝",
        "events": ["command"],
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Velaclaw" }],
      },
  }
---
...
2. **Appends to log file** - Writes a JSON line to `~/.velaclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.velaclaw/logs/commands.log ~/.velaclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/velaclaw`:
   ```
   /home/username/.velaclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
