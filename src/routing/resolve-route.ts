...
function listAgents(cfg: VelaclawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: VelaclawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<VelaclawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: VelaclawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: VelaclawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: VelaclawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<VelaclawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  VelaclawConfig,
  {
    bindingsRef: VelaclawConfig["bindings"];
    agentsRef: VelaclawConfig["agents"];
    sessionRef: VelaclawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: VelaclawConfig): Map<string, ResolvedAgentRoute> {
