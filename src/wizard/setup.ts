...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Velaclaw is a hobby project and still in beta. Expect sharp edges.",
      "By default, Velaclaw is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Velaclaw is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Velaclaw.",
      ...
      "",
      "Must read: https://docs.velaclaw.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("Velaclaw setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: VelaclawConfig = snapshot.valid
    ? snapshot.exists
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.velaclaw.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("velaclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("velaclaw doctor")}`,
        `Inspect: ${formatCliCommand("velaclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("velaclaw configure")}.`;
  ...
  let localGatewayToken = process.env.VELACLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.VELACLAW_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: VelaclawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
