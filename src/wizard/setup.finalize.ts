...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.velaclaw.ai/gateway/health",
            "https://docs.velaclaw.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("velaclaw gateway run")}`,
          `Or rerun with: ${formatCliCommand("velaclaw onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("velaclaw onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.velaclaw.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: $VELACLAW_CONFIG_PATH (default: ~/.Zavianx/velaclaw-dev.json) under gateway.auth.token, or in VELACLAW_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("velaclaw config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("velaclaw doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("velaclaw dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Velaclaw."
            : "Copy/paste this URL in a browser on this machine to control Velaclaw.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("velaclaw dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.velaclaw.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.velaclaw.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("velaclaw configure --section web")}`,
          "",
          "Docs: https://docs.velaclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("velaclaw configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.velaclaw.ai/tools/web"}`,
          "Docs: https://docs.velaclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("velaclaw configure --section web")}`,
          "",
          "Docs: https://docs.velaclaw.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://velaclaw.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Velaclaw."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Velaclaw.",
  );
