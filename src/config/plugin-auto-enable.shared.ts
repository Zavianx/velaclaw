...
function collectModelRefs(cfg: VelaclawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: VelaclawConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: VelaclawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: VelaclawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: VelaclawConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: VelaclawConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: VelaclawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: VelaclawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: VelaclawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: VelaclawConfig): boolean {
...
function hasSetupAutoEnableRelevantConfig(cfg: VelaclawConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: VelaclawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function configMayNeedPluginManifestRegistry(cfg: VelaclawConfig): boolean {
...
function isPluginExplicitlyDisabled(cfg: VelaclawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: VelaclawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isBuiltInChannelAlreadyEnabled(cfg: VelaclawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: VelaclawConfig, pluginId: string): VelaclawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
