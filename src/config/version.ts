...
export function parseVelaclawVersion(raw: string | null | undefined): VelaclawVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeVelaclawVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseVelaclawVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameVelaclawStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseVelaclawVersion(a);
  const parsedB = parseVelaclawVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareVelaclawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseVelaclawVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseVelaclawVersion(current);
  const parsedTouched = parseVelaclawVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameVelaclawStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareVelaclawVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: VelaclawVersion): number {
