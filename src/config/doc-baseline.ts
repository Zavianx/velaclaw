...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.VELACLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveVelaclawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    VELACLAW_STATE_DIR: path.join(os.tmpdir(), "velaclaw-config-doc-baseline-state"),
    VELACLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
