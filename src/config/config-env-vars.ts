...
function collectConfigEnvVarsByTarget(cfg?: VelaclawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: VelaclawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: VelaclawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: VelaclawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.VELACLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
