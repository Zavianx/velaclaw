...
import type { VelaclawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function coerceConfig(value: unknown): VelaclawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as VelaclawConfig;
}
...
function stampConfigVersion(cfg: VelaclawConfig): VelaclawConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: VelaclawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer Velaclaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as VelaclawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function finalizeLoadedRuntimeConfig(cfg: VelaclawConfig): VelaclawConfig {
  ...
  function loadConfig(): VelaclawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as VelaclawConfig, {
        env: deps.env,
        homedir: deps.homedir,
      });
  ...
  async function readBestEffortConfig(): Promise<VelaclawConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<VelaclawConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
    ...
    let cfgToWrite = persistCandidate as VelaclawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as VelaclawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.VELACLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.VELACLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `VELACLAW_CONFIG_PATH` (and friends) are expected to work even
...
function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: VelaclawConfig;
  candidate: VelaclawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: VelaclawConfig): VelaclawConfig {
...
export function loadConfig(): VelaclawConfig {
  ...
  // reparsing velaclaw.json on hot code paths.
  return loadPinnedRuntimeConfig(() => createConfigIO().loadConfig());
}

export function getRuntimeConfig(): VelaclawConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<VelaclawConfig> {
  return await createConfigIO().readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<VelaclawConfig> {
