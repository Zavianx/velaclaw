...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.velaclaw) || !isRecord(entry.velaclaw.channel)) {
      continue;
    }
    const channel = entry.velaclaw.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: VelaclawConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: VelaclawConfig, pluginId: string) => boolean;
