...
import type { VelaclawConfig, ConfigValidationIssue } from "./types.js";
import { coerceSecretRef } from "./types.secrets.js";
import { VelaclawSchema } from "./zod-schema.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.velaclaw.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: VelaclawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: VelaclawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: VelaclawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = VelaclawSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as VelaclawConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: VelaclawConfig | null | undefined;
  ...
  const ensureCompatConfig = (): VelaclawConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Velaclaw config schema.
