...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("velaclaw doctor --fix")} or ${formatCliCommand("velaclaw memory status --fix")}`
    : `Verify: ${formatCliCommand("velaclaw memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: VelaclawConfig): Promise<void> {
  try {
 ...
 * Runs as part of `velaclaw doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("velaclaw config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("velaclaw config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("velaclaw memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`velaclaw config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("velaclaw configure --section model")}`,
        `- To disable: ${formatCliCommand("velaclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("velaclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
