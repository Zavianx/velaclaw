...
export async function loadValidConfigOrThrow(): Promise<VelaclawConfig> {
...
export async function updateConfig(
  mutator: (cfg: VelaclawConfig) => VelaclawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: VelaclawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: VelaclawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("velaclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
