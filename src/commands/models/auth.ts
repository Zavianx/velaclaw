...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("velaclaw plugins list --json")}\`.`,
  );
}
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("velaclaw plugins install")}\`.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in Velaclaw.");
    runtime.log("Velaclaw prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this Velaclaw path is allowed again.");
  }
}
...
function maybeLogOpenAICodexNativeSearchTip(runtime: RuntimeEnv, providerId: string) {
  ...
  runtime.log(
    "Tip: Codex-capable models can use native Codex web search. Enable it with velaclaw configure --section web (recommended mode: cached). Docs: https://docs.velaclaw.ai/tools/web",
  );
}
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("velaclaw plugins install")}\`.`,
    );
  }
