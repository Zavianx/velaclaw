...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("velaclaw tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("velaclaw tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: VelaclawConfig) {
