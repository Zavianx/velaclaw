...
import type { ConfigFileSnapshot } from "../config/types.velaclaw.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("velaclaw config set gateway.auth.mode token")} or ${formatCliCommand("velaclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
