...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.VELACLAW_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: VelaclawConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: VelaclawConfig) => {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: VelaclawConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: VelaclawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.VELACLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<VelaclawConfig> {
