...
export const resolveVelaclawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadVelaclawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../infra/velaclaw-root.js", () => ({
  resolveVelaclawPackageRoot,
  resolveVelaclawPackageRootSync: vi.fn(() => "/tmp/velaclaw"),
}));
...
beforeEach(() => {
  ...
  resolveVelaclawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.VELACLAW_STATE_DIR;
  originalUpdateInProgress = process.env.VELACLAW_UPDATE_IN_PROGRESS;
  process.env.VELACLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "velaclaw-doctor-state-"));
  process.env.VELACLAW_STATE_DIR = tempStateDir;
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.VELACLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
