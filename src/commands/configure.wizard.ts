...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.VELACLAW_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.VELACLAW_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from velaclaw.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<VelaclawConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Velaclaw update wizard" : "Velaclaw configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    let currentBaseHash = snapshot.hash;
    const baseConfig: VelaclawConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("velaclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.VELACLAW_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.VELACLAW_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
    ...
    const newPassword =
      process.env.VELACLAW_GATEWAY_PASSWORD ??
    ...
    const token =
      process.env.VELACLAW_GATEWAY_TOKEN ??
      (await resolveGatewaySecretInputForWizard({
        cfg: nextConfig,
