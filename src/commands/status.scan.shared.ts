...
export function hasExplicitMemorySearchConfig(cfg: VelaclawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: VelaclawConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: VelaclawConfig, agentId: string) => { store: { path: string } } | null;
