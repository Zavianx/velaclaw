...
export async function setupWizardCommand(
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://docs.velaclaw.ai/security",
        `Re-run with: ${formatCliCommand("velaclaw onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected - Velaclaw runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.velaclaw.ai/windows",
      ].join("\n"),
    );
  }
