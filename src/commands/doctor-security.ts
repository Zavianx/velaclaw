...
import type { VelaclawConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: VelaclawConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: VelaclawConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Velaclaw default (full)";
  const defaultRequestedAskSource = "Velaclaw default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("velaclaw approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: VelaclawConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: VelaclawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("velaclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.velaclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("velaclaw approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.velaclaw.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("velaclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("velaclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("velaclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("velaclaw config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('velaclaw config set session.dmScope "per-channel-peer"') +
