...
const SEARCH_TOOL = "https://docs.velaclaw.ai/mcp.SearchVelaclaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.velaclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('velaclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.velaclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('velaclaw docs "your query"')}`);
    }
    return;
  }
