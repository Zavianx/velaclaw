...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as VelaclawConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as VelaclawConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<VelaclawConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<VelaclawConfig["models"]>>;
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<VelaclawConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as VelaclawConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<VelaclawConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyMistralModelMaxTokens(
  ...
  return {
    ...
    models: {
      ...cfg.models,
      providers: nextProviders as NonNullable<VelaclawConfig["models"]>["providers"],
    },
  };
}
