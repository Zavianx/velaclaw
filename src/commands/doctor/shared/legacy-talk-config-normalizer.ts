...
export function normalizeLegacyTalkConfig(cfg: VelaclawConfig, changes: string[]): VelaclawConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as VelaclawConfig["talk"]) ?? {};
