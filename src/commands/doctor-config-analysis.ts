...
import { VelaclawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: VelaclawConfig): {
  config: VelaclawConfig;
  removed: string[];
} {
  const parsed = VelaclawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: VelaclawConfig): void {
