...
export function createGatewayReloadHandlers(params: {
  ...
  createHealthMonitor: (config: VelaclawConfig) => ChannelHealthMonitor | null;
}) {
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: VelaclawConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (VELACLAW_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.VELACLAW_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.VELACLAW_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (VELACLAW_SKIP_CHANNELS=1 or VELACLAW_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: VelaclawConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
...
export function startManagedGatewayConfigReloader(params: {
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: VelaclawConfig) => string | undefined;
