...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__velaclaw__/assistant-media";
...
async function resolveAssistantMediaAvailability(
  ...
  try {
    assertLocalMediaAllowed(source, localRoots);
...
export async function handleControlUiAssistantMediaRequest(
  ...
  try {
    ...
    const mime = detectMime({
      buffer: sniffBuffer?.subarray(0, bytesRead) ?? Buffer.alloc(0),
      filePath: source,
    });
    if (mime) {
