...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("VELACLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.VELACLAW_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.VELACLAW_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.VELACLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    configPath: process.env.VELACLAW_CONFIG_PATH,
    stateDir: process.env.VELACLAW_STATE_DIR,
    token: process.env.VELACLAW_GATEWAY_TOKEN,
    skipChannels: process.env.VELACLAW_SKIP_CHANNELS,
    skipProviders: process.env.VELACLAW_SKIP_PROVIDERS,
    skipGmail: process.env.VELACLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.VELACLAW_SKIP_CRON,
    skipCanvas: process.env.VELACLAW_SKIP_CANVAS_HOST,
    skipBrowserControl: process.env.VELACLAW_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.VELACLAW_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.VELACLAW_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  process.env.VELACLAW_SKIP_CHANNELS = "1";
  process.env.VELACLAW_SKIP_PROVIDERS = "1";
  process.env.VELACLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.VELACLAW_SKIP_CRON = "1";
  process.env.VELACLAW_SKIP_CANVAS_HOST = "1";
  process.env.VELACLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.VELACLAW_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("VELACLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("VELACLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("VELACLAW_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("VELACLAW_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("VELACLAW_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("VELACLAW_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("VELACLAW_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("VELACLAW_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("VELACLAW_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("VELACLAW_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("VELACLAW_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
