...
function listConfiguredAgentIds(cfg: VelaclawConfig): string[] {
...
export function listAgentsForGateway(cfg: VelaclawConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
export function loadCombinedSessionStoreForGateway(cfg: VelaclawConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: VelaclawConfig): GatewaySessionsDefaults {
