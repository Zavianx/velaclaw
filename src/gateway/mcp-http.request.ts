...
function resolveScopedSessionKey(cfg: VelaclawConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  const senderIsOwnerRaw = normalizeOptionalLowercaseString(
    getHeader(req, "x-velaclaw-sender-is-owner"),
  );
  return {
    sessionKey: resolveScopedSessionKey(cfg, getHeader(req, "x-session-key")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-velaclaw-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-velaclaw-account-id")),
    senderIsOwner:
