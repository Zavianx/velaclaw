...
function loadGatewayConfig(): VelaclawConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: VelaclawConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.VELACLAW_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as VelaclawConfig) : loadGatewayConfig());
