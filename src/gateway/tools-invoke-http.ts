...
function resolveMemoryToolDisableReasons(cfg: VelaclawConfig): string[] {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-velaclaw-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-velaclaw-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-velaclaw-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-velaclaw-thread-id"));
