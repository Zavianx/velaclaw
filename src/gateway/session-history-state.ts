...
import { attachVelaclawTranscriptMeta, readSessionMessages } from "./session-utils.js";
...
export type SessionHistoryMessage = Record<string, unknown> & {
  __velaclaw?: SessionHistoryTranscriptMeta;
};
...
export function resolveMessageSeq(message: SessionHistoryMessage | undefined): number | undefined {
  const seq = message?.__velaclaw?.seq;
...
export class SessionHistorySseState {
  ...
  appendInlineMessage(update: {
    ...
    const nextMessage = attachVelaclawTranscriptMeta(update.message, {
