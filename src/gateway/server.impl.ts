...
import { ensureVelaclawCliOnPath } from "../infra/path-env.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.VELACLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.VELACLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "VELACLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "VELACLAW_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `velaclaw config set gateway.auth.mode token` and `velaclaw config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: VelaclawConfig) =>
  ...
  try {
    ...
    const gatewayRequestContext = createGatewayRequestContext({
      ...
      clients,
      enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: VelaclawConfig) => {
        enforceSharedGatewaySessionGenerationForConfigWrite({
