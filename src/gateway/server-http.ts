...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-Velaclaw-Token header (query parameters are not allowed).",
      );
      return true;
    }
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      if (canvasHost) {
        ...
        requestStages.push({
          name: "canvas-http",
          run: () => canvasHost.handleHttpRequest?.(req, res) ?? false,
        });
      }
        ...
        wss.handleUpgrade(req, socket, head, (ws) => {
          (
            ws as unknown as import("ws").WebSocket & {
              __velaclawPreauthBudgetClaimed?: boolean;
              __velaclawPreauthBudgetKey?: string;
            }
          ).__velaclawPreauthBudgetKey = preauthBudgetKey;
          ...
          const budgetClaimed = Boolean(
            ...
            ).__velaclawPreauthBudgetClaimed,
          );
          if (budgetClaimed) {
            budgetTransferred = true;
