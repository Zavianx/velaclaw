...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__velaclaw_unknown_client_ip__";

export function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.VELACLAW_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.VELACLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
