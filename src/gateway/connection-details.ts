...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: VelaclawConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.VELACLAW_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.VELACLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set VELACLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: velaclaw doctor --fix",
