...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([VELACLAW_MODEL_ID, VELACLAW_DEFAULT_MODEL_ID]);
  ids.add(`velaclaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`velaclaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== VELACLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
