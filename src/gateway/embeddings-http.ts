...
function resolveAutoExplicitProviders(cfg: VelaclawConfig): Set<string> {
  return new Set(
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== VELACLAW_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `velaclaw` or `velaclaw/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-velaclaw-model")) ||
