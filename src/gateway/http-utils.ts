...
export const VELACLAW_MODEL_ID = "velaclaw";
export const VELACLAW_DEFAULT_MODEL_ID = "velaclaw/default";
...
export function resolveTrustedHttpOperatorScopes(
  ...
  const headerValue = getHeader(req, "x-velaclaw-scopes");
...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-velaclaw-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-velaclaw-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === VELACLAW_MODEL_ID || lowered === VELACLAW_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^velaclaw[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `velaclaw` or `velaclaw/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-velaclaw-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-velaclaw-model`." };
  }
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-velaclaw-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-velaclaw-message-channel")) ??
