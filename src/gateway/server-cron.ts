...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.VELACLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: VelaclawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: VelaclawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: VelaclawConfig, requestedAgentId: string) => {
