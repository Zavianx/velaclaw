...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    const testsWithoutDefaultServer = new Set([
      ...
      "prefers VELACLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string",
    ]);
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers VELACLAW_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.VELACLAW_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.VELACLAW_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.VELACLAW_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.VELACLAW_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.VELACLAW_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.VELACLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            VELACLAW_VERSION: " ",
            VELACLAW_SERVICE_VERSION: "2.4.6-service",
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `velaclaw-test-device-${randomUUID()}.json`),
        nonce,
      });
