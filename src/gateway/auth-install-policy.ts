...
type GatewayInstallAuthMode = NonNullable<NonNullable<VelaclawConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: VelaclawConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.VELACLAW_GATEWAY_PASSWORD) ||
