...
export function attachVelaclawTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__velaclaw && typeof record.__velaclaw === "object" && !Array.isArray(record.__velaclaw)
      ? (record.__velaclaw as Record<string, unknown>)
      : {};
  return {
    ...record,
    __velaclaw: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachVelaclawTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "velaclaw" && model === "delivery-mirror";
