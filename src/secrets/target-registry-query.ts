...
let compiledSecretTargetRegistryState: {
  ...
  velaClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  velaClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
} | null = null;

let compiledCoreVelaclawTargetState: {
  knownTargetIds: Set<string>;
  velaClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  velaClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledBundledChannelVelaclawTargets = new Map<
  string,
...
function getCompiledSecretTargetRegistryState() {
  ...
  const velaClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "velaclaw.json",
  );
  ...
  compiledSecretTargetRegistryState = {
    ...
    velaClawCompiledSecretTargets,
    velaClawTargetsById: buildConfigTargetIdIndex(velaClawCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(compiledSecretTargetRegistry),
  };
  return compiledSecretTargetRegistryState;
}

function getCompiledCoreVelaclawTargetState() {
  if (compiledCoreVelaclawTargetState) {
    return compiledCoreVelaclawTargetState;
  }
  const velaClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "velaclaw.json")
    .map(compileTargetRegistryEntry);
  compiledCoreVelaclawTargetState = {
    knownTargetIds: new Set(velaClawCompiledSecretTargets.map((entry) => entry.id)),
    velaClawCompiledSecretTargets,
    velaClawTargetsById: buildConfigTargetIdIndex(velaClawCompiledSecretTargets),
    targetsByType: buildTargetTypeIndex(velaClawCompiledSecretTargets),
  };
  return compiledCoreVelaclawTargetState;
}

function getCompiledBundledChannelVelaclawTargets(
  channelId: string,
): CompiledTargetRegistryEntry[] | null {
  ...
  if (compiledBundledChannelVelaclawTargets.has(normalizedChannelId)) {
    return compiledBundledChannelVelaclawTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadBundledChannelSecretContractApi(normalizedChannelId)
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "velaclaw.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledBundledChannelVelaclawTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreVelaclawTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreVelaclawTargetState().velaClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitBundledChannelEntries = explicitBundledChannelId
    ? getCompiledBundledChannelVelaclawTargets(explicitBundledChannelId)
    : null;
  ...
  for (const entry of getCompiledSecretTargetRegistryState().velaClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
    const matched = matchPathTokens(pathSegments, entry.pathTokens);
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreVelaclawTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreVelaclawTargetState()
      : getCompiledSecretTargetRegistryState();
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: registryState.velaClawCompiledSecretTargets,
    entriesById: registryState.velaClawTargetsById,
  });
