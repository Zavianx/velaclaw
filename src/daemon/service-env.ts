...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.VELACLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    VELACLAW_PROFILE: profile,
    VELACLAW_GATEWAY_PORT: String(port),
    VELACLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    VELACLAW_SYSTEMD_UNIT: systemdUnit,
    VELACLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    VELACLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    VELACLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    VELACLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.VELACLAW_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    VELACLAW_GATEWAY_TOKEN: gatewayToken,
    VELACLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    VELACLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    VELACLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    VELACLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    VELACLAW_LOG_PREFIX: "node",
    VELACLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    VELACLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    VELACLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    VELACLAW_STATE_DIR: sharedEnv.stateDir,
    VELACLAW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.VELACLAW_STATE_DIR;
  const configPath = env.VELACLAW_CONFIG_PATH;
