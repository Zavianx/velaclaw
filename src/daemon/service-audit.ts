...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds VELACLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `velaclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service VELACLAW_GATEWAY_TOKEN does not match gateway.auth.token in velaclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.VELACLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.VELACLAW_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `velaclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
