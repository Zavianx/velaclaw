...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<VelaclawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<VelaclawConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: VelaclawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: VelaclawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: VelaclawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: VelaclawConfig, result: VelaclawConfig): VelaclawConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: VelaclawConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as VelaclawConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with velaclaw configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Velaclaw will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.velaclaw.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Velaclaw will store a reference instead of the API key.",
