...
export type PromptDefaultModelResult = { model?: string; config?: VelaclawConfig };
...
function resolveConfiguredModelRaw(cfg: VelaclawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: VelaclawConfig): string[] {
...
export function applyModelAllowlist(cfg: VelaclawConfig, models: string[]): VelaclawConfig {
