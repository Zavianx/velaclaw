...
export class VelaclawChannelBridge {
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClient({
      ...
      clientDisplayName: "Velaclaw MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`velaclaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `velaclaw mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
