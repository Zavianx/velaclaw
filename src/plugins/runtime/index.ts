...
type TtsRuntimeStubFn = (...args: unknown[]) => unknown;
type TtsRuntimeStubVoicesFn = (...args: unknown[]) => unknown[];
const ttsUnavailable: TtsRuntimeStubFn = () => {
  throw new Error("TTS runtime unavailable (speech subsystem removed)");
};
const ttsUnavailableVoices: TtsRuntimeStubVoicesFn = () => [];
const loadTtsRuntime = createLazyRuntimeModule<{
  textToSpeech: TtsRuntimeStubFn;
  textToSpeechTelephony: TtsRuntimeStubFn;
  listSpeechVoices: TtsRuntimeStubVoicesFn;
}>(() =>
  Promise.resolve({
    textToSpeech: ttsUnavailable,
    textToSpeechTelephony: ttsUnavailable,
    listSpeechVoices: ttsUnavailableVoices,
  }),
);
...
const GATEWAY_SUBAGENT_SYMBOL: unique symbol = Symbol.for(
  "velaclaw.plugin.gatewaySubagentRuntime",
...
export function createPluginRuntime(_options: CreatePluginRuntimeOptions = {}): PluginRuntime {
  ...
  const runtime = {
    // Sourced from the shared Velaclaw version resolver (#52899) so plugins
