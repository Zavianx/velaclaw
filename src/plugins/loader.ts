...
import { clearRegisteredTeamAssetTypes } from "../velaclaw/asset-types.js";
...
import { discoverVelaclawPlugins } from "./discovery.js";
...
import type { VelaclawPluginDefinition, VelaclawPluginModule, PluginLogger } from "./types.js";

export type PluginLoadResult = PluginRegistry;

export type PluginLoadOptions = {
  config?: VelaclawConfig;
  activationSourceConfig?: VelaclawConfig;
...
function shouldProfilePluginLoader(): boolean {
  return process.env.VELACLAW_PLUGIN_LOAD_PROFILE === "1";
}
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadVelaclawPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadVelaclawPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: VelaclawPluginDefinition;
  register?: VelaclawPluginDefinition["register"];
} {
  const resolved = unwrapDefaultModuleExport(moduleExport);
  if (typeof resolved === "function") {
    return {
      register: resolved as VelaclawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as VelaclawPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.VELACLAW_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverVelaclawPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverVelaclawPlugins>["candidates"][number];
  right: ReturnType<typeof discoverVelaclawPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadVelaclawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadVelaclawPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  try {
    ...
    const discovery = discoverVelaclawPlugins({
      workspaceDir: options.workspaceDir,
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into Velaclaw yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: VelaclawPluginModule | null = null;
      try {
        ...
        mod = profilePluginLoaderSync({
          ...
          pluginId: record.id,
          source: safeSource,
          run: () => getJiti(safeSource)(safeImportSource) as VelaclawPluginModule,
        });
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: VelaclawPluginModule | null = null;
            try {
              runtimeMod = profilePluginLoaderSync({
                ...
                run: () =>
                  getJiti(safeRuntimeSource)(safeRuntimeImportSource) as VelaclawPluginModule,
              });
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'velaclaw plugins list' for details.`,
        );
      }
    }
...
export async function loadVelaclawPluginCliRegistry(
  options: PluginLoadOptions = {},
