...
import type {
  VelaclawPluginHookOptions,
  VelaclawPluginToolContext,
  VelaclawPluginToolFactory,
  VelaclawPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type VelaclawPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<VelaclawConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Velaclaw uses this to expose
 ...
 *
 * Runs after Velaclaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Velaclaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Velaclaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Velaclaw builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Velaclaw no
...
/**
 * Provider-owned stream wrapper hook after Velaclaw applies its generic
 ...
 *
 * Return `true` or `false` to override Velaclaw's built-in provider cache TTL
 ...
 *
 * Runs only after Velaclaw exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Velaclaw should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Velaclaw loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Velaclaw falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Velaclaw calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Velaclaw performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Velaclaw registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Velaclaw wrappers.
   *
   ...
   *
   * Called after Velaclaw resolves the raw configured credential but before the
   ...
   * hint than Velaclaw's generic auth-store guidance.
   */
   ...
   * Velaclaw's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Velaclaw should surface a provider-specific hint for
   ...
   * merging. Velaclaw deduplicates by `provider/id`, so plugins only need to
   ...
   * without replacing the full Velaclaw-owned system prompt.
   */
   ...
   * rewrites after Velaclaw has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Velaclaw handles
   ...
   *
   * Velaclaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `velaclaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `velaclaw doctor` should offer.
   *
   ...
   *
   * Velaclaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Velaclaw's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Velaclaw falls back to that stored profile.
   */
...
export type VelaclawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Velaclaw session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Velaclaw configuration */
  config: VelaclawConfig;
 ...
 */
export type VelaclawPluginCommandDefinition = {
...
export type VelaclawPluginHttpRouteAuth = "gateway" | "plugin";
export type VelaclawPluginHttpRouteMatch = "exact" | "prefix";
export type VelaclawPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type VelaclawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type VelaclawPluginHttpRouteParams = {
  path: string;
  handler: VelaclawPluginHttpRouteHandler;
  auth: VelaclawPluginHttpRouteAuth;
  match?: VelaclawPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: VelaclawPluginGatewayRuntimeScopeSurface;
  replaceExisting?: boolean;
};

export type VelaclawPluginCliContext = {
  program: Command;
  config: VelaclawConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type VelaclawPluginCliRegistrar = (ctx: VelaclawPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Velaclaw to keep a plugin command lazy-loaded while still
 ...
 */
export type VelaclawPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type VelaclawPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type VelaclawPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type VelaclawPluginSecurityAuditContext = {
  config: VelaclawConfig;
...
export type VelaclawPluginSecurityAuditCollector = (
  ctx: VelaclawPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/** Context passed to long-lived plugin services. */
export type VelaclawPluginServiceContext = {
  config: VelaclawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type VelaclawPluginService = {
  id: string;
  start: (ctx: VelaclawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: VelaclawPluginServiceContext) => void | Promise<void>;
};

export type VelaclawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type VelaclawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: VelaclawPluginReloadRegistration;
  nodeHostCommands?: VelaclawPluginNodeHostCommand[];
  securityAuditCollectors?: VelaclawPluginSecurityAuditCollector[];
  register?: (api: VelaclawPluginApi) => void | Promise<void>;
  activate?: (api: VelaclawPluginApi) => void | Promise<void>;
};

export type VelaclawPluginModule =
  | VelaclawPluginDefinition
  | ((api: VelaclawPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: VelaclawConfig) =>
  | {
      config: VelaclawConfig;
      changes: string[];
    }
...
export type VelaclawPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | VelaclawPluginToolFactory,
    opts?: VelaclawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: VelaclawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: VelaclawPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: VelaclawPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: VelaclawPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Velaclaw can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: VelaclawPluginCliCommandDescriptor[];
    },
  ) => void;
  registerReload: (registration: VelaclawPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: VelaclawPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: VelaclawPluginSecurityAuditCollector) => void;
  registerService: (service: VelaclawPluginService) => void;
   ...
   */
  registerCommand: (command: VelaclawPluginCommandDefinition) => void;
