...
import type {
  ...
  VelaclawPluginApi,
  VelaclawPluginChannelRegistration,
  VelaclawPluginCliCommandDescriptor,
  VelaclawPluginCliRegistrar,
  VelaclawPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  VelaclawPluginGatewayRuntimeScopeSurface,
  VelaclawPluginHttpRouteParams,
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("velaclaw.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: VelaclawPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: VelaclawPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: VelaclawPluginHookOptions | undefined,
    config: VelaclawPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: VelaclawPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: VelaclawPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as VelaclawPluginChannelRegistration).plugin === "object"
        ? (registration as VelaclawPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: VelaclawPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerReload = (record: PluginRecord, registration: VelaclawPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: VelaclawPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: VelaclawPluginNodeHostCommand,
  ) => {
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: VelaclawPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: VelaclawPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: VelaclawPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationMode === "full"
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<VelaclawPluginApi["registerCompactionProvider"]>[0],
              ) => {
