...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "velaclaw skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Velaclaw runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Velaclaw >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
    ...
    return {
      ...
      clawhub: {
        ...
        clawhubUrl:
          ...
          normalizeOptionalString(process.env.VELACLAW_CLAWHUB_URL) ||
          "https://clawhub.ai",
