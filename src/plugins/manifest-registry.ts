...
import { discoverVelaclawPlugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.VELACLAW_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.VELACLAW_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  channelConfigs?: Record<string, PluginManifestChannelConfig>;
  packageChannel?: VelaclawPackageManifest["channel"];
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "velaclaw",
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "velaclaw") === "bundle";
    const manifestRes:
        ...
        message:
          minHostVersionCheck.kind === "invalid"
            ...
            : minHostVersionCheck.kind === "unknown_host_version"
              ? `plugin requires Velaclaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
              : `plugin requires Velaclaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
      });
      continue;
    }
