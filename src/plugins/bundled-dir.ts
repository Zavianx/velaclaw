...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "velaclaw-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.VELACLAW_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "velaclaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.VELACLAW_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveVelaclawPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveVelaclawPackageRootSync({ argv1: process.argv[1] }),
      resolveVelaclawPackageRootSync({ cwd: process.cwd() }),
      resolveVelaclawPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
