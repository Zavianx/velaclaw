...
function hasTrustedVelaclawRootIndicator(params: {
  packageRoot: string;
  ...
  const hasVelaclawBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("velaclaw")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.velaclaw === "string");
  const hasVelaclawEntrypoint = fs.existsSync(path.join(params.packageRoot, "velaclaw.mjs"));
  return hasCliEntryExport || hasVelaclawBin || hasVelaclawEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedVelaclawRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedVelaclawRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveVelaclawPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedVelaclawRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveVelaclawPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedVelaclawRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["velaclaw/plugin-sdk", "@velaclaw/plugin-sdk"] as const;
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.VELACLAW_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `velaclaw/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? { "velaclaw/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.velaClawPackageRoot, "extensions"),
    path.join(params.velaClawPackageRoot, "dist", "extensions"),
    path.join(params.velaClawPackageRoot, "dist-runtime", "extensions"),
