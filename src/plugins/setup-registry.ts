...
function collectConfiguredPluginEntryIds(config: VelaclawConfig): string[] {
...
function resolveRegister(mod: VelaclawPluginModule): {
...
export function resolvePluginSetupRegistry(params?: {
  ...
  for (const record of manifestRegistry.plugins) {
    ...
    let mod: VelaclawPluginModule;
    try {
      mod = getJiti(setupSource)(setupSource) as VelaclawPluginModule;
    } catch {
      continue;
    }

    const resolved = resolveRegister((mod as { default?: VelaclawPluginModule }).default ?? mod);
