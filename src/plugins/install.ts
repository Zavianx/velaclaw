...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing velaclaw.extensions; update the plugin package to include velaclaw.extensions (for example ["./dist/index.js"]). See https://docs.velaclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-velaclaw-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "velaclaw.plugin.json",
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_VELACLAW_EXTENSIONS: "missing_velaclaw_extensions",
  EMPTY_VELACLAW_EXTENSIONS: "empty_velaclaw_extensions",
...
function ensureVelaclawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_VELACLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json velaclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_VELACLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "velaclaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureVelaclawExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureVelaclawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from velaclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-velaclaw"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json velaclaw.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Velaclaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set VELACLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Velaclaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Velaclaw and retry.`,
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "velaclaw-npm-pack-",
    spec,
    timeoutMs,
