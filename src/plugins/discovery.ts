...
export type PluginCandidate = {
  ...
  packageDir?: string;
  packageManifest?: VelaclawPackageManifest;
...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.VELACLAW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.VELACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @velaclaw/voice-call -> voice-call).
...
export function discoverVelaclawPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
  ...
  if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
    // Keep workspace auto-discovery constrained to the Velaclaw extensions root.
