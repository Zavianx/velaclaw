...
export const PLUGIN_MANIFEST_FILENAME = "velaclaw.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `velaclaw qa`, for example `matrix`. */
  commandName: string;
...
// package.json "velaclaw" metadata (used for setup/catalog)
...
export type VelaclawPackageStartup = {
  /**
...
export type VelaclawPackageManifest = {
  extensions?: string[];
  setupEntry?: string;
  channel?: PluginPackageChannel;
  install?: PluginPackageInstall;
  startup?: VelaclawPackageStartup;
};
...
export type PackageManifest = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, VelaclawPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): VelaclawPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
