...
import { loadVelaclawPluginCliRegistry, loadVelaclawPlugins } from "./loader.js";
...
export type PluginCliCommandGroupEntry = {
  pluginId: string;
  placeholders: readonly VelaclawPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: VelaclawPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadVelaclawPluginCliRegistry(
...
export async function loadPluginCliCommandRegistryWithContext(params: {
  ...
  const runtimeRegistry = loadVelaclawPlugins(
  ...
  try {
    const metadataRegistry = await loadVelaclawPluginCliRegistry(
      buildPluginCliLoaderParams(
        params.context,
        { primaryCommand: params.primaryCommand },
        params.loaderOptions,
...
): Promise<VelaclawPluginCliCommandDescriptor[]> {
  try {
