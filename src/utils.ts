...
export function resolveConfigDir(
  ...
  const configPath = env.VELACLAW_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".velaclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.VELACLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$VELACLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via VELACLAW_STATE_DIR.
