...
type AgentEntry = NonNullable<NonNullable<VelaclawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: VelaclawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: VelaclawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: VelaclawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: VelaclawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: VelaclawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
