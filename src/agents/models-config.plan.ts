...
type ModelsConfig = NonNullable<VelaclawConfig["models"]>;
...
function resolveExplicitBaseUrlProviders(
  providers: VelaclawConfig["models"] | undefined,
): ReadonlySet<string> {
  return new Set(
...
export async function planVelaclawModelsJsonWithDeps(
  params: {
    cfg: VelaclawConfig;
    sourceConfigForSecrets?: VelaclawConfig;
    agentDir: string;
    env: NodeJS.ProcessEnv;
    existingRaw: string;
    existingParsed: unknown;
...
export async function planVelaclawModelsJson(
  params: Parameters<typeof planVelaclawModelsJsonWithDeps>[0],
): Promise<ModelsJsonPlan> {
  return planVelaclawModelsJsonWithDeps(params);
}
