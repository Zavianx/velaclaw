...
import { planVelaclawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: VelaclawConfig): {
  config: VelaclawConfig;
  sourceConfigForSecrets: VelaclawConfig;
} {
...
export async function ensureVelaclawModelsJson(
  config?: VelaclawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveVelaclawAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planVelaclawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
