...
import { markVelaclawExecEnv } from "../../infra/velaclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "velaclaw.sandbox=1");
  args.push("--label", `velaclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `velaclaw.createdAtMs=${createdAtMs}`);
  args.push("--label", `velaclaw.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `velaclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markVelaclawExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "velaclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`velaclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`velaclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("velaclaw sandbox recreate --all");
}
