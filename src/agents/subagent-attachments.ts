...
function resolveAttachmentLimits(config: VelaclawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".velaclaw", "attachments");
  const relDir = path.posix.join(".velaclaw", "attachments", attachmentId);
