...
function createOpenAIResponsesClient(
  ...
  return new OpenAI({
    ...
    defaultHeaders: applyNonOfficialOpenAiHeaderCleanup(
      model.baseUrl,
      buildOpenAIClientHeaders(model, context, optionHeaders, turnHeaders),
    ),
    fetch: buildGuardedModelFetch(model),
  });
}

// Some OpenAI-compatible relays (saymycode, custom Cloudflare frontends, etc.)
// block the vanilla `OpenAI/JS` User-Agent + Stainless telemetry headers that
// the SDK hard-codes. When the baseUrl points anywhere other than OpenAI
// official, strip those fingerprint headers and replace the UA with a neutral
// one so the SDK traffic looks the same as plain `fetch`/`undici`.
function applyNonOfficialOpenAiHeaderCleanup(
  baseUrl: string | undefined,
  headers: Record<string, string>,
): Record<string, string | null> {
  const isOfficial = typeof baseUrl === "string" && /^https:\/\/api\.openai\.com\//i.test(baseUrl);
  if (isOfficial) {
    return headers;
  }
...
function createOpenAICompletionsClient(
  ...
  return new OpenAI({
    ...
    defaultHeaders: applyNonOfficialOpenAiHeaderCleanup(
      model.baseUrl,
      buildOpenAIClientHeaders(model, context, optionHeaders),
    ),
    fetch: buildGuardedModelFetch(model),
  });
}
