...
const prepareDeps = {
  ...
  createMcpLoopbackServerConfig,
  resolveVelaclawDocsPath: async (
    params: Parameters<typeof import("../docs-path.js").resolveVelaclawDocsPath>[0],
  ) => (await import("../docs-path.js")).resolveVelaclawDocsPath(params),
};
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          VELACLAW_MCP_TOKEN: mcpLoopbackRuntime.token,
          VELACLAW_MCP_AGENT_ID: sessionAgentId ?? "",
          VELACLAW_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          VELACLAW_MCP_SESSION_KEY: params.sessionKey ?? "",
          VELACLAW_MCP_MESSAGE_CHANNEL: params.messageProvider ?? "",
          VELACLAW_MCP_SENDER_IS_OWNER: params.senderIsOwner === true ? "true" : "false",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const docsPath = await prepareDeps.resolveVelaclawDocsPath({
    workspaceDir,
