...
const VELACLAW_CLAUDE_PLUGIN_NAME = "velaclaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, VELACLAW_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: VELACLAW_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Velaclaw skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
