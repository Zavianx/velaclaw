...
import { KeyedAsyncQueue } from "velaclaw/plugin-sdk/keyed-async-queue";
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".velaclaw-cli-images");
  }
  return path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredVelaclawTmpDir(), "velaclaw-cli-system-prompt-"),
  );
