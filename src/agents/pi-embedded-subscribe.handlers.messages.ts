...
function isTranscriptOnlyVelaclawAssistantMessage(message: AgentMessage | undefined): boolean {
  if (!message || message.role !== "assistant") {
    return false;
  }
  const provider = normalizeOptionalString(message.provider) ?? "";
  const model = normalizeOptionalString(message.model) ?? "";
  return provider === "velaclaw" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function handleMessageStart(
  ...
  if (msg?.role !== "assistant" || isTranscriptOnlyVelaclawAssistantMessage(msg)) {
    return;
  }
