...
import { resolveVelaclawMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: VelaclawConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "velaclaw-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "velaclaw-bundled") {
    return {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "velaclaw-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "velaclaw-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveVelaclawMetadata(frontmatter),
      invocation,
      exposure: {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`velaclaw skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`velaclaw skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
