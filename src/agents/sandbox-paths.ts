...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const velaClawTmpDir = path.resolve(resolvePreferredVelaclawTmpDir());
  if (!isPathInside(velaClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: velaClawTmpDir });
  return resolved;
}
