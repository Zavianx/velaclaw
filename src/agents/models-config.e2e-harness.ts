...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousVelaclawAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousVelaclawAgentDir = process.env.VELACLAW_AGENT_DIR;
    previousPiCodingAgentDir = process.env.PI_CODING_AGENT_DIR;
    delete process.env.VELACLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousVelaclawAgentDir === undefined) {
      delete process.env.VELACLAW_AGENT_DIR;
    } else {
      process.env.VELACLAW_AGENT_DIR = previousVelaclawAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "VELACLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.VELACLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.VELACLAW_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.VELACLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: VelaclawConfig = {
  models: {
    providers: {
      "custom-proxy": {
