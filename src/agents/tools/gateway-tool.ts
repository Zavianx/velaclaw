...
import { isVelaclawOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `velaclaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as VelaclawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as VelaclawConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isVelaclawOwnerOnlyCoreToolName("gateway"),
    description:
