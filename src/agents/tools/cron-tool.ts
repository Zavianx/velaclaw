...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  return {
    label: "Cron",
    name: "cron",
    ownerOnly: isVelaclawOwnerOnlyCoreToolName("cron"),
...
Main-session cron jobs enqueue system events for heartbeat handling. Isolated cron jobs create background task runs that appear in \`velaclaw tasks\`.
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "add": {
          ...
          // See: https://github.com/Zavianx/velaclaw-dev/issues/11310
          if (
            !params.job ||
