...
export function setTempStateDir(workspaceDir: string): string {
  ...
  process.env.VELACLAW_STATE_DIR = stateDir;
  return stateDir;
}
...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("velaclaw-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "velaclaw-skills-install-"));
  try {
