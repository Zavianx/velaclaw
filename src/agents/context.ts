...
function isLikelyVelaclawCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "velaclaw" ||
    entryBasename === "velaclaw.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Velaclaw CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureVelaclawModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyVelaclawCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): VelaclawConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureVelaclawModelsJson(cfg);
    } catch {
