...
import { resolveVelaclawDocsPath } from "../../docs-path.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveVelaclawAgentDir();
    ...
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createVelaclawCodingTools({
            agentId: sessionAgentId,
    ...
    const docsPath = await resolveVelaclawDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/Zavianx/velaclaw-dev/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("velaclaw:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/Zavianx/velaclaw-dev/issues/8643
