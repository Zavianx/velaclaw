...
const OPENAI_CODEX_API_KEY_FALLBACK_MODEL_IDS = new Set([
  "gpt-5.4",
  "gpt-5.4-pro",
  "gpt-5.4-mini",
  "gpt-5.2",
...
function resolveProviderApiKeyFallback(params: {
  provider: string;
  modelId: string;
  cfg?: VelaclawConfig;
}): string {
  ...
  if (!OPENAI_CODEX_API_KEY_FALLBACK_MODEL_IDS.has(normalizedModelId)) {
    return params.provider;
  }
  const codexAuthMode = resolveModelAuthMode("openai-codex", params.cfg);
  if (codexAuthMode && codexAuthMode !== "unknown") {
    return params.provider;
  }
  const openAiAuthMode = resolveModelAuthMode("openai", params.cfg);
  if (openAiAuthMode === "api-key" || openAiAuthMode === "mixed") {
    return "openai";
  }
  return params.provider;
}
...
export function resolveModel(
  ...
  const effectiveProvider = resolveProviderApiKeyFallback({
    provider,
    modelId,
    cfg,
  });
  const normalizedRef = {
    ...
    model: normalizeStaticProviderModelId(normalizeProviderId(effectiveProvider), modelId),
  };
  const resolvedAgentDir = agentDir ?? resolveVelaclawAgentDir();
 ...
 *
 * See: https://github.com/Zavianx/velaclaw-dev/issues/17328
 */
