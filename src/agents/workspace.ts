...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.VELACLAW_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".velaclaw", `workspace-${profile}`);
  }
  return path.join(home, ".velaclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".velaclaw";
