...
import { createVelaclawTools } from "./velaclaw-tools.js";
...
function resolveExecConfig(params: { cfg?: VelaclawConfig; agentId?: string }) {
...
export function createVelaclawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createVelaclawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
