...
const GOOGLE_LIVE_SINGLE_KEY = "VELACLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "VELACLAW_LIVE_ANTHROPIC_KEY",
    listVar: "VELACLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "VELACLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `VELACLAW_LIVE_${base}_KEY`;
