...
const VELACLAW_ATTRIBUTION_PRODUCT = "Velaclaw";
const VELACLAW_ATTRIBUTION_ORIGINATOR = "velaclaw";
...
function formatVelaclawUserAgent(version: string): string {
  return `${VELACLAW_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: VELACLAW_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in Velaclaw runtime wrapper.",
    ...identity,
    headers: {
      "HTTP-Referer": "https://velaclaw.ai",
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: VELACLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatVelaclawUserAgent(identity.version),
    },
  };
}
