...
const SKIP_DELETE_CONFIRM_KEY = "velaclaw:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__velaclaw__|media)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__velaclaw__/assistant-media?${params.toString()}`;
}
