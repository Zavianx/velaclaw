const SETTINGS_KEY_PREFIX = "velaclaw.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "velaclaw.control.settings.v1";
const LEGACY_TOKEN_SESSION_KEY = "velaclaw.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "velaclaw.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window.__VELACLAW_CONTROL_UI_BASE_PATH__);
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("velaclaw.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
