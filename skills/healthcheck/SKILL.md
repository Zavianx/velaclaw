---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Velaclaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Velaclaw cron scheduling for periodic checks, or version status checks on a machine running Velaclaw (laptop, workstation, Pi, VPS).
---

# Velaclaw Host Hardening

## Overview

Assess and harden the host running Velaclaw, then align it to a user-defined risk tolerance without breaking access. Use Velaclaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Velaclaw changes the host firewall, SSH, or OS updates; it does not.
...
5. Velaclaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Velaclaw can access sensitive data.
...
### 2) Run Velaclaw security audits (read-only)

As part of the default read-only checks, run `velaclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `velaclaw security audit` (faster, non-probing)
2. `velaclaw security audit --json` (structured output)

Offer to apply Velaclaw safe defaults (numbered):

1. `velaclaw security audit --fix`

Be explicit that `--fix` only tightens Velaclaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Velaclaw version/update status (read-only)

As part of the default read-only checks, run `velaclaw update status`.
...
- Credential hygiene notes (location of Velaclaw creds, prefer disk encryption)
...
- Velaclaw security audit (re-run)
...
After Velaclaw install or first hardening pass, run at least one baseline audit and version check:

- `velaclaw security audit`
- `velaclaw security audit --deep`
- `velaclaw update status`

Ongoing monitoring is recommended. Use the Velaclaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `velaclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `velaclaw update status`
...
Before creating, `velaclaw cron list` and match on exact `name`. If found, `velaclaw cron edit <id> ...`.
If not found, `velaclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `velaclaw update status` (preferred for source checkouts and channels)
2. `npm view velaclaw version` (published npm version)

## Velaclaw command accuracy

Use only supported commands and flags:

- `velaclaw security audit [--deep] [--fix] [--json]`
- `velaclaw status` / `velaclaw status --deep`
- `velaclaw health --json`
- `velaclaw update status`
- `velaclaw cron add|list|runs|run`

Do not invent CLI flags or imply Velaclaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Velaclaw compaction:
