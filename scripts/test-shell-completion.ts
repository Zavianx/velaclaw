 ...
 * `velaclaw update`. Use it to verify the completion installation flow
...
\`velaclaw update\`, \`velaclaw doctor\`, and \`velaclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Velaclaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`velaclaw completion --install\` later to enable.`));
    return;
  }
