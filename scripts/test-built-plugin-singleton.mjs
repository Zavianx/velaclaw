...
const { clearPluginCommands, getPluginCommandSpecs, loadVelaclawPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadVelaclawPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "velaclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@velaclaw/build-smoke-plugin",
      type: "module",
      velaclaw: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "velaclaw.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'velaclaw/plugin-sdk';",
...
const registry = loadVelaclawPlugins({
  cache: false,
  workspaceDir: tempRoot,
  env: {
    ...process.env,
    VELACLAW_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
