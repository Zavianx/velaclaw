...
velaclaw_live_stage_source_tree() {
...
velaclaw_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"

  ln -s /app/node_modules "$dest_dir/node_modules"
  ln -s /app/dist "$dest_dir/dist"
  if [ -d /app/dist-runtime/extensions ]; then
    export VELACLAW_BUNDLED_PLUGINS_DIR=/app/dist-runtime/extensions
  elif [ -d /app/dist/extensions ]; then
    export VELACLAW_BUNDLED_PLUGINS_DIR=/app/dist/extensions
  fi
}

velaclaw_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.velaclaw"
  ...
  export VELACLAW_STATE_DIR="$dest_dir"
  export VELACLAW_CONFIG_PATH="$dest_dir/velaclaw.json"
}

velaclaw_live_prepare_staged_config() {
  if [ ! -f "${VELACLAW_CONFIG_PATH:-}" ]; then
    return 0
  fi
