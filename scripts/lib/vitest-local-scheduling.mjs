...
function isSystemThrottleDisabled(env) {
  const normalized = env.VELACLAW_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.VELACLAW_VITEST_MAX_WORKERS ?? env.VELACLAW_TEST_WORKERS);
  if (override !== null) {
