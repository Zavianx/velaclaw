...
export function createPluginSdkScope(_repoRoot: string): TopologyScope {
  const entrypoints = pluginSdkEntrypoints.map((entrypoint) => ({
    ...
    importSpecifier:
      entrypoint === "index" ? "velaclaw/plugin-sdk" : `velaclaw/plugin-sdk/${entrypoint}`,
  }));
  return buildScopeFromEntrypoints("plugin-sdk", "Velaclaw plugin-sdk public surface", entrypoints);
}
