...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "velaclaw-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "velaclaw.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".velaclaw", "workspace");
