...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.velaclaw?.extensions)
    ? packageJson.velaclaw.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.velaclaw?.setupEntry === "string" &&
    packageJson.velaclaw.setupEntry.trim().length > 0
      ? packageJson.velaclaw.setupEntry
      : undefined;
  if (setupEntry) {
...
function shouldStageBundledPluginRuntimeDependencies(packageJson) {
  return packageJson?.velaclaw?.bundle?.stageRuntimeDependencies === true;
}
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "velaclaw.plugin.json"));
    }
    if (hasPackageJson) {
