...
// Usage:
// VELACLAW_PROMPT_TRANSPORT=direct|gateway
// VELACLAW_PROMPT_MODE=extra|override
// VELACLAW_PROMPT_TEXT='...'
// VELACLAW_PROMPT_CAPTURE=1
...
import { resolveVelaclawAgentDir } from "../src/agents/agent-paths.js";
...
const TRANSPORT = process.env.VELACLAW_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.VELACLAW_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.VELACLAW_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.VELACLAW_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.VELACLAW_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = process.env.VELACLAW_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.VELACLAW_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.VELACLAW_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.VELACLAW_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.VELACLAW_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
const SETUP_TOKEN_RAW = process.env.VELACLAW_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.VELACLAW_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.VELACLAW_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const DIRECT_CLAUDE_ARGS = ["-p", "--append-system-prompt"];

if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing VELACLAW_PROMPT_TEXT or VELACLAW_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  const agentDir = resolveVelaclawAgentDir();
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set VELACLAW_LIVE_SETUP_TOKEN_VALUE or VELACLAW_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "velaclaw-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["velaclaw.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        VELACLAW_CONFIG_PATH: params.configPath,
        VELACLAW_STATE_DIR: params.stateDir,
        VELACLAW_AGENT_DIR: params.agentDir,
        VELACLAW_GATEWAY_TOKEN: params.gatewayToken,
        VELACLAW_SKIP_CHANNELS: "1",
        VELACLAW_SKIP_GMAIL_WATCHER: "1",
        VELACLAW_SKIP_CANVAS_HOST: "1",
        VELACLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
        VELACLAW_DISABLE_BONJOUR: "1",
        VELACLAW_SKIP_CRON: "1",
        VELACLAW_TEST_MINIMAL_GATEWAY: "1",
        VELACLAW_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "velaclaw-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "velaclaw.json");
