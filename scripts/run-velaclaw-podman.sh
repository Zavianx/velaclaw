#!/usr/bin/env bash
# Rootless Velaclaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-velaclaw-podman.sh launch        # Start gateway
#   ./scripts/run-velaclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   velaclaw --container velaclaw dashboard --no-open
#   velaclaw --container velaclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      VELACLAW_GATEWAY_TOKEN|VELACLAW_PODMAN_CONTAINER|VELACLAW_PODMAN_IMAGE|VELACLAW_IMAGE|VELACLAW_PODMAN_PULL|VELACLAW_PODMAN_GATEWAY_HOST_PORT|VELACLAW_GATEWAY_PORT|VELACLAW_PODMAN_BRIDGE_HOST_PORT|VELACLAW_BRIDGE_PORT|VELACLAW_GATEWAY_BIND|VELACLAW_PODMAN_USERNS|VELACLAW_BIND_MOUNT_OPTIONS|VELACLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-velaclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${VELACLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.velaclaw}"
ENV_FILE="${VELACLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${VELACLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.velaclaw}"
ENV_FILE="${VELACLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${VELACLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${VELACLAW_PODMAN_CONTAINER:-velaclaw}"
VELACLAW_IMAGE="${VELACLAW_PODMAN_IMAGE:-${VELACLAW_IMAGE:-velaclaw:local}}"
PODMAN_PULL="${VELACLAW_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${VELACLAW_PODMAN_GATEWAY_HOST_PORT:-${VELACLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${VELACLAW_PODMAN_BRIDGE_HOST_PORT:-${VELACLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${VELACLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$VELACLAW_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v velaclaw >/dev/null 2>&1; then
    return 0
  fi
  VELACLAW_CONTAINER="" VELACLAW_CONFIG_DIR="$config_dir" \
    velaclaw config get gateway.bind 2>/dev/null || true
}
...
# VELACLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${VELACLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'VELACLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v velaclaw >/dev/null 2>&1; then
    echo "Warning: velaclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    VELACLAW_CONTAINER="" VELACLAW_CONFIG_DIR="$config_dir" \
      velaclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    VELACLAW_CONTAINER="" VELACLAW_CONFIG_DIR="$config_dir" \
      velaclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  VELACLAW_CONTAINER="" VELACLAW_CONFIG_DIR="$config_dir" \
    velaclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${VELACLAW_GATEWAY_TOKEN:-}" ]]; then
  export VELACLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "VELACLAW_GATEWAY_TOKEN" "$VELACLAW_GATEWAY_TOKEN"
  echo "Generated VELACLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/velaclaw.json"
...
PODMAN_USERNS="${VELACLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported VELACLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (VELACLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${VELACLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${VELACLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$VELACLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.velaclaw/.npm \
    -e VELACLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.velaclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.velaclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$VELACLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME velaclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME velaclaw devices approve --latest  # if pairing required"
