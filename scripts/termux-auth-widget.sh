...
# Velaclaw Auth Widget for Termux
...
SERVER="${VELACLAW_SERVER:-l36}"

# Check auth status
termux-toast "Checking Velaclaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/velaclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/velaclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|VELACLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/velaclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|VELACLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Velaclaw won't work until re-auth"
                ;;
        esac
        ;;
