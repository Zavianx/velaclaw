...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("velaclaw/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("velaclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced velaclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced velaclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
