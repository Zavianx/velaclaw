...
import type { VelaclawConfig } from "../src/config/config.js";
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.velaclaw?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.velaclaw?.channel?.order;
      const labelRaw = parsed.velaclaw?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".velaclaw-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "velaclaw-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "velaclaw-build",
    ...
    NO_COLOR: "1",
    VELACLAW_BUNDLED_PLUGINS_DIR: bundledPluginsDir,
    VELACLAW_DISABLE_BUNDLED_PLUGINS: "",
    VELACLAW_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
    VELACLAW_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
    VELACLAW_PLUGIN_DISCOVERY_CACHE_MS: "0",
    VELACLAW_PLUGIN_MANIFEST_CACHE_MS: "0",
    VELACLAW_STATE_DIR: stateDir,
  };
  const config: VelaclawConfig = {
    agents: {
      defaults: {
