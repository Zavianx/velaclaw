...
const extensionRestartMetadataFiles = new Set(["velaclaw.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.VELACLAW_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.VELACLAW_BUILD_PRIVATE_QA === "1" &&
...
const BUILD_REASON_LABELS = {
  force_build: "forced by VELACLAW_FORCE_BUILD",
...
const logRunner = (message, deps) => {
  if (deps.env.VELACLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[velaclaw] ${message}\n`);
};
...
const runVelaclaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["velaclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.VELACLAW_WATCH_MODE === "1";
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.VELACLAW_BUILD_PRIVATE_QA = "1";
    deps.env.VELACLAW_ENABLE_PRIVATE_QA_CLI = "1";
  }
