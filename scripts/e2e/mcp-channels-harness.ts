...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/velaclaw-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...process.env,
      VELACLAW_ALLOW_INSECURE_PRIVATE_WS: "1",
      VELACLAW_STATE_DIR: "/tmp/velaclaw-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[velaclaw mcp] ${String(chunk)}`);
  });
