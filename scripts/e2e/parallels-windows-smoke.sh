...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-velaclaw-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://velaclaw.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/velaclaw-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-velaclaw-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://velaclaw.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run velaclaw update --channel dev.
...
\$velaclaw = Join-Path \$env:APPDATA 'npm\velaclaw.cmd'
\$args = $args_literal
if ('${env_name_q}' -ne '') {
  Set-Item -Path ('Env:' + '${env_name_q}') -Value '${env_value_q}'
}
# velaclaw.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$velaclaw @args 2>&1
if (\$null -ne \$output) {
...
baseline_install_version() {
  ...
  npm view "velaclaw@$INSTALL_VERSION" version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "velaclaw-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_powershell "$(cat <<EOF
\$depsRoot = Join-Path \$env:LOCALAPPDATA 'Velaclaw\deps'
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/velaclaw-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
write_latest_install_runner_script() {
  ...
  WINDOWS_LATEST_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/velaclaw-install-latest.ps1"
...
try {
  ...
  & (Join-Path \$env:APPDATA 'npm\velaclaw.cmd') --version *>&1 | Tee-Object -FilePath \$LogPath -Append | Out-Null
  if (\$LASTEXITCODE -ne 0) {
    throw "velaclaw --version failed with exit code \$LASTEXITCODE"
  }
...
write_baseline_npm_install_runner_script() {
  WINDOWS_BASELINE_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/velaclaw-install-baseline-npm.ps1"
...
try {
  $portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Velaclaw\deps') 'portable-git') ''
  $env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
  $velaclaw = Join-Path $env:APPDATA 'npm\velaclaw.cmd'

  Write-ProgressLog 'install.start'
  Invoke-Logged 'npm install baseline release' {
    & npm.cmd install -g "velaclaw@$Version" --no-fund --no-audit --loglevel=error
  }

  Write-ProgressLog 'install.version'
  Invoke-Logged 'velaclaw --version' { & $velaclaw --version }
...
install_baseline_npm_release() {
  ...
  runner_name="velaclaw-install-baseline-$RANDOM-$RANDOM.ps1"
  log_name="velaclaw-install-baseline-$RANDOM-$RANDOM.log"
  done_name="velaclaw-install-baseline-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/velaclaw-install-baseline-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/velaclaw-install-baseline-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
...
install_latest_release() {
  ...
  runner_name="velaclaw-install-latest-$RANDOM-$RANDOM.ps1"
  log_name="velaclaw-install-latest-$RANDOM-$RANDOM.log"
  done_name="velaclaw-install-latest-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/velaclaw-install-latest-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
install_main_tgz() {
  ...
  runner_name="velaclaw-install-$RANDOM-$RANDOM.ps1"
  log_name="velaclaw-install-$RANDOM-$RANDOM.log"
  done_name="velaclaw-install-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/velaclaw-install-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/velaclaw-install-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
...
write_dev_update_runner_script() {
  WINDOWS_DEV_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/velaclaw-update-dev.ps1"
...
try {
  ...
  $velaclaw = Join-Path $env:APPDATA 'npm\velaclaw.cmd'
  $gitRoot = Join-Path $env:USERPROFILE 'velaclaw'
  $gitEntry = Join-Path $gitRoot 'velaclaw.mjs'
...
"@ | Set-Content -Path $pnpmPsShim -Encoding UTF8
  ...
  Invoke-Logged 'velaclaw update --channel dev --yes --json' {
    & $velaclaw update --channel dev --yes --json
  }
  ...
  Invoke-Logged 'git velaclaw --version' { & node.exe $gitEntry --version }
  Invoke-Logged 'git velaclaw update status --json' { & node.exe $gitEntry update status --json }
...
run_dev_channel_update() {
  ...
  runner_name="velaclaw-update-dev-$RANDOM-$RANDOM.ps1"
  log_name="velaclaw-update-dev-$RANDOM-$RANDOM.log"
  done_name="velaclaw-update-dev-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/velaclaw-update-dev-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
...
write_install_runner_script() {
  WINDOWS_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/velaclaw-install-main.ps1"
...
try {
  $env:PATH = "$env:LOCALAPPDATA\Velaclaw\deps\portable-git\cmd;$env:LOCALAPPDATA\Velaclaw\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\Velaclaw\deps\portable-git\usr\bin;$env:PATH"
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_velaclaw "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/velaclaw-onboard-$PROVIDER.ps1"
...
try {
  \$velaclaw = Join-Path \$env:APPDATA 'npm\velaclaw.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json > "{1}" 2>&1' -f \$velaclaw, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="velaclaw-onboard-$RANDOM-$RANDOM.ps1"
  log_name="velaclaw-onboard-$RANDOM-$RANDOM.log"
  done_name="velaclaw-onboard-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
verify_gateway() {
  guest_run_velaclaw "" "" gateway status --deep --require-rpc
}
...
verify_gateway_reachable() {
  ...
  for attempt in 1 2 3 4 5 6; do
    probe_json="$(
      guest_run_velaclaw "" "" gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json
    )"
    printf '%s\n' "$probe_json"
...
$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Velaclaw\deps') 'portable-git') ''
$env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
$gitEntry = Join-Path (Join-Path $env:USERPROFILE 'velaclaw') 'velaclaw.mjs'
if (-not (Test-Path $gitEntry)) {
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="velaclaw-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="velaclaw-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="velaclaw-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$velaclaw = Join-Path \$env:APPDATA 'npm\velaclaw.cmd'
  & \$velaclaw gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_velaclaw "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_velaclaw "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_velaclaw "" "" gateway status --deep
}

verify_turn() {
  guest_run_velaclaw "" "" models set "$MODEL_ID"
  guest_run_velaclaw "$API_KEY_ENV" "$API_KEY_VALUE" \
...
run_fresh_main_lane() {
  ...
  if phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "velaclaw-main-fresh.tgz"; then
    ...
    phase_run "fresh.install-main-retry" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "velaclaw-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-baseline-package" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "velaclaw-main-upgrade.tgz" || return $?
