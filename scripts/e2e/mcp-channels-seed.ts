...
async function main() {
  const stateDir = process.env.VELACLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".velaclaw");
  const configPath =
    process.env.VELACLAW_CONFIG_PATH?.trim() || path.join(stateDir, "velaclaw.json");
