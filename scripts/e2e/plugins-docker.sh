...
IMAGE_NAME="velaclaw-plugins-e2e"
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/velaclaw-plugins-run.XXXXXX.log")"
if ! docker run --rm "${DOCKER_ENV_ARGS[@]}" -i "$IMAGE_NAME" bash -s >"$RUN_LOG" 2>&1 <<'EOF'
set -euo pipefail

if [ -f dist/index.mjs ]; then
  VELACLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  VELACLAW_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export VELACLAW_ENTRY
...
home_dir=$(mktemp -d "/tmp/velaclaw-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
VELACLAW_PLUGIN_HOME="$HOME/.velaclaw/$BUNDLED_PLUGIN_ROOT_DIR"
...
const configPath = path.join(process.env.HOME, ".velaclaw", "velaclaw.json");
...
run_logged() {
  local label="$1"
  shift
  local log_file="/tmp/velaclaw-plugins-e2e-${label}.log"
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$VELACLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$VELACLAW_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$VELACLAW_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/velaclaw.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/velaclaw.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$VELACLAW_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/velaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$VELACLAW_ENTRY" plugins list --json > /tmp/plugins.json
node "$VELACLAW_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/velaclaw-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@velaclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "velaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/velaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
run_logged install-tgz node "$VELACLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$VELACLAW_ENTRY" plugins list --json > /tmp/plugins2.json
node "$VELACLAW_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/velaclaw-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@velaclaw/demo-plugin-dir",
  "version": "0.0.1",
  "velaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/velaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
run_logged install-dir node "$VELACLAW_ENTRY" plugins install "$dir_plugin"
node "$VELACLAW_ENTRY" plugins list --json > /tmp/plugins3.json
node "$VELACLAW_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/velaclaw-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@velaclaw/demo-plugin-file",
  "version": "0.0.1",
  "velaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/velaclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
run_logged install-file node "$VELACLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$VELACLAW_ENTRY" plugins list --json > /tmp/plugins4.json
node "$VELACLAW_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$VELACLAW_PLUGIN_HOME/$bundle_plugin_id"
...
gateway_log="/tmp/velaclaw-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/velaclaw-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@velaclaw/slash-install-plugin",
  "version": "0.0.1",
  "velaclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/velaclaw.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "velaclaw/fixture-marketplace"
    }
  }
}
JSON

node "$VELACLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
run_logged install-marketplace-shortcut node "$VELACLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
run_logged install-marketplace-direct node "$VELACLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$VELACLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$VELACLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$VELACLAW_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
run_logged update-marketplace-shortcut-dry-run node "$VELACLAW_ENTRY" plugins update marketplace-shortcut --dry-run
run_logged update-marketplace-shortcut node "$VELACLAW_ENTRY" plugins update marketplace-shortcut
node "$VELACLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$VELACLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
