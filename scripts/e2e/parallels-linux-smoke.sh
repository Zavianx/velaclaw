...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/velaclaw-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/velaclaw-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/velaclaw-install.sh
  guest_exec /usr/bin/env VELACLAW_NO_ONBOARD=1 bash /tmp/velaclaw-install.sh "${version_args[@]}" --no-onboard
  guest_exec velaclaw --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec velaclaw --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" velaclaw onboard \
    --non-interactive \
    --mode local \
...
set -euo pipefail
plugin_dir=/root/.velaclaw/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{
  "name": "@velaclaw/test-bad-plugin",
  "version": "1.0.0",
  "velaclaw": {
...
JSON
cat >"$plugin_dir/velaclaw.plugin.json" <<'JSON'
{
  "id": "test-bad-plugin",
  "configSchema": {
    "type": "object",
...
config_path = Path("/root/.Zavianx/velaclaw-dev.json")
config = {}
...
verify_bad_plugin_diagnostic() {
  guest_bash_script <<'EOF'
grep -F "failed to load setup entry" /tmp/velaclaw-parallels-linux-gateway.log
EOF
}
...
start_gateway_background() {
  ...
  cmd="$(cat <<EOF
pkill -f "velaclaw gateway run" >/dev/null 2>&1 || true
rm -f /tmp/velaclaw-parallels-linux-gateway.log
setsid sh -lc 'exec env VELACLAW_HOME=/root VELACLAW_STATE_DIR=/root/.velaclaw VELACLAW_CONFIG_PATH=/root/.Zavianx/velaclaw-dev.json ${API_KEY_ENV}=${api_key_value_q} velaclaw gateway run --bind loopback --port 18789 --force >/tmp/velaclaw-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec velaclaw gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec velaclaw gateway status --deep --require-rpc
    return
  fi
  guest_exec velaclaw gateway status --deep
}

verify_local_turn() {
  guest_exec velaclaw models set "$MODEL_ID"
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" velaclaw agent \
    --local \
    --agent main \
    --message ping \
    --json
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "velaclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "velaclaw-main-upgrade.tgz"
