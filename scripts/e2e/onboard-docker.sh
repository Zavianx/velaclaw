...
IMAGE_NAME="velaclaw-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/velaclaw-bin:$PATH"
  mkdir -p /tmp/velaclaw-bin
  cat > /tmp/velaclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/velaclaw-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$VELACLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_velaclaw_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/velaclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/velaclaw-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$VELACLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/velaclaw-e2e-$1.XXXXXX"
  }

  set_isolated_velaclaw_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export VELACLAW_HOME="$home_dir"
    export VELACLAW_STATE_DIR="$home_dir/.velaclaw"
    export VELACLAW_CONFIG_PATH="$VELACLAW_STATE_DIR/velaclaw.json"
    mkdir -p "$VELACLAW_STATE_DIR"
  }
  ...
  run_case_logged() {
    local label="$1"
    shift
    local log_path="/tmp/velaclaw-onboard-${label}.log"
  ...
  run_case_local_basic() {
    ...
    set_isolated_velaclaw_env "$home_dir"
    run_case_logged local-basic node "$VELACLAW_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$VELACLAW_STATE_DIR/workspace"
    config_path="$VELACLAW_CONFIG_PATH"
    sessions_dir="$VELACLAW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_velaclaw_env "$home_dir"
	    ...
	    run_case_logged remote-non-interactive node "$VELACLAW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
JSON
	    ...
	    run_case_logged reset-config node "$VELACLAW_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$VELACLAW_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$VELACLAW_ENTRY\" configure --section skills" send_skills_flow
