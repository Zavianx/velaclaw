...
IMAGE_NAME="${VELACLAW_IMAGE:-velaclaw-mcp-channels-e2e}"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="velaclaw-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t velaclaw-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "VELACLAW_SKIP_GMAIL_WATCHER=1" \
  -e "VELACLAW_SKIP_CRON=1" \
  -e "VELACLAW_SKIP_CANVAS_HOST=1" \
  -e "VELACLAW_STATE_DIR=/tmp/velaclaw-state" \
  -e "VELACLAW_CONFIG_PATH=/tmp/velaclaw-state/velaclaw.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "VELACLAW_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
