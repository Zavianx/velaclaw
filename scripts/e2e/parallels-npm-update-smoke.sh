...
UPDATE_TARGET=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/velaclaw-parallels-npm-update.XXXXXX)"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: velaclaw@latest
  --update-target <target>    Target passed to guest 'velaclaw update --tag'.
                             Default: host-served tgz packed from current checkout.
                             Examples: latest, beta, 2026.4.10, http://host/velaclaw.tgz
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/velaclaw-main-$CURRENT_HEAD_SHORT.tgz"
...
write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/velaclaw-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/velaclaw warnings do not
...
function Wait-GatewayRpcReady {
  param(
    [Parameter(Mandatory = $true)][string]$VelaclawPath,
    [int]$Attempts = 20,
    [int]$SleepSeconds = 3
  )
  ...
  for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
    ...
    try {
      Invoke-Logged 'velaclaw gateway status' { & $VelaclawPath gateway status --deep --require-rpc }
      return
    } catch {
...
function Stop-VelaclawGatewayProcesses {
  Write-ProgressLog 'update.stop-old-gateway'
  $patterns = @(
    'velaclaw-gateway',
    'velaclaw.*gateway --port 18789',
    'velaclaw.*gateway run',
    'velaclaw\.mjs gateway',
...
function Restart-GatewayWithRecovery {
  ...
  if ($null -ne $restartCompleted) {
    ...
    if ($restartResult.ExitCode -ne 0) {
      ...
      "velaclaw gateway restart failed with exit code $($restartResult.ExitCode)" | Tee-Object -FilePath $LogPath -Append | Out-Null
    }
  } else {
    $restartFailed = $true
    Stop-Job $restartJob -ErrorAction SilentlyContinue
    Write-ProgressLog 'update.restart-gateway.timeout'
    'velaclaw gateway restart timed out after 20s; continuing to RPC readiness checks' | Tee-Object -FilePath $LogPath -Append | Out-Null
  }
  Remove-Job $restartJob -Force -ErrorAction SilentlyContinue

  Write-ProgressLog 'update.gateway-status'
  try {
    Wait-GatewayRpcReady -VelaclawPath $VelaclawPath
    return
  } catch {
    ...
    Invoke-Logged 'velaclaw gateway start' { & $VelaclawPath gateway start }
...
try {
  ...
  Stop-VelaclawGatewayProcesses
  Write-ProgressLog 'update.velaclaw-update'
  Invoke-Logged 'velaclaw update' { & $velaclaw update --tag $UpdateTarget --yes --json }
  Write-ProgressLog 'update.verify-version'
  $version = Invoke-CaptureLogged 'velaclaw --version' { & $velaclaw --version }
  ...
  Invoke-Logged 'velaclaw update status' { & $velaclaw update status --json }
  Write-ProgressLog 'update.set-model'
  Invoke-Logged 'velaclaw models set' { & $velaclaw models set $ModelId }
  ...
  Restart-GatewayWithRecovery -VelaclawPath $velaclaw
  Write-ProgressLog 'update.agent-turn'
  Invoke-CaptureLogged 'velaclaw agent' { & $velaclaw agent --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/velaclaw-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
matches = [match for match in matches if re.search(r"Velaclaw \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="velaclaw-update-$RANDOM-$RANDOM.ps1"
  log_name="velaclaw-update-$RANDOM-$RANDOM.log"
  done_name="velaclaw-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/velaclaw-update-log-state.XXXXXX")"
  : >"$log_state_path"
...
run_macos_update() {
  ...
  cat <<EOF | prlctl exec "$MACOS_VM" /usr/bin/tee /tmp/velaclaw-main-update.sh >/dev/null
set -euo pipefail
...
cd "\$HOME"
stop_velaclaw_gateway_processes() {
  /opt/homebrew/bin/velaclaw gateway stop >/dev/null 2>&1 || true
  /usr/bin/pkill -9 -f velaclaw-gateway || true
  /usr/bin/pkill -9 -f 'velaclaw gateway run' || true
  /usr/bin/pkill -9 -f 'velaclaw.mjs gateway' || true
...
stop_velaclaw_gateway_processes
/opt/homebrew/bin/velaclaw update --tag "$update_target" --yes --json
...
stop_velaclaw_gateway_processes
version="\$(/opt/homebrew/bin/velaclaw --version)"
printf '%s\n' "\$version"
...
/opt/homebrew/bin/velaclaw update status --json
/opt/homebrew/bin/velaclaw models set "$MODEL_ID"
...
/opt/homebrew/bin/velaclaw gateway restart || true
gateway_ready=0
for _ in 1 2 3 4 5 6 7 8; do
  if /opt/homebrew/bin/velaclaw gateway status --deep --require-rpc >/dev/null 2>&1; then
    gateway_ready=1
    break
  fi
  sleep 2
...
if [ "\$gateway_ready" != "1" ]; then
  stop_velaclaw_gateway_processes
  /opt/homebrew/bin/velaclaw gateway run --bind loopback --port 18789 --force >/tmp/velaclaw-parallels-npm-update-macos-gateway.log 2>&1 </dev/null &
  for _ in 1 2 3 4 5 6 7 8; do
    if /opt/homebrew/bin/velaclaw gateway status --deep --require-rpc >/dev/null 2>&1; then
      gateway_ready=1
      break
...
if [ "\$gateway_ready" != "1" ]; then
  tail -n 120 /tmp/velaclaw-parallels-npm-update-macos-gateway.log 2>/dev/null || true
fi
/opt/homebrew/bin/velaclaw gateway status --deep --require-rpc
/opt/homebrew/bin/velaclaw agent --agent main --session-id parallels-npm-update-macos-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  macos_desktop_user_exec /bin/bash /tmp/velaclaw-main-update.sh
}
...
run_linux_update() {
  local update_target="$1"
  local expected_needle="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/velaclaw-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
stop_velaclaw_gateway_processes() {
  velaclaw gateway stop >/dev/null 2>&1 || true
  pkill -9 -f velaclaw-gateway || true
  pkill -9 -f 'velaclaw gateway run' || true
  pkill -9 -f 'velaclaw.mjs gateway' || true
...
stop_velaclaw_gateway_processes
velaclaw update --tag "$update_target" --yes --json
...
velaclaw update status --json
velaclaw models set "$MODEL_ID"
velaclaw agent --local --agent main --session-id parallels-npm-update-linux-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/velaclaw-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="velaclaw@$LATEST_VERSION"
fi
resolve_current_head
...
say "Run same-guest velaclaw update to $UPDATE_TARGET_EFFECTIVE"
