...
IMAGE_NAME="velaclaw-openwebui-e2e"
...
MODEL="${VELACLAW_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${VELACLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${VELACLAW_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${VELACLAW_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${VELACLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${VELACLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="velaclaw-openwebui-e2e-$$"
GW_NAME="velaclaw-openwebui-gateway-$$"
OW_NAME="velaclaw-openwebui-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "VELACLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "VELACLAW_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/velaclaw-openwebui-config.XXXXXX.json)"
    VELACLAW_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.VELACLAW_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.VELACLAW_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.VELACLAW_OPENWEBUI_MODEL },
];
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Velaclaw E2E" \
  -e WEBUI_SECRET_KEY="velaclaw-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Velaclaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="velaclaw/default" \
...
echo "Running Open WebUI -> Velaclaw smoke..."
if ! docker exec \
