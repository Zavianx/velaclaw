...
IMAGE_NAME="velaclaw-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/velaclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/velaclaw-bin/systemctl

  cat > /tmp/velaclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/velaclaw-bin/loginctl
  ...
  npm_log="/tmp/velaclaw-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/velaclaw"
	  npm_root="/tmp/npm-prefix/lib/node_modules/velaclaw"
  ...
  run_flow() {
    ...
    local install_log="/tmp/velaclaw-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/velaclaw-doctor-switch-${name}-doctor.log"

    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/velaclaw-switch-${name}.XXXXXX")
    ...
    unit_path="$HOME/.config/systemd/user/velaclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
