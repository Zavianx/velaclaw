...
type PackageJson = {
  ...
  velaclaw?: {
    ...
    build?: {
      velaclawVersion?: string;
    };
  };
};

const VELACLAW_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncVelaclawDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.velaclaw;
  if (!current || current === "workspace:*" || !VELACLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.velaclaw?.compat;
  const current = compat?.pluginApi;
  if (!current || !VELACLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildVelaclawVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.velaclaw?.build;
  const current = build?.velaclawVersion;
  if (!current) {
    return false;
  }
  ...
  build.velaclawVersion = targetVersion;
  return true;
}
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Velaclaw release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncVelaclawDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncVelaclawDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildVelaclawVersionChanged = syncBuildVelaclawVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildVelaclawVersionChanged;
    if (!packageChanged) {
