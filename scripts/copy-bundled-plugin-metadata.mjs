...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.VELACLAW_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@velaclaw/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "velaclaw.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "velaclaw.plugin.json");
    ...
    if (packageJson.velaclaw && "extensions" in packageJson.velaclaw) {
      packageJson.velaclaw = {
        ...packageJson.velaclaw,
        extensions: rewritePackageExtensions(packageJson.velaclaw.extensions),
        ...(typeof packageJson.velaclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.velaclaw.setupEntry) }
          : {}),
      };
    }
