...
# Build and bundle Velaclaw into a minimal .app we can open.
# Outputs to dist/Velaclaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Velaclaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Velaclaw"
BUNDLE_ID="${BUNDLE_ID:-ai.velaclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/Zavianx/velaclaw-dev/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Velaclaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :VelaclawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :VelaclawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Velaclaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Velaclaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Velaclaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Velaclaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Velaclaw/Resources/Velaclaw.icns" "$APP_ROOT/Contents/Resources/Velaclaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Velaclaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying VelaclawKit resources"
VELACLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/VelaclawKit_VelaclawKit.bundle"
if [ -d "$VELACLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/VelaclawKit_VelaclawKit.bundle"
  cp -R "$VELACLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/VelaclawKit_VelaclawKit.bundle"
else
  echo "WARN: VelaclawKit resource bundle not found at $VELACLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Velaclaw"
killall -q Velaclaw 2>/dev/null || true
