...
function makePluginOwnedTempDir(pluginDir, label) {
  return makeTempDir(pluginDir, `.velaclaw-runtime-deps-${label}-`);
}
...
function replaceDirAtomically(targetPath, sourcePath) {
  ...
  const backupPath = makeTempDir(
    targetParentDir,
    `.velaclaw-runtime-deps-backup-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function writeJsonAtomically(targetPath, value) {
  ...
  const tempDir = makeTempDir(
    targetParentDir,
    `.velaclaw-runtime-deps-stamp-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function createRuntimeInstallManifest(pluginId, pinnedGroups) {
  const manifest = {
    name: `velaclaw-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}`,
    private: true,
    version: "0.0.0",
  };
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".velaclaw-runtime-deps-stamp.json");
}
