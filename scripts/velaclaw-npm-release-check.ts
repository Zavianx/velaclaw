...
const EXPECTED_REPOSITORY_URL = "https://github.com/Zavianx/velaclaw-dev";
...
const skipPackValidationEnv = "VELACLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "velaclaw") {
    errors.push(`package.json name must be "velaclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.velaclaw !== "velaclaw.mjs") {
    errors.push(
      `package.json bin.velaclaw must be "velaclaw.mjs"; found "${pkg.bin?.velaclaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`velaclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `velaclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
