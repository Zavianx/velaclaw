...
type DriverMode = "token" | "webhook" | "velaclaw";
...
function resolveStateDir(): string {
  const override = process.env.VELACLAW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.VELACLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".velaclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver velaclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Velaclaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|velaclaw> Driver transport mode (default: token)\n" +
    ...
    "  --velaclaw-bin <path>        Velaclaw CLI binary for driver=velaclaw (default: velaclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  VELACLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  VELACLAW_DISCORD_SMOKE_DRIVER\n" +
    "  VELACLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  VELACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  VELACLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  VELACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  VELACLAW_DISCORD_SMOKE_AGENT\n" +
    "  VELACLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  VELACLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  VELACLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  VELACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  VELACLAW_DISCORD_SMOKE_VELACLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.VELACLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.VELACLAW_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "velaclaw"
        ? "velaclaw"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.VELACLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.VELACLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.VELACLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.VELACLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.VELACLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.VELACLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.VELACLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.VELACLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.VELACLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.VELACLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const velaclawBin =
    resolveArg("--velaclaw-bin") || process.env.VELACLAW_DISCORD_SMOKE_VELACLAW_BIN || "velaclaw";
...
async function velaclawCliJson<T>(params: { velaclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.velaclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`velaclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithVelaclaw(params: {
  velaclawBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await velaclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    velaclawBin: params.velaclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "velaclaw") {
    return await readMessagesWithVelaclaw({
      velaclawBin: params.args.velaclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `velaclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await velaclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("velaclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "velaclaw"
            ? await readMessagesWithVelaclaw({
                velaclawBin: args.velaclawBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Velaclaw.`,
        diagnostics: {
          bindingCandidates: [
            {
