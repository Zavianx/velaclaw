#!/bin/bash
# Setup Velaclaw Auth Management System
...
echo "=== Velaclaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'velaclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Velaclaw message: Send warning via Velaclaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/velaclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/velaclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/velaclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now velaclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status velaclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
