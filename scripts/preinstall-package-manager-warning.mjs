...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[velaclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[velaclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[velaclaw] prefer: corepack pnpm install",
  ].join("\n");
}
