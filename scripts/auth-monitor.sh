...
#
# Suggested cron: */30 * * * * /home/admin/velaclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Velaclaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., velaclaw-alerts)
...
STATE_FILE="$HOME/.velaclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Velaclaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use velaclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Velaclaw to $NOTIFY_PHONE..."
            velaclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Velaclaw is down. Run: ssh l36 '~/velaclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
