...
# Velaclaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://velaclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Velaclaw."
...
GUM_VERSION="${VELACLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "⛵ Velaclaw 🦞 Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://velaclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$VELACLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.velaclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_velaclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.velaclaw-* "$npm_root"/velaclaw 2>/dev/null || true
}

extract_velaclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_velaclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/velaclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/velaclaw"|"/usr/local/bin/velaclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/velaclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale velaclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing velaclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Velaclaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_velaclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Velaclaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*velaclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_velaclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_velaclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Velaclaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an velaclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${VELACLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${VELACLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((VELACLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${VELACLAW_NO_ONBOARD:-0}
NO_PROMPT=${VELACLAW_NO_PROMPT:-0}
DRY_RUN=${VELACLAW_DRY_RUN:-0}
INSTALL_METHOD=${VELACLAW_INSTALL_METHOD:-}
VELACLAW_VERSION=${VELACLAW_VERSION:-latest}
USE_BETA=${VELACLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/velaclaw"
GIT_DIR=${VELACLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${VELACLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${VELACLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${VELACLAW_VERBOSE:-0}"
VERIFY_INSTALL="${VELACLAW_VERIFY_INSTALL:-0}"
VELACLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Velaclaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://velaclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/velaclaw)
...
Environment variables:
  VELACLAW_INSTALL_METHOD=git|npm
  VELACLAW_VERSION=latest|next|main|<semver>|<spec>
  VELACLAW_BETA=0|1
  VELACLAW_GIT_DIR=...
  VELACLAW_GIT_UPDATE=0|1
  VELACLAW_NO_PROMPT=1
  VELACLAW_VERIFY_INSTALL=1
  VELACLAW_DRY_RUN=1
  VELACLAW_NO_ONBOARD=1
  VELACLAW_VERBOSE=1
  VELACLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
...
Examples:
  ...
  curl -fsSL --proto '=https' --tlsv1.2 https://velaclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://velaclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://velaclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Velaclaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Velaclaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_velaclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"velaclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://velaclaw.ai/install.sh | bash"
}
...
ensure_velaclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/velaclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/velaclaw" ]]; then
        ln -sf "$npm_root/velaclaw/dist/entry.js" "${npm_bin}/velaclaw"
        ui_info "Created velaclaw bin link at ${npm_bin}/velaclaw"
    fi
    return 0
}

# Check for existing Velaclaw installation
check_existing_velaclaw() {
    if [[ -n "$(type -P velaclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing Velaclaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make velaclaw show as \"command not found\" in new terminals."
...
warn_velaclaw_not_found() {
    ui_warn "Installed, but velaclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t velaclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named velaclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_velaclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P velaclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/velaclaw" ]]; then
        echo "${npm_bin}/velaclaw"
        return 0
    fi
...
install_velaclaw_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        ui_info "Installing Velaclaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Velaclaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Velaclaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/velaclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/velaclaw"
    ui_success "Velaclaw wrapper installed to \$HOME/.local/bin/velaclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view velaclaw dist-tags.beta 2>/dev/null || true)"
...
install_velaclaw() {
    local package_name="velaclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            VELACLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="velaclaw"
        else
            VELACLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${VELACLAW_VERSION}" ]]; then
        VELACLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${VELACLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${VELACLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Velaclaw v${resolved_version}"
    else
        ui_info "Installing Velaclaw (${VELACLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${VELACLAW_VERSION}")"

    if ! install_velaclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_velaclaw_paths
        install_velaclaw_npm "${install_spec}"
    fi

    if [[ "${VELACLAW_VERSION}" == "latest" && "${package_name}" == "velaclaw" ]]; then
        if ! resolve_velaclaw_bin &> /dev/null; then
            ui_warn "npm install velaclaw@latest failed; retrying velaclaw@next"
            cleanup_npm_velaclaw_paths
            install_velaclaw_npm "velaclaw@next"
        fi
    fi

    ensure_velaclaw_bin_link || true

    ui_success "Velaclaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${VELACLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_velaclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (velaclaw not on PATH yet)"
        warn_velaclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${VELACLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.velaclaw/workspace-${profile}"
    else
        echo "${HOME}/.velaclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${VELACLAW_CONFIG_PATH:-$HOME/.Zavianx/velaclaw-dev.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run velaclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but velaclaw not on PATH; skipping onboarding"
        warn_velaclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run velaclaw onboard to retry"
        return
    }
}
...
resolve_velaclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${VELACLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v velaclaw &> /dev/null; then
        claw="$(command -v velaclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_velaclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/velaclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/velaclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: velaclaw not on PATH yet"
        warn_velaclaw_not_found
        return 1
    fi

    run_quiet_step "Checking Velaclaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: velaclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_velaclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Velaclaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set VELACLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_velaclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Velaclaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g velaclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g velaclaw 2>/dev/null || true
        ...
        install_velaclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/velaclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/velaclaw"
    ...
    VELACLAW_BIN="$(resolve_velaclaw_bin || true)"
    ...
    installed_version=$(resolve_velaclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "⛵ Velaclaw 🦞 installed successfully (${installed_version})!"
    else
        ui_celebrate "⛵ Velaclaw 🦞 installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running velaclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                VELACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                VELACLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                VELACLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run velaclaw doctor and velaclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run velaclaw onboard later"
        else
            local config_path="${VELACLAW_CONFIG_PATH:-$HOME/.Zavianx/velaclaw-dev.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (velaclaw not on PATH yet)"
                    warn_velaclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run velaclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v velaclaw &> /dev/null; then
        local claw="${VELACLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_velaclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (velaclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if VELACLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: velaclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${VELACLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
