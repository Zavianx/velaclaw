...
#
# Environment:
#   VELACLAW_NAMESPACE   Kubernetes namespace (default: velaclaw)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MANIFESTS="$SCRIPT_DIR/manifests"
NS="${VELACLAW_NAMESPACE:-velaclaw}"
...
Usage: ./scripts/k8s/deploy.sh [OPTION]
  ...
  (no args)        Deploy Velaclaw (creates secret from env if needed)
...
_apply_secret() {
  ...
  if kubectl get secret velaclaw-secrets -n "$NS" &>/dev/null; then
    EXISTING_SECRET=true
    EXISTING_TOKEN="$(kubectl get secret velaclaw-secrets -n "$NS" -o jsonpath='{.data.VELACLAW_GATEWAY_TOKEN}' | base64 -d)"
    ANTHROPIC_VALUE="$(kubectl get secret velaclaw-secrets -n "$NS" -o jsonpath='{.data.ANTHROPIC_API_KEY}' 2>/dev/null | base64 -d)"
    OPENAI_VALUE="$(kubectl get secret velaclaw-secrets -n "$NS" -o jsonpath='{.data.OPENAI_API_KEY}' 2>/dev/null | base64 -d)"
    GEMINI_VALUE="$(kubectl get secret velaclaw-secrets -n "$NS" -o jsonpath='{.data.GEMINI_API_KEY}' 2>/dev/null | base64 -d)"
    OPENROUTER_VALUE="$(kubectl get secret velaclaw-secrets -n "$NS" -o jsonpath='{.data.OPENROUTER_API_KEY}' 2>/dev/null | base64 -d)"
  fi
  ...
  printf '%s' "$TOKEN" > "$TMP_DIR/VELACLAW_GATEWAY_TOKEN"
  ...
  chmod 600 \
    "$TMP_DIR/VELACLAW_GATEWAY_TOKEN" \
    "$TMP_DIR/ANTHROPIC_API_KEY" \
    "$TMP_DIR/OPENAI_API_KEY" \
    "$TMP_DIR/GEMINI_API_KEY" \
    "$TMP_DIR/OPENROUTER_API_KEY"

  kubectl create secret generic velaclaw-secrets \
    -n "$NS" \
    --from-file=VELACLAW_GATEWAY_TOKEN="$TMP_DIR/VELACLAW_GATEWAY_TOKEN" \
  ...
  kubectl apply --server-side --field-manager=velaclaw -f "$SECRET_MANIFEST" >/dev/null
  # Clean up any annotation left by older client-side apply runs.
  kubectl annotate secret velaclaw-secrets -n "$NS" kubectl.kubernetes.io/last-applied-configuration- >/dev/null 2>&1 || true
  rm -rf "$TMP_DIR"
  trap - EXIT
  ...
  if $SHOW_TOKEN; then
    ...
    echo "  kubectl get secret velaclaw-secrets -n $NS -o jsonpath='{.data.VELACLAW_GATEWAY_TOKEN}' | base64 -d && echo"
  fi
}
...
kubectl rollout restart deployment/velaclaw -n "$NS" 2>/dev/null || true
echo ""
echo "Waiting for rollout..."
kubectl rollout status deployment/velaclaw -n "$NS" --timeout=300s
echo ""
echo "Done. Access the gateway:"
echo "  kubectl port-forward svc/velaclaw 18789:18789 -n $NS"
echo "  open http://localhost:18789"
echo ""
if $SHOW_TOKEN; then
  echo "Gateway token (paste into Control UI):"
  echo "  $(kubectl get secret velaclaw-secrets -n "$NS" -o jsonpath='{.data.VELACLAW_GATEWAY_TOKEN}' | base64 -d)"
echo ""
fi
