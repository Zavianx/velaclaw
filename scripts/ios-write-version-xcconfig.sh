...
- VELACLAW_IOS_VERSION = exact canonical iOS version
- VELACLAW_MARKETING_VERSION = short iOS/App Store version
- VELACLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    VELACLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    VELACLAW_MARKETING_VERSION)
...
VELACLAW_IOS_VERSION = ${IOS_VERSION}
VELACLAW_MARKETING_VERSION = ${MARKETING_VERSION}
VELACLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
