...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[velaclaw] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.VELACLAW_WATCH_MODE = "1";
  childEnv.VELACLAW_WATCH_SESSION = watchSession;
  ...
  childEnv.VELACLAW_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.VELACLAW_WATCH_COMMAND = deps.args.join(" ");
  }
