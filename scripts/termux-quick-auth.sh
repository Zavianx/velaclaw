...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/velaclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Velaclaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/velaclaw/scripts/mobile-reauth.sh'" --id velaclaw-auth
        ;;
    *)
