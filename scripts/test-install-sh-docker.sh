...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${VELACLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$VELACLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.VELACLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `VELACLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${VELACLAW_INSTALL_SMOKE_IMAGE:-velaclaw-install-smoke:local}"
NONROOT_IMAGE="${VELACLAW_INSTALL_NONROOT_IMAGE:-velaclaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${VELACLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${VELACLAW_INSTALL_URL:-https://velaclaw.bot/install.sh}"
CLI_INSTALL_URL="${VELACLAW_INSTALL_CLI_URL:-https://velaclaw.bot/install-cli.sh}"
PACKAGE_NAME="${VELACLAW_INSTALL_PACKAGE:-velaclaw}"
SKIP_NONROOT="${VELACLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${VELACLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${VELACLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${VELACLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${VELACLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${VELACLAW_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${VELACLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${VELACLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${VELACLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${VELACLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${VELACLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${VELACLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (VELACLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e VELACLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e VELACLAW_INSTALL_METHOD=npm \
    -e VELACLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e VELACLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e VELACLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e VELACLAW_NO_ONBOARD=1 \
    -e VELACLAW_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${UPDATE_DOCKER_HOST_ARGS[@]}" \
    -e VELACLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e VELACLAW_INSTALL_SMOKE_MODE=update \
    -e VELACLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e VELACLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e VELACLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e VELACLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e VELACLAW_NO_ONBOARD=1 \
    -e VELACLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (VELACLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${UPDATE_DOCKER_HOST_ARGS[@]}" \
      -e VELACLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e VELACLAW_INSTALL_SMOKE_MODE=npm-global \
      -e VELACLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e VELACLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e VELACLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e VELACLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (VELACLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e VELACLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e VELACLAW_INSTALL_METHOD=npm \
    -e VELACLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e VELACLAW_NO_ONBOARD=1 \
    -e VELACLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${VELACLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (VELACLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e VELACLAW_INSTALL_URL="$INSTALL_URL" \
  -e VELACLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e VELACLAW_NO_ONBOARD=1 \
  -e VELACLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
