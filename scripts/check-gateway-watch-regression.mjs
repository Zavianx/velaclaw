...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  VELACLAW_SKIP_ACPX_RUNTIME: "1",
  VELACLAW_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$VELACLAW_WATCH_PID_FILE"',
    'mkdir -p "$VELACLAW_HOME/.velaclaw"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}}}' > "$VELACLAW_HOME/.Zavianx/velaclaw-dev.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    VELACLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    VELACLAW_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "velaclaw-gateway-watch-"));
