...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.velaclaw) ? packageJson.velaclaw : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.velaclaw?.channel?.id) || left.name;
    const rightId = trimString(right.velaclaw?.channel?.id) || right.name;
