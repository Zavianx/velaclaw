...
usage() {
  cat <<'EOF'
Usage:
  VELACLAW_PUSH_RELAY_BASE_URL=https://relay.example.com \
...
- regenerates apps/ios/Velaclaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${VELACLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid VELACLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid VELACLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid VELACLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid VELACLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing VELACLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(VELACLAW_URL_SLASH)$(VELACLAW_URL_SLASH)#g'
)"
...
VELACLAW_CODE_SIGN_STYLE = Automatic
VELACLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
VELACLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
VELACLAW_APP_BUNDLE_ID = ai.velaclaw.client
VELACLAW_SHARE_BUNDLE_ID = ai.velaclaw.client.share
VELACLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.velaclaw.client.activitywidget
VELACLAW_WATCH_APP_BUNDLE_ID = ai.velaclaw.client.watchkitapp
VELACLAW_WATCH_EXTENSION_BUNDLE_ID = ai.velaclaw.client.watchkitapp.extension
VELACLAW_APP_PROFILE =
VELACLAW_SHARE_PROFILE =
VELACLAW_PUSH_TRANSPORT = relay
VELACLAW_PUSH_DISTRIBUTION = official
VELACLAW_URL_SLASH = /
VELACLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
VELACLAW_PUSH_APNS_ENVIRONMENT = production
EOF
