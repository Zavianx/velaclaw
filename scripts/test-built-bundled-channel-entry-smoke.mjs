...
process.env.VELACLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

function parseArgs(argv) {
  let packageRoot = process.env.VELACLAW_BUNDLED_CHANNEL_SMOKE_ROOT;
...
const installedLayoutEnv = "VELACLAW_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";

function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/velaclaw");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "velaclaw-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "velaclaw");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    if (!packageJson.velaclaw?.channel) {
      continue;
    }

    const extensionEntries =
      Array.isArray(packageJson.velaclaw.extensions) && packageJson.velaclaw.extensions.length > 0
        ? packageJson.velaclaw.extensions
        : ["./index.ts"];
    ...
    const setupEntry = packageJson.velaclaw.setupEntry;
