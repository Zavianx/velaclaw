#!/usr/bin/env bash
# ClawDock - Docker helpers for Velaclaw
# Inspired by Simon Willison's "Running Velaclaw in Docker"
# https://til.simonwillison.net/llms/velaclaw-docker
#
# Installation:
...
# Common paths to check for Velaclaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/velaclaw"
  "${HOME}/workspace/velaclaw"
  "${HOME}/projects/velaclaw"
  "${HOME}/dev/velaclaw"
  "${HOME}/code/velaclaw"
  "${HOME}/src/velaclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Velaclaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/velaclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Velaclaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^VELACLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d velaclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart velaclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f velaclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.velaclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show velaclaw.json
  if [[ -f "${config_dir}/velaclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/velaclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/velaclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No velaclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.velaclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec velaclaw-gateway \
    bash -c 'echo "alias velaclaw=\"./velaclaw.mjs\"" > /tmp/.bashrc_velaclaw && bash --rcfile /tmp/.bashrc_velaclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm velaclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build velaclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d velaclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build velaclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "VELACLAW_GATEWAY_TOKEN=$token" velaclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" velaclaw-gateway \
    bash -c './velaclaw.mjs config set gateway.remote.token "$TOKEN" && ./velaclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec velaclaw-gateway \
    bash -c "./velaclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart velaclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm velaclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec velaclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'velaclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Velaclaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (velaclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to velaclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.velaclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'velaclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'velaclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.velaclaw.ai${_CLR_RESET}"
  echo ""
}
