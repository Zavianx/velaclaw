...
BASE_IMAGE="${BASE_IMAGE:-velaclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-velaclaw-sandbox-common:bookworm-slim}"
...
VELACLAW_DOCKER_BUILD_USE_BUILDX="${VELACLAW_DOCKER_BUILD_USE_BUILDX:-0}"
VELACLAW_DOCKER_BUILD_CACHE_FROM="${VELACLAW_DOCKER_BUILD_CACHE_FROM:-}"
VELACLAW_DOCKER_BUILD_CACHE_TO="${VELACLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${VELACLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${VELACLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${VELACLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${VELACLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${VELACLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=velaclaw.sandbox=1)
NOTE
