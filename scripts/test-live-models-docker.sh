...
if velaclaw_live_truthy "${VELACLAW_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  VELACLAW_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${VELACLAW_LIVE_PROVIDERS:-}" || -n "${VELACLAW_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      velaclaw_live_collect_auth_dirs_from_csv "${VELACLAW_LIVE_PROVIDERS:-}"
      velaclaw_live_collect_auth_dirs_from_csv "${VELACLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      velaclaw_live_collect_auth_files_from_csv "${VELACLAW_LIVE_PROVIDERS:-}"
      velaclaw_live_collect_auth_files_from_csv "${VELACLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${VELACLAW_DOCKER_PROFILE_ENV_ONLY:-0}"
...
docker run --rm -t \
  ...
  -e VELACLAW_LIVE_MODELS="${VELACLAW_LIVE_MODELS:-modern}" \
  -e VELACLAW_LIVE_PROVIDERS="${VELACLAW_LIVE_PROVIDERS:-}" \
  -e VELACLAW_LIVE_MAX_MODELS="${VELACLAW_LIVE_MAX_MODELS:-12}" \
  -e VELACLAW_LIVE_MODEL_TIMEOUT_MS="${VELACLAW_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e VELACLAW_LIVE_REQUIRE_PROFILE_KEYS="${VELACLAW_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e VELACLAW_LIVE_GATEWAY_MODELS="${VELACLAW_LIVE_GATEWAY_MODELS:-}" \
  -e VELACLAW_LIVE_GATEWAY_PROVIDERS="${VELACLAW_LIVE_GATEWAY_PROVIDERS:-}" \
  -e VELACLAW_LIVE_GATEWAY_MAX_MODELS="${VELACLAW_LIVE_GATEWAY_MAX_MODELS:-}" \
  -v "$ROOT_DIR":/src:ro \
