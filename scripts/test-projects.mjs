...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.VELACLAW_VITEST_MAX_WORKERS || env.VELACLAW_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      VELACLAW_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
  ...
  if (isFullSuiteRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.VELACLAW_TEST_PROJECTS_PARALLEL &&
        !process.env.VELACLAW_VITEST_MAX_WORKERS &&
        !process.env.VELACLAW_TEST_WORKERS &&
